...
- **`define-success`** (`/define-success`): The model blocks any task until the user provides success criteria that a third party not present for the conversation could use to evaluate completion. The bar is third-party evaluability across free-form stop conditions such as metrics, examples, artifacts, tests, output shape, review checks, scope boundaries, or source standards.
- **`no-conclusions`** (`/no-conclusions`): The model initially acts as a pure information and mechanism provider, withholding conclusions, recommendations, diagnoses, or decisions while using a permitted vocabulary of observations and mechanisms. If the user directly pushes a second time, the model gives the answer and anchors it to the observations already presented.
...
12. The skill SHALL block the user's request immediately and ask for free-form third-party evaluable success criteria.
13. The skill SHALL explain that success criteria can include metrics, examples, artifacts, tests, output shape, review checks, source standards, scope boundaries, and stop conditions.
14. The skill SHALL evaluate each criterion against the third-party evaluability test: could someone who has never spoken to the user read this and know whether the task succeeded?
15. The skill SHALL return any criterion that fails the third-party evaluability test with a specific explanation of why it fails.
...
22. The skill SHALL redirect when the user first pushes for conclusions: "Here is what I can observe: [restates]. What are you seeing?"
...
25. The skill SHALL give the answer on a second direct push and tie it to the observations already presented.
26. The skill SHALL include cross-domain examples in the body.
...
[Agent with define-success skill loaded]
         ...
         |     No --> Normal response
         |    Yes
         |     +-- Block and ask for free-form success criteria
         |     +-- Suggest relevant criteria categories
         |     +-- User responds with criteria in any useful shape
         |     +-- Test each criterion: third-party evaluable?
         |     |     Fails? --> Return that criterion with specific feedback
...
#### What it does
A user-invoked slash command that blocks any task until the user provides free-form success criteria that pass a third-party evaluability test. The bar is that someone not present for the conversation could read the criteria and determine whether the task was completed successfully.
...
description: >
  ...
  examples, artifacts, thresholds, tests, output shape, or review checks that define
  when the model should stop. Returns vague, circular, or unverifiable criteria with
  specific feedback on what fails.
---
```
...
4. **Success Criteria Categories** — Detailed explanation of generalized criteria types: metrics, behavior changes, output shape, examples, artifacts, tests, source standards, review checks, scope boundaries, and stop conditions.
...
6. **Algorithm** — Step-by-step execution:
   - Step 1: Detect invocation
   - Step 2: Block and ask for free-form success criteria
   - Step 3: Receive user response
   - Step 4: Test each criterion against third-party evaluability
   - Step 5: Return failing criteria with specific feedback
   - Step 6: Repeat until the criteria pass
   - Step 7: Proceed with original task
7. **Accepted Criteria Preservation** — Criteria that pass are preserved; only failing or missing criteria are returned.
8. **Cross-Domain Examples** — Product, medicine, law, finance, personal
9. **Constraints** — No forced universal template, no accepting vague/circular/unverifiable criteria, no proceeding before criteria are evaluable
10. **Success Criteria** — Verifiable outcomes

#### Edge Cases & Error Handling

- **User provides too few criteria to define completion**: Return with a request for the missing stop condition, verification method, or boundary.
...
9. **Constraints** — Do not conclude in the initial response, never apologize for not concluding, redirect the first push, answer on the second push, and keep the factual-contradiction exception
10. **Success Criteria** — Verifiable outcomes

#### Edge Cases & Error Handling

- **User pushes aggressively**: Redirect the first push with "Here is what I can observe. What are you seeing?" If the user pushes a second time, give the answer and cite the observations that point there.
...
| `define-success` | Define third-party evaluable success criteria before beginning any task. Blocks until free-form stop conditions such as metrics, examples, artifacts, tests, output shape, review checks, or scope boundaries pass the evaluability test. |
...
1. **Basic invocation**: Given `/define-success refactor the auth module`, the model asks what criteria it should check against to know the auth module is completely refactored.
2. **Vague criteria returned**: Given the user responds "the code is cleaner", the model returns it with a request for observable evidence such as module boundaries, line limits, tests, or review checks.
3. **Circular criteria returned**: Given the user responds "the bug is fixed", the model returns it with "What behavior currently occurs? What behavior should occur? How will it be verified?"
4. **Free-form criteria pass**: Given specific, verifiable criteria in bullets, prose, examples, tests, or a mixed format, the model proceeds.
5. **Missing completion boundary**: Given criteria with no stop condition, the model requests the missing boundary.
...
- [ ] Should `no-conclusions` have a stronger exception for safety-critical situations (e.g., the user is about to run `rm -rf /`)? The current design redirects once before answering, but safety-critical contexts may require an immediate direct warning.
