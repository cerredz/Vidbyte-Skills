...
- Implement the ~35 “Niche Strategies by Domain” candidates or their top-three recommendations in this change.
...
5. Migration SHALL require rewrite, delegate, or strike for every open task; “later” SHALL fail. State SHALL track task age and flag tasks migrated three or more times.
...
12. Every classification SHALL run the four-question decision flow and require a one-sentence justification. “Might need someday” SHALL route to Archives/delete rather than Resources.
...
17. INSERT SHALL present sources in 1–3 paragraph chunks and require at least one user-selected mark per chunk with symbol, location/phrase, and required note for `?`/`→`.
18. INSERT SHALL cap `*` at the top 3–5 ideas, challenge all-`✓` marking, and prompt for at least one defensible connection when none appears.
...
20. Confidence calibration SHALL require an original one-sentence explanation for every `✓`; failed explanations downgrade the mark to `?` and update Questions.
...
26. Synthesis SHALL occur immediately when possible or record elapsed time, hide detailed notes, and require 2–3 original sentences describing the argument's movement rather than topic.
...
29. `dr-ta` SHALL attribute Russell Stauffer, teach predict→read→verify, and support source input, `--section <n>`, and `--synthesis`.
...
56. Big Task SHALL be specific, one-day completable, most important, and roughly 2–4 hours; Mediums exactly three at 30–60 minutes; Smalls exactly five at 5–15 minutes.
...
63. Outcome SHALL follow a 2–3 minute visualization when timing is available or an explicit self-timed halt, and require one vivid result-focused sentence.
...
5. Increment migration age, flag age ≥3, close month only after all decisions.
...
3. Classify notes one at a time through the P→A→R→Archive/delete flow with justification.
...
- Resource dumping and “someday” justifications fail.
...
4. Hide details and gate 2–3 sentence throughline synthesis.
...
- “Within 30 minutes” is recorded honestly from timestamps.
...
2. Generate a relational open question and collect ≥3 unaided sentences.
...
- Why rejected: They lack the same full interactive specifications, would dilute quality, and materially exceed “do the same thing” for the ten detailed skills.
