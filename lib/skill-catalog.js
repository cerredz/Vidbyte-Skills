/**
 * Context Protocol
 * Description: Catalog and discovery engine for agentic skills in Vidbyte CLI.
 * Purpose: Provides functions to read, validate, select, and filter skills by version and category.
 * Architecture: Uses Node.js fs/path APIs to scan `skills/` directories, parsing markdown frontmatter.
 * Key Functions:
 *   - discoverSkills: lists and parses all valid skills in the catalog.
 *   - readSkillCategories: parses skills-manifest.json to resolve category mappings.
 *   - filterSkillsByCategory: filters a list of skills down to a specific category.
 *   - filterByVersion: filters skills based on release tiers from lib/skill-versions.json.
 * Relation to Codebase: Serves as the database layer for installation and validation flows.
 * Similar Files: lib/installer.js, scripts/validate.js.
 */
...
const VALID_CATEGORIES = ["learning", "reasoning", "roleplay"];
