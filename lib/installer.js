...
import { discoverSkills, filterByVersion, filterSkillsByCategory, readSkillCategories, selectRequestedSkills } from "./skill-catalog.js";
...
export function installVidbyteSkills(argv, category = null) {
  ...
  const skills = readInstallableSkills(environment, category, options.skillNames, options.version);
...
function readInstallableSkills(environment, category, skillNames, version) {
  const allSkills = discoverSkills(environment.skillsRoot);
  ...
  return filterByVersion(selected, version);
}
