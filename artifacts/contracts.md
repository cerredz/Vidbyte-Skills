...
Contracts also change how you decompose a task. Instead of asking "what should the agent do?", you ask "what would I check to know the agent did it right?" This inversion forces you to make your expectations concrete before the agent writes a single line. The act of designing the contract is often more valuable than reading the agent's output — it surfaces hidden assumptions, edge cases, and quality standards that a prose prompt alone would leave unspoken. A well-written contract is a diagnostic tool as much as a gate: when the agent fails it, the failure tells you exactly what went wrong and where to aim the next iteration.

The contract is also a forcing function for iteration quality. Without a contract, the agent's feedback loop is slow and ambiguous — it finishes, you review, you describe what looks wrong, it tries again, you review again. With a contract, the agent gets a PASS or FAIL in seconds and can steer immediately. The contract does not just check correctness; it compresses the iteration cycle, which is the single largest lever on agent output quality. Every second you invest in designing a tight contract pays back in faster, more accurate iterations.
...
Designing validation contracts is itself a process that benefits from iteration. The following algorithm — brainstorm, choose, refine, repeat — turns contract design into a deliberate, repeatable discipline rather than a one-shot checklist. Run through the full cycle at least five times to converge on a contract pipeline that is both tight and comprehensive.

**Brainstorm.** Generate as many potential contracts as you can without filtering. Think across every dimension — correctness, types, performance, security, architecture, API, data, concurrency, frontend, infrastructure, build, docs, CLI, ML, config, search, caching, logging, i18n. For each dimension, ask: "If the agent got this wrong, how would I know?" and "What is the cheapest check that catches that failure?" Do not judge quality yet. Volume first, curation second.

**Choose.** From the brainstormed set, select the contracts that give the highest failure-detection coverage for the lowest feedback-loop cost. Prefer contracts that catch high-severity failure modes, that run in seconds not minutes, and that are hard for the agent to accidentally satisfy. Drop contracts that overlap, that produce only warnings, or that would take longer to maintain than the problem they prevent. Aim for a pipeline of 5-15 contracts that stack across different dimensions — correctness alone is not enough.

**Refine.** For each chosen contract, sharpen it until it is unambiguous and machine-checkable. Replace prose descriptions with runnable checks. Define the exact input, the exact expected output, and the exact tool invocation. Make sure the contract cannot be gamed — a contract that checks "tests exist" can be satisfied with empty test functions, but a contract that checks "tests pass with >80% line coverage" cannot. Ensure every contract produces a hard PASS or FAIL with no interpretive middle ground.

**Repeat.** Run through the full brainstorm-choose-refine cycle at least five times. Each pass will surface blind spots the previous pass missed. Contracts that seemed essential on pass one may look redundant by pass four. Contracts that seemed impossible to check may become obvious once you reframe the dimension. The goal is convergence — a pipeline where each remaining contract pulls its weight and the whole stack gives you confidence that passing means done. Stop when two consecutive cycles produce the same pipeline, or when every remaining contract is one you would stake the release on.
...
These examples are meant to inspire the model's own contract design, not to be copied exactly. The model should read them as patterns — reusable ideas it can adapt, combine, and remix for the specific task at hand. A contract for a real project will often draw ideas from multiple categories and reshape them to fit the language, framework, and constraints of the work.
