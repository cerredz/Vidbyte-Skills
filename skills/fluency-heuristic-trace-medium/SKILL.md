---
name: fluency-heuristic-trace-medium
description: >
  Use this skill when the user invokes /fluency-heuristic-trace-medium or asks for a medium public reasoning trace using Fluency Heuristic.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Fluency Heuristic as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by inferring value or truth from how easily something is processed instead of a generic response.
---

# Fluency Heuristic Reasoning Trace Medium

## Goal
Use Fluency Heuristic to answer the user's question through inferring value or truth from how easily something is processed, not through a generic checklist or interchangeable trace.
The trace should infer that objects processed more fluently, faster, or more smoothly have higher value or truth, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make fluency heuristic useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Fluency Heuristic move: infer that objects processed more fluently, faster, or more smoothly have higher value or truth.
...
Fluency Heuristic is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "infer that objects processed more fluently, faster, or more smoothly have higher value or truth" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Fluency Heuristic trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Fluency Heuristic needs to inspect.
2. Apply the core move explicitly: infer that objects processed more fluently, faster, or more smoothly have higher value or truth.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Fluency Heuristic rather than from a generic summary.
