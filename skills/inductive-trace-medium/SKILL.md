...
Inductive Reasoning is a evidence generalization strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to move from observations to patterns, estimate strength, and mark where the generalization could fail, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, inductive reasoning changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Inductive Reasoning, not like a separate response pasted after the trace.
