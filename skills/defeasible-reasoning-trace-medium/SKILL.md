---
name: defeasible-reasoning-trace-medium
description: >
  Use this skill when the user invokes /defeasible-reasoning-trace-medium or asks for a medium public reasoning trace using Defeasible Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Defeasible Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by revisable logic instead of a generic response.
---

# Defeasible Reasoning Reasoning Trace Medium

## Goal
Use Defeasible Reasoning to answer the user's question through revisable logic, not through a generic checklist or interchangeable trace.
The trace should make provisional conclusions that can be withdrawn when stronger contrary evidence appears, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on premises, rules, cases, implications, contradictions, and validity checks, because those artifacts make defeasible reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Defeasible Reasoning move: make provisional conclusions that can be withdrawn when stronger contrary evidence appears.
...
Defeasible Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "make provisional conclusions that can be withdrawn when stronger contrary evidence appears" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Defeasible Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Defeasible Reasoning needs to inspect.
2. Apply the core move explicitly: make provisional conclusions that can be withdrawn when stronger contrary evidence appears.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Defeasible Reasoning rather than from a generic summary.
