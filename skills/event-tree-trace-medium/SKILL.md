---
name: event-tree-trace-medium
description: >
  Use this skill when the user invokes /event-tree-trace-medium or asks for a medium public reasoning trace using Event Tree Analysis.
...
# Event Tree Analysis Reasoning Trace Medium
...
Event Tree Analysis is a risk analysis strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to start from an initiating event and branch through possible outcomes and controls, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, event tree analysis changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Event Tree Analysis, not like a separate response pasted after the trace.
