---
name: constraint-removal-trace-medium
description: >
  Use this skill when the user invokes /constraint-removal-trace-medium or asks for a medium public reasoning trace using Constraint Removal.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Constraint Removal as the actual structure of the analysis.
...
# Constraint Removal Reasoning Trace Medium

## Goal
Use Constraint Removal to answer the user's question through creative problem solving, not through a generic checklist or interchangeable trace.
The trace should temporarily remove assumed limits and inspect what new options appear, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on frames, constraints, provocations, alternatives, combinations, and prototypeable ideas, because those artifacts make constraint removal useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Constraint Removal move: temporarily remove assumed limits and inspect what new options appear.
...
Constraint Removal is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "temporarily remove assumed limits and inspect what new options appear" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Constraint Removal trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Constraint Removal needs to inspect.
2. Apply the core move explicitly: temporarily remove assumed limits and inspect what new options appear.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Constraint Removal rather than from a generic summary.
