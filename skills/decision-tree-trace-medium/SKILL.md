---
name: decision-tree-trace-medium
description: >
  Use this skill when the user invokes /decision-tree-trace-medium or asks for a medium public reasoning trace using Decision Tree.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Decision Tree as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by branching decision analysis instead of a generic response.
---

# Decision Tree Reasoning Trace Medium

## Goal
Use Decision Tree to answer the user's question through branching decision analysis, not through a generic checklist or interchangeable trace.
The trace should represent choices, uncertain events, and outcomes as branches that can be compared, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make decision tree useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Decision Tree move: represent choices, uncertain events, and outcomes as branches that can be compared.
...
Decision Tree is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "represent choices, uncertain events, and outcomes as branches that can be compared" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Decision Tree trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Decision Tree needs to inspect.
2. Apply the core move explicitly: represent choices, uncertain events, and outcomes as branches that can be compared.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Decision Tree rather than from a generic summary.
