---
name: naive-diversification-trace-medium
description: >
  Use this skill when the user invokes /naive-diversification-trace-medium or asks for a medium public reasoning trace using Naive Diversification.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Naive Diversification as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by spreading choices evenly when making multiple simultaneous decisions instead of a generic response.
---

# Naive Diversification Reasoning Trace Medium

## Goal
Use Naive Diversification to answer the user's question through spreading choices evenly when making multiple simultaneous decisions, not through a generic checklist or interchangeable trace.
The trace should when making multiple simultaneous choices, spread selections evenly across available options rather than concentrating on the best ones, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make naive diversification useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Naive Diversification move: when making multiple simultaneous choices, spread selections evenly across available options rather than concentrating on the best ones.
...
Naive Diversification is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "when making multiple simultaneous choices, spread selections evenly across available options rather than concentrating on the best ones" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Naive Diversification trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Naive Diversification needs to inspect.
2. Apply the core move explicitly: when making multiple simultaneous choices, spread selections evenly across available options rather than concentrating on the best ones.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Naive Diversification rather than from a generic summary.
