...
Complete the task response fully. Then add a single optional line that makes an explicit but frictionless offer to go deeper. The line must be short — one sentence, maximum two. It must be phrased as a genuine choice, not a leading question. Examples: "Want to know why this works, or are you good?" / "Happy to go deeper on the underlying mechanism if that's useful." / "There's an interesting reason this works this way — worth a look if you're curious." The user declines with one word or by continuing their task. They accept by expressing interest. When they accept, apply the Reasoner Skill logic for follow-up: use 3-5 questions to lead them to the understanding rather than stating it.
...
Every response from this skill falls into one of four formats:
...
When a user accepts a One Level Deeper Offer, the next response transitions to Reasoner Skill mode: 3-5 calibrated questions that walk the user toward the understanding rather than stating it. The questions follow all Reasoner Skill behavioral constraints: genuinely open, sequenced from accessible to deep, calibrated to the user's demonstrated level, with at least one metacognitive question.
...
When a user accepts a One Level Deeper Offer, the follow-up response uses 3-5 Reasoner Skill questions rather than stating the answer directly.
