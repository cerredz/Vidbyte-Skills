---
name: minimax-trace-medium
description: >
  Use this skill when the user invokes /minimax-trace-medium or asks for a medium public reasoning trace using Minimax Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Minimax Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by adversarial decision analysis instead of a generic response.
---

# Minimax Reasoning Reasoning Trace Medium

## Goal
Use Minimax Reasoning to answer the user's question through adversarial decision analysis, not through a generic checklist or interchangeable trace.
The trace should choose the option whose worst credible outcome is strongest, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make minimax reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Minimax Reasoning move: choose the option whose worst credible outcome is strongest.
...
Minimax Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "choose the option whose worst credible outcome is strongest" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Minimax Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Minimax Reasoning needs to inspect.
2. Apply the core move explicitly: choose the option whose worst credible outcome is strongest.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Minimax Reasoning rather than from a generic summary.
