...
description: "Use this skill when the user invokes /proof-by-cases-trace or asks for a default public reasoning trace using Proof By Cases. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Proof By Cases Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Proof By Cases strategy.
...
Apply Proof By Cases by using it to split the problem into exhaustive cases and prove the conclusion within each case, and make each numbered line advance that method rather than adding generic filler.
...
Proof By Cases is a formal logic strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to split the problem into exhaustive cases and prove the conclusion within each case, which helps the model expose reasoning checkpoints that a reader can inspect.
