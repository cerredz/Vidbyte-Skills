---
name: nth-order-effects-trace-medium
description: >
  Use this skill when the user invokes /nth-order-effects-trace-medium or asks for a medium public reasoning trace using Nth Order Effects.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Nth Order Effects as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by consequence analysis instead of a generic response.
---

# Nth Order Effects Reasoning Trace Medium

## Goal
Use Nth Order Effects to answer the user's question through consequence analysis, not through a generic checklist or interchangeable trace.
The trace should extend consequence chains across multiple steps while pruning speculative branches, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make nth order effects useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Nth Order Effects move: extend consequence chains across multiple steps while pruning speculative branches.
...
Nth Order Effects is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "extend consequence chains across multiple steps while pruning speculative branches" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Nth Order Effects trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Nth Order Effects needs to inspect.
2. Apply the core move explicitly: extend consequence chains across multiple steps while pruning speculative branches.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Nth Order Effects rather than from a generic summary.
