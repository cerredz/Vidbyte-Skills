...
description: "Use this skill when the user invokes /comparative-case-trace or asks for a default public reasoning trace using Comparative Case Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Comparative Case Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Comparative Case Reasoning strategy.
...
Apply Comparative Case Reasoning by using it to compare cases systematically to isolate similarities, differences, and transfer limits, and make each numbered line advance that method rather than adding generic filler.
...
Comparative Case Reasoning is a case comparison strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to compare cases systematically to isolate similarities, differences, and transfer limits, which helps the model expose reasoning checkpoints that a reader can inspect.
