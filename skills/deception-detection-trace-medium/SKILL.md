---
name: deception-detection-trace-medium
description: >
  Use this skill when the user invokes /deception-detection-trace-medium or asks for a medium public reasoning trace using Deception Detection.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Deception Detection as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by adversarial evidence analysis instead of a generic response.
---

# Deception Detection Reasoning Trace Medium

## Goal
Use Deception Detection to answer the user's question through adversarial evidence analysis, not through a generic checklist or interchangeable trace.
The trace should ask how evidence could be manipulated, fabricated, omitted, or misread, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make deception detection useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Deception Detection move: ask how evidence could be manipulated, fabricated, omitted, or misread.
...
Deception Detection is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "ask how evidence could be manipulated, fabricated, omitted, or misread" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Deception Detection trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Deception Detection needs to inspect.
2. Apply the core move explicitly: ask how evidence could be manipulated, fabricated, omitted, or misread.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Deception Detection rather than from a generic summary.
