---
name: systematic-inventive-thinking-trace-medium
description: >
  Use this skill when the user invokes /systematic-inventive-thinking-trace-medium or asks for a medium public reasoning trace using Systematic Inventive Thinking.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Systematic Inventive Thinking as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by generating creative solutions by applying five thinking patterns inside a closed world instead of a generic response.
---

# Systematic Inventive Thinking Reasoning Trace Medium

## Goal
Use Systematic Inventive Thinking to answer the user's question through generating creative solutions by applying five thinking patterns inside a closed world, not through a generic checklist or interchangeable trace.
The trace should apply five thinking patterns: subtraction, multiplication, division, task unification, and attribute dependency, within the constraints of a closed world, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make systematic inventive thinking useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Systematic Inventive Thinking move: apply five thinking patterns: subtraction, multiplication, division, task unification, and attribute dependency, within the constraints of a closed world.
...
Systematic Inventive Thinking is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "apply five thinking patterns: subtraction, multiplication, division, task unification, and attribute dependency, within the constraints of a closed world" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Systematic Inventive Thinking trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Systematic Inventive Thinking needs to inspect.
2. Apply the core move explicitly: apply five thinking patterns: subtraction, multiplication, division, task unification, and attribute dependency, within the constraints of a closed world.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Systematic Inventive Thinking rather than from a generic summary.
