---
name: empathy-mapping-trace-medium
description: >
  Use this skill when the user invokes /empathy-mapping-trace-medium or asks for a medium public reasoning trace using Empathy Mapping.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Empathy Mapping as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by capturing what users say, think, do, and feel to build deeper understanding instead of a generic response.
---

# Empathy Mapping Reasoning Trace Medium

## Goal
Use Empathy Mapping to answer the user's question through capturing what users say, think, do, and feel to build deeper understanding, not through a generic checklist or interchangeable trace.
The trace should capture what a user says, thinks, does, and feels across four quadrants to build deeper understanding of their experience, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make empathy mapping useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Empathy Mapping move: capture what a user says, thinks, does, and feels across four quadrants to build deeper understanding of their experience.
...
Empathy Mapping is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "capture what a user says, thinks, does, and feels across four quadrants to build deeper understanding of their experience" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Empathy Mapping trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Empathy Mapping needs to inspect.
2. Apply the core move explicitly: capture what a user says, thinks, does, and feels across four quadrants to build deeper understanding of their experience.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Empathy Mapping rather than from a generic summary.
