---
name: mece-decomposition-trace-medium
description: >
  Use this skill when the user invokes /mece-decomposition-trace-medium or asks for a medium public reasoning trace using MECE Decomposition.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses MECE Decomposition as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by structured decomposition instead of a generic response.
---

# MECE Decomposition Reasoning Trace Medium

## Goal
Use MECE Decomposition to answer the user's question through structured decomposition, not through a generic checklist or interchangeable trace.
The trace should split the problem into mutually exclusive and collectively exhaustive parts before solving each part, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make mece decomposition useful rather than decorative.
...
Build the scratchpad by repeatedly applying the MECE Decomposition move: split the problem into mutually exclusive and collectively exhaustive parts before solving each part.
...
MECE Decomposition is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "split the problem into mutually exclusive and collectively exhaustive parts before solving each part" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong MECE Decomposition trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that MECE Decomposition needs to inspect.
2. Apply the core move explicitly: split the problem into mutually exclusive and collectively exhaustive parts before solving each part.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from MECE Decomposition rather than from a generic summary.
