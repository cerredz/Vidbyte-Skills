...
description: "Use this skill when the user invokes /ooda-loop-trace or asks for a default public reasoning trace using OODA Loop. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# OODA Loop Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the OODA Loop strategy.
...
Apply OODA Loop by using it to observe, orient, decide, and act while updating the trace as the situation changes, and make each numbered line advance that method rather than adding generic filler.
...
OODA Loop is a adaptive decision cycle strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to observe, orient, decide, and act while updating the trace as the situation changes, which helps the model expose reasoning checkpoints that a reader can inspect.
