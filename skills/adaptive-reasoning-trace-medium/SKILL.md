---
name: adaptive-reasoning-trace-medium
description: >
  Use this skill when the user invokes /adaptive-reasoning-trace-medium or asks for a medium public reasoning trace using Adaptive Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Adaptive Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by adjusting the reasoning strategy to match the characteristics of the problem instead of a generic response.
---

# Adaptive Reasoning Reasoning Trace Medium

## Goal
Use Adaptive Reasoning to answer the user's question through adjusting the reasoning strategy to match the characteristics of the problem, not through a generic checklist or interchangeable trace.
The trace should adjust the reasoning strategy to match the characteristics of the problem domain and available information, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make adaptive reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Adaptive Reasoning move: adjust the reasoning strategy to match the characteristics of the problem domain and available information.
...
Adaptive Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "adjust the reasoning strategy to match the characteristics of the problem domain and available information" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Adaptive Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Adaptive Reasoning needs to inspect.
2. Apply the core move explicitly: adjust the reasoning strategy to match the characteristics of the problem domain and available information.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Adaptive Reasoning rather than from a generic summary.
