...
description: "Use this skill when the user invokes /decision-tree-trace or asks for a default public reasoning trace using Decision Tree. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Decision Tree Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Decision Tree strategy.
...
Apply Decision Tree by using it to represent choices, uncertain events, and outcomes as branches that can be compared, and make each numbered line advance that method rather than adding generic filler.
...
Decision Tree is a branching decision analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to represent choices, uncertain events, and outcomes as branches that can be compared, which helps the model expose reasoning checkpoints that a reader can inspect.
