...
description: "Use this skill when the user invokes /bowtie-risk-trace or asks for a default public reasoning trace using Bowtie Risk Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Bowtie Risk Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Bowtie Risk Reasoning strategy.
...
Apply Bowtie Risk Reasoning by using it to connect threats, preventive controls, the central event, recovery controls, and consequences, and make each numbered line advance that method rather than adding generic filler.
...
Bowtie Risk Reasoning is a risk analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to connect threats, preventive controls, the central event, recovery controls, and consequences, which helps the model expose reasoning checkpoints that a reader can inspect.
