...
description: "Use this skill when the user invokes /constraint-satisfaction-trace or asks for a default public reasoning trace using Constraint Satisfaction. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Constraint Satisfaction Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Constraint Satisfaction strategy.
...
Apply Constraint Satisfaction by using it to list constraints, domains, and conflicts, then narrow toward feasible assignments, and make each numbered line advance that method rather than adding generic filler.
...
Constraint Satisfaction is a search and optimization strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to list constraints, domains, and conflicts, then narrow toward feasible assignments, which helps the model expose reasoning checkpoints that a reader can inspect.
