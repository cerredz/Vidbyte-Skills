---
name: bcg-matrix-trace-medium
description: >
  Use this skill when the user invokes /bcg-matrix-trace-medium or asks for a medium public reasoning trace using BCG Growth-Share Matrix.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses BCG Growth-Share Matrix as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by analyzing business units by market growth rate and relative market share instead of a generic response.
---

# BCG Growth-Share Matrix Reasoning Trace Medium

## Goal
Use BCG Growth-Share Matrix to answer the user's question through analyzing business units by market growth rate and relative market share, not through a generic checklist or interchangeable trace.
The trace should plot business units on two dimensions: market growth rate and relative market share, classifying them as stars, cash cows, question marks, or dogs, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make bcg growth-share matrix useful rather than decorative.
...
Build the scratchpad by repeatedly applying the BCG Growth-Share Matrix move: plot business units on two dimensions: market growth rate and relative market share, classifying them as stars, cash cows, question marks, or dogs.
...
BCG Growth-Share Matrix is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "plot business units on two dimensions: market growth rate and relative market share, classifying them as stars, cash cows, question marks, or dogs" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong BCG Growth-Share Matrix trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that BCG Growth-Share Matrix needs to inspect.
2. Apply the core move explicitly: plot business units on two dimensions: market growth rate and relative market share, classifying them as stars, cash cows, question marks, or dogs.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from BCG Growth-Share Matrix rather than from a generic summary.
