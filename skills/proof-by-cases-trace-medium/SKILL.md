---
name: proof-by-cases-trace-medium
description: >
  Use this skill when the user invokes /proof-by-cases-trace-medium or asks for a medium public reasoning trace using Proof By Cases.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Proof By Cases as the actual structure of the analysis.
...
# Proof By Cases Reasoning Trace Medium

## Goal
Use Proof By Cases to answer the user's question through formal logic, not through a generic checklist or interchangeable trace.
The trace should split the problem into exhaustive cases and prove the conclusion within each case, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on premises, rules, cases, implications, contradictions, and validity checks, because those artifacts make proof by cases useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Proof By Cases move: split the problem into exhaustive cases and prove the conclusion within each case.
...
Proof By Cases is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "split the problem into exhaustive cases and prove the conclusion within each case" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Proof By Cases trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Proof By Cases needs to inspect.
2. Apply the core move explicitly: split the problem into exhaustive cases and prove the conclusion within each case.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Proof By Cases rather than from a generic summary.
