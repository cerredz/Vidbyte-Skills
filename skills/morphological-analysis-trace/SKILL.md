...
description: "Use this skill when the user invokes /morphological-analysis-trace or asks for a default public reasoning trace using Morphological Analysis. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Morphological Analysis Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Morphological Analysis strategy.
...
Apply Morphological Analysis by using it to list dimensions of the problem and recombine alternatives across dimensions, and make each numbered line advance that method rather than adding generic filler.
...
Morphological Analysis is a creative decomposition strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to list dimensions of the problem and recombine alternatives across dimensions, which helps the model expose reasoning checkpoints that a reader can inspect.
