---
name: policy-analysis-trace-medium
description: >
  Use this skill when the user invokes /policy-analysis-trace-medium or asks for a medium public reasoning trace using Policy Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Policy Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by public decision analysis instead of a generic response.
---

# Policy Analysis Reasoning Trace Medium

## Goal
Use Policy Analysis to answer the user's question through public decision analysis, not through a generic checklist or interchangeable trace.
The trace should compare policy options by goals, feasibility, effects, equity, and implementation risk, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make policy analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Policy Analysis move: compare policy options by goals, feasibility, effects, equity, and implementation risk.
...
Policy Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "compare policy options by goals, feasibility, effects, equity, and implementation risk" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Policy Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Policy Analysis needs to inspect.
2. Apply the core move explicitly: compare policy options by goals, feasibility, effects, equity, and implementation risk.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Policy Analysis rather than from a generic summary.
