...
description: "Use this skill when the user invokes /forward-chaining-trace or asks for a default public reasoning trace using Forward Chaining. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Forward Chaining Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Forward Chaining strategy.
...
Apply Forward Chaining by using it to start from known facts and apply rules until useful conclusions emerge, and make each numbered line advance that method rather than adding generic filler.
...
Forward Chaining is a data-driven inference strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to start from known facts and apply rules until useful conclusions emerge, which helps the model expose reasoning checkpoints that a reader can inspect.
