---
name: pragmatism-trace-medium
description: >
  Use this skill when the user invokes /pragmatism-trace-medium or asks for a medium public reasoning trace using Pragmatism.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Pragmatism as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by evaluating ideas and beliefs by their practical consequences and usefulness instead of a generic response.
---

# Pragmatism Reasoning Trace Medium

## Goal
Use Pragmatism to answer the user's question through evaluating ideas and beliefs by their practical consequences and usefulness, not through a generic checklist or interchangeable trace.
The trace should evaluate ideas and beliefs by their practical consequences and usefulness in solving real problems, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make pragmatism useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Pragmatism move: evaluate ideas and beliefs by their practical consequences and usefulness in solving real problems.
...
Pragmatism is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "evaluate ideas and beliefs by their practical consequences and usefulness in solving real problems" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Pragmatism trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Pragmatism needs to inspect.
2. Apply the core move explicitly: evaluate ideas and beliefs by their practical consequences and usefulness in solving real problems.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Pragmatism rather than from a generic summary.
