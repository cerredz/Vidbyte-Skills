...
description: "Use this skill when the user invokes /ab-testing-trace or asks for a default public reasoning trace using A B Testing. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# A B Testing Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the A B Testing strategy.
...
Apply A B Testing by using it to compare variants with predefined metrics, sample rules, and decision thresholds, and make each numbered line advance that method rather than adding generic filler.
...
A B Testing is a experimental decision analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to compare variants with predefined metrics, sample rules, and decision thresholds, which helps the model expose reasoning checkpoints that a reader can inspect.
