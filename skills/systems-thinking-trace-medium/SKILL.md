---
name: systems-thinking-trace-medium
description: >
  Use this skill when the user invokes /systems-thinking-trace-medium or asks for a medium public reasoning trace using Systems Thinking.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Systems Thinking as the actual structure of the analysis.
...
# Systems Thinking Reasoning Trace Medium

## Goal
Use Systems Thinking to answer the user's question through systems analysis, not through a generic checklist or interchangeable trace.
The trace should map components, relationships, boundaries, feedback, delays, and emergent behavior, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on boundaries, components, feedback loops, constraints, delays, and leverage points, because those artifacts make systems thinking useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Systems Thinking move: map components, relationships, boundaries, feedback, delays, and emergent behavior.
...
Systems Thinking is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "map components, relationships, boundaries, feedback, delays, and emergent behavior" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Systems Thinking trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Systems Thinking needs to inspect.
2. Apply the core move explicitly: map components, relationships, boundaries, feedback, delays, and emergent behavior.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Systems Thinking rather than from a generic summary.
