---
name: nine-windows-trace-medium
description: >
  Use this skill when the user invokes /nine-windows-trace-medium or asks for a medium public reasoning trace using Nine Windows.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Nine Windows as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by examining a problem across three time horizons and three system levels instead of a generic response.
---

# Nine Windows Reasoning Trace Medium

## Goal
Use Nine Windows to answer the user's question through examining a problem across three time horizons and three system levels, not through a generic checklist or interchangeable trace.
The trace should examine a problem across three time horizons (past, present, future) and three system levels (subsystem, system, supersystem) to discover solution opportunities, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make nine windows useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Nine Windows move: examine a problem across three time horizons (past, present, future) and three system levels (subsystem, system, supersystem) to discover solution opportunities.
...
Nine Windows is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "examine a problem across three time horizons (past, present, future) and three system levels (subsystem, system, supersystem) to discover solution opportunities" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Nine Windows trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Nine Windows needs to inspect.
2. Apply the core move explicitly: examine a problem across three time horizons (past, present, future) and three system levels (subsystem, system, supersystem) to discover solution opportunities.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Nine Windows rather than from a generic summary.
