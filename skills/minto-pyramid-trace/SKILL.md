...
description: "Use this skill when the user invokes /minto-pyramid-trace or asks for a default public reasoning trace using Minto Pyramid. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Minto Pyramid Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Minto Pyramid strategy.
...
Apply Minto Pyramid by using it to state the answer first, group supporting arguments, and order evidence so the logic is easy to inspect, and make each numbered line advance that method rather than adding generic filler.
...
Minto Pyramid is a communication reasoning strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to state the answer first, group supporting arguments, and order evidence so the logic is easy to inspect, which helps the model expose reasoning checkpoints that a reader can inspect.
