---
name: first-principles-trace-medium
description: >
  Use this skill when the user invokes /first-principles-trace-medium or asks for a medium public reasoning trace using First Principles.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses First Principles as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by foundational decomposition instead of a generic response.
---

# First Principles Reasoning Trace Medium

## Goal
Use First Principles to answer the user's question through foundational decomposition, not through a generic checklist or interchangeable trace.
The trace should break claims into primitive facts, ask what must be true at each smaller level, and rebuild the answer upward, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make first principles useful rather than decorative.
...
Build the scratchpad by repeatedly applying the First Principles move: break claims into primitive facts, ask what must be true at each smaller level, and rebuild the answer upward.
...
First Principles is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "break claims into primitive facts, ask what must be true at each smaller level, and rebuild the answer upward" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong First Principles trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that First Principles needs to inspect.
2. Apply the core move explicitly: break claims into primitive facts, ask what must be true at each smaller level, and rebuild the answer upward.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from First Principles rather than from a generic summary.
