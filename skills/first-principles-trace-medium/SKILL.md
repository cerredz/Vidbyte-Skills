...
First Principles is a foundational decomposition strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to break claims into primitive facts, ask what must be true at each smaller level, and rebuild the answer upward, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, first principles changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of First Principles, not like a separate response pasted after the trace.
