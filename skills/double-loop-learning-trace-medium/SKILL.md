---
name: double-loop-learning-trace-medium
description: >
  Use this skill when the user invokes /double-loop-learning-trace-medium or asks for a medium public reasoning trace using Double-Loop Learning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Double-Loop Learning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by questioning underlying assumptions and governing variables, not just correcting actions instead of a generic response.
---

# Double-Loop Learning Reasoning Trace Medium

## Goal
Use Double-Loop Learning to answer the user's question through questioning underlying assumptions and governing variables, not just correcting actions, not through a generic checklist or interchangeable trace.
The trace should question the underlying assumptions, values, and governing variables behind actions, not just whether the actions achieved their stated goal, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make double-loop learning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Double-Loop Learning move: question the underlying assumptions, values, and governing variables behind actions, not just whether the actions achieved their stated goal.
...
Double-Loop Learning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "question the underlying assumptions, values, and governing variables behind actions, not just whether the actions achieved their stated goal" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Double-Loop Learning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Double-Loop Learning needs to inspect.
2. Apply the core move explicitly: question the underlying assumptions, values, and governing variables behind actions, not just whether the actions achieved their stated goal.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Double-Loop Learning rather than from a generic summary.
