---
name: counterfactual-trace-medium
description: >
  Use this skill when the user invokes /counterfactual-trace-medium or asks for a medium public reasoning trace using Counterfactual Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Counterfactual Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by alternative possibility analysis instead of a generic response.
---

# Counterfactual Reasoning Reasoning Trace Medium

## Goal
Use Counterfactual Reasoning to answer the user's question through alternative possibility analysis, not through a generic checklist or interchangeable trace.
The trace should ask what would change under different conditions and use those contrasts to clarify causation, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make counterfactual reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Counterfactual Reasoning move: ask what would change under different conditions and use those contrasts to clarify causation.
...
Counterfactual Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "ask what would change under different conditions and use those contrasts to clarify causation" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Counterfactual Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Counterfactual Reasoning needs to inspect.
2. Apply the core move explicitly: ask what would change under different conditions and use those contrasts to clarify causation.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Counterfactual Reasoning rather than from a generic summary.
