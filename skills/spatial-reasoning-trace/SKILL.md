...
description: "Use this skill when the user invokes /spatial-reasoning-trace or asks for a default public reasoning trace using Spatial Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Spatial Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Spatial Reasoning strategy.
...
Apply Spatial Reasoning by using it to reason about position, adjacency, distance, containment, and movement, and make each numbered line advance that method rather than adding generic filler.
...
Spatial Reasoning is a geometric and layout logic strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to reason about position, adjacency, distance, containment, and movement, which helps the model expose reasoning checkpoints that a reader can inspect.
