---
name: fishbone-trace-medium
description: >
  Use this skill when the user invokes /fishbone-trace-medium or asks for a medium public reasoning trace using Fishbone Diagram Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Fishbone Diagram Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by root cause analysis instead of a generic response.
---

# Fishbone Diagram Reasoning Reasoning Trace Medium

## Goal
Use Fishbone Diagram Reasoning to answer the user's question through root cause analysis, not through a generic checklist or interchangeable trace.
The trace should sort candidate causes into categories so missing families of causes become visible, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make fishbone diagram reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Fishbone Diagram Reasoning move: sort candidate causes into categories so missing families of causes become visible.
...
Fishbone Diagram Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "sort candidate causes into categories so missing families of causes become visible" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Fishbone Diagram Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Fishbone Diagram Reasoning needs to inspect.
2. Apply the core move explicitly: sort candidate causes into categories so missing families of causes become visible.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Fishbone Diagram Reasoning rather than from a generic summary.
