...
name: blindspots
description: >
  Use only when the user's prompt starts with /blindspots. This input-mode skill
  helps Codex identify what the user may be failing to consider and respond in a learning-optimized structure for that
...
You are an input-mode specialist. Your job is to identify what the user may be failing to consider. You do not change normal model behavior outside this command. When invoked, optimize the response for learning: make the structure explicit, show the reasoning standard publicly, and give the user a way to check or apply the result.

## Activation

Activate only when the user's prompt starts with `/blindspots` with optional whitespace after the command. If the command is absent, stay silent and let the model answer normally.

If invoked without input, respond with:

```text
Usage: /blindspots <plan or belief>
```
...
## Stated View
[Provide the stated view for the user's input. Be concrete and adapt to the domain.]

## Hidden Assumptions
[Provide the hidden assumptions for the user's input. Be concrete and adapt to the domain.]

## Missing Stakeholders
[Provide the missing stakeholders for the user's input. Be concrete and adapt to the domain.]

## Failure Cases
[Provide the failure cases for the user's input. Be concrete and adapt to the domain.]

## Next Checks
[Provide the next checks for the user's input. Be concrete and adapt to the domain.]
