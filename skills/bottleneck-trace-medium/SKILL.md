---
name: bottleneck-trace-medium
description: >
  Use this skill when the user invokes /bottleneck-trace-medium or asks for a medium public reasoning trace using Bottleneck Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Bottleneck Reasoning as the actual structure of the analysis.
...
# Bottleneck Reasoning Reasoning Trace Medium

## Goal
Use Bottleneck Reasoning to answer the user's question through constraint analysis, not through a generic checklist or interchangeable trace.
The trace should locate the limiting factor that governs overall throughput or progress, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on boundaries, components, feedback loops, constraints, delays, and leverage points, because those artifacts make bottleneck reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Bottleneck Reasoning move: locate the limiting factor that governs overall throughput or progress.
...
Bottleneck Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "locate the limiting factor that governs overall throughput or progress" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Bottleneck Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Bottleneck Reasoning needs to inspect.
2. Apply the core move explicitly: locate the limiting factor that governs overall throughput or progress.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Bottleneck Reasoning rather than from a generic summary.
