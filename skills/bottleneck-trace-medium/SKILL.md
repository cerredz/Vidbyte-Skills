...
Bottleneck Reasoning is a constraint analysis strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to locate the limiting factor that governs overall throughput or progress, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, bottleneck reasoning changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Bottleneck Reasoning, not like a separate response pasted after the trace.
