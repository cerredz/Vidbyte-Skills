...
description: "Use this skill when the user invokes /syllogistic-trace or asks for a default public reasoning trace using Syllogistic Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Syllogistic Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Syllogistic Reasoning strategy.
...
Apply Syllogistic Reasoning by using it to evaluate categorical premises and derive whether the conclusion follows, and make each numbered line advance that method rather than adding generic filler.
...
Syllogistic Reasoning is a formal logic strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to evaluate categorical premises and derive whether the conclusion follows, which helps the model expose reasoning checkpoints that a reader can inspect.
