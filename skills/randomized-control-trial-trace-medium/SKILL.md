---
name: randomized-control-trial-trace-medium
description: >
  Use this skill when the user invokes /randomized-control-trial-trace-medium or asks for a medium public reasoning trace using Randomized Control Trial.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Randomized Control Trial as the actual structure of the analysis.
...
# Randomized Control Trial Reasoning Trace Medium

## Goal
Use Randomized Control Trial to answer the user's question through causal inference, not through a generic checklist or interchangeable trace.
The trace should compare treatment and control groups while guarding against confounds, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on causes, mechanisms, counterfactual tests, evidence quality, and intervention points, because those artifacts make randomized control trial useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Randomized Control Trial move: compare treatment and control groups while guarding against confounds.
...
Randomized Control Trial is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "compare treatment and control groups while guarding against confounds" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Randomized Control Trial trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Randomized Control Trial needs to inspect.
2. Apply the core move explicitly: compare treatment and control groups while guarding against confounds.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Randomized Control Trial rather than from a generic summary.
