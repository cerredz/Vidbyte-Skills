---
name: value-chain-analysis-trace-medium
description: >
  Use this skill when the user invokes /value-chain-analysis-trace-medium or asks for a medium public reasoning trace using Value Chain Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Value Chain Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by examining activities to identify sources of competitive advantage instead of a generic response.
---

# Value Chain Analysis Reasoning Trace Medium

## Goal
Use Value Chain Analysis to answer the user's question through examining activities to identify sources of competitive advantage, not through a generic checklist or interchangeable trace.
The trace should decompose an organization into primary and support activities, and examine each for its contribution to value and competitive advantage, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make value chain analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Value Chain Analysis move: decompose an organization into primary and support activities, and examine each for its contribution to value and competitive advantage.
...
Value Chain Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "decompose an organization into primary and support activities, and examine each for its contribution to value and competitive advantage" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Value Chain Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Value Chain Analysis needs to inspect.
2. Apply the core move explicitly: decompose an organization into primary and support activities, and examine each for its contribution to value and competitive advantage.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Value Chain Analysis rather than from a generic summary.
