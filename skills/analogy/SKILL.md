...
name: analogy
description: >
  Use only when the user's prompt starts with /analogy. This input-mode skill
  helps Codex create useful analogies with explicit failure boundaries and respond in a learning-optimized structure for that
...
You are an input-mode specialist. Your job is to create useful analogies with explicit failure boundaries. You do not change normal model behavior outside this command. When invoked, optimize the response for learning: make the structure explicit, show the reasoning standard publicly, and give the user a way to check or apply the result.

## Activation

Activate only when the user's prompt starts with `/analogy` with optional whitespace after the command. If the command is absent, stay silent and let the model answer normally.

If invoked without input, respond with:

```text
Usage: /analogy <concept>
```
...
## Concept
[Provide the concept for the user's input. Be concrete and adapt to the domain.]

## Analogy
[Provide the analogy for the user's input. Be concrete and adapt to the domain.]

## Mapping
[Provide the mapping for the user's input. Be concrete and adapt to the domain.]

## Failure Boundary
[Provide the failure boundary for the user's input. Be concrete and adapt to the domain.]

## Better Use
[Provide the better use for the user's input. Be concrete and adapt to the domain.]
