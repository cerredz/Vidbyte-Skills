---
name: alternative-futures-trace-medium
description: >
  Use this skill when the user invokes /alternative-futures-trace-medium or asks for a medium public reasoning trace using Alternative Futures.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Alternative Futures as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by scenario planning instead of a generic response.
---

# Alternative Futures Reasoning Trace Medium

## Goal
Use Alternative Futures to answer the user's question through scenario planning, not through a generic checklist or interchangeable trace.
The trace should build multiple futures from key uncertainties and compare implications across them, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make alternative futures useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Alternative Futures move: build multiple futures from key uncertainties and compare implications across them.
...
Alternative Futures is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "build multiple futures from key uncertainties and compare implications across them" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Alternative Futures trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Alternative Futures needs to inspect.
2. Apply the core move explicitly: build multiple futures from key uncertainties and compare implications across them.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Alternative Futures rather than from a generic summary.
