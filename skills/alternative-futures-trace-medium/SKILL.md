...
Alternative Futures is a scenario planning strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to build multiple futures from key uncertainties and compare implications across them, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, alternative futures changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Alternative Futures, not like a separate response pasted after the trace.
