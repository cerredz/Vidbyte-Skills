---
name: fast-and-frugal-trees-trace-medium
description: >
  Use this skill when the user invokes /fast-and-frugal-trees-trace-medium or asks for a medium public reasoning trace using Fast-and-Frugal Trees.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Fast-and-Frugal Trees as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by classifying with minimal cues in a tree structure for rapid decisions instead of a generic response.
---

# Fast-and-Frugal Trees Reasoning Trace Medium

## Goal
Use Fast-and-Frugal Trees to answer the user's question through classifying with minimal cues in a tree structure for rapid decisions, not through a generic checklist or interchangeable trace.
The trace should classify options by asking a sequence of cues in order, exiting as soon as a classification is reached, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make fast-and-frugal trees useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Fast-and-Frugal Trees move: classify options by asking a sequence of cues in order, exiting as soon as a classification is reached.
...
Fast-and-Frugal Trees is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "classify options by asking a sequence of cues in order, exiting as soon as a classification is reached" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Fast-and-Frugal Trees trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Fast-and-Frugal Trees needs to inspect.
2. Apply the core move explicitly: classify options by asking a sequence of cues in order, exiting as soon as a classification is reached.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Fast-and-Frugal Trees rather than from a generic summary.
