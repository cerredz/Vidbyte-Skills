...
description: "Use this skill when the user invokes /key-assumptions-check-trace or asks for a default public reasoning trace using Key Assumptions Check. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Key Assumptions Check Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Key Assumptions Check strategy.
...
Apply Key Assumptions Check by using it to surface critical assumptions, test fragility, and revise conclusions where assumptions are weak, and make each numbered line advance that method rather than adding generic filler.
...
Key Assumptions Check is a structured analytic technique strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to surface critical assumptions, test fragility, and revise conclusions where assumptions are weak, which helps the model expose reasoning checkpoints that a reader can inspect.
