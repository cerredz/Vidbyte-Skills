---
name: assumption-ladder-trace-medium
description: >
  Use this skill when the user invokes /assumption-ladder-trace-medium or asks for a medium public reasoning trace using Assumption Ladder.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Assumption Ladder as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by metacognitive analysis instead of a generic response.
---

# Assumption Ladder Reasoning Trace Medium

## Goal
Use Assumption Ladder to answer the user's question through metacognitive analysis, not through a generic checklist or interchangeable trace.
The trace should climb from observations to interpretations, beliefs, and actions while checking each rung, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make assumption ladder useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Assumption Ladder move: climb from observations to interpretations, beliefs, and actions while checking each rung.
...
Assumption Ladder is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "climb from observations to interpretations, beliefs, and actions while checking each rung" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Assumption Ladder trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Assumption Ladder needs to inspect.
2. Apply the core move explicitly: climb from observations to interpretations, beliefs, and actions while checking each rung.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Assumption Ladder rather than from a generic summary.
