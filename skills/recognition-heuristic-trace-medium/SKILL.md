---
name: recognition-heuristic-trace-medium
description: >
  Use this skill when the user invokes /recognition-heuristic-trace-medium or asks for a medium public reasoning trace using Recognition Heuristic.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Recognition Heuristic as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by inferring higher value for a recognized option over an unrecognized one instead of a generic response.
---

# Recognition Heuristic Reasoning Trace Medium

## Goal
Use Recognition Heuristic to answer the user's question through inferring higher value for a recognized option over an unrecognized one, not through a generic checklist or interchangeable trace.
The trace should when one option is recognized and the other is not, infer the recognized option has the higher value on the criterion, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make recognition heuristic useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Recognition Heuristic move: when one option is recognized and the other is not, infer the recognized option has the higher value on the criterion.
...
Recognition Heuristic is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "when one option is recognized and the other is not, infer the recognized option has the higher value on the criterion" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Recognition Heuristic trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Recognition Heuristic needs to inspect.
2. Apply the core move explicitly: when one option is recognized and the other is not, infer the recognized option has the higher value on the criterion.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Recognition Heuristic rather than from a generic summary.
