...
description: "Use this skill when the user invokes /random-stimulus-trace or asks for a default public reasoning trace using Random Stimulus. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Random Stimulus Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Random Stimulus strategy.
...
Apply Random Stimulus by using it to introduce an unrelated cue and force useful connections to the target problem, and make each numbered line advance that method rather than adding generic filler.
...
Random Stimulus is a creative provocation strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to introduce an unrelated cue and force useful connections to the target problem, which helps the model expose reasoning checkpoints that a reader can inspect.
