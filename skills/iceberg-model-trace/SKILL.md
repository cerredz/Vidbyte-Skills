...
description: "Use this skill when the user invokes /iceberg-model-trace or asks for a default public reasoning trace using Iceberg Model. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Iceberg Model Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Iceberg Model strategy.
...
Apply Iceberg Model by using it to move from visible events to patterns, structures, and mental models underneath, and make each numbered line advance that method rather than adding generic filler.
...
Iceberg Model is a systems analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to move from visible events to patterns, structures, and mental models underneath, which helps the model expose reasoning checkpoints that a reader can inspect.
