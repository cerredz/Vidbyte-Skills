...
description: "Use this skill when the user invokes /regression-reasoning-trace or asks for a default public reasoning trace using Regression Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Regression Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Regression Reasoning strategy.
...
Apply Regression Reasoning by using it to explain relationships while checking controls, residuals, and model limits, and make each numbered line advance that method rather than adding generic filler.
...
Regression Reasoning is a statistical modeling strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to explain relationships while checking controls, residuals, and model limits, which helps the model expose reasoning checkpoints that a reader can inspect.
