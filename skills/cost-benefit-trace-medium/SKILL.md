---
name: cost-benefit-trace-medium
description: >
  Use this skill when the user invokes /cost-benefit-trace-medium or asks for a medium public reasoning trace using Cost Benefit Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Cost Benefit Reasoning as the actual structure of the analysis.
...
# Cost Benefit Reasoning Reasoning Trace Medium

## Goal
Use Cost Benefit Reasoning to answer the user's question through decision analysis, not through a generic checklist or interchangeable trace.
The trace should identify costs, benefits, timing, risk, and distribution before recommending action, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make cost benefit reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Cost Benefit Reasoning move: identify costs, benefits, timing, risk, and distribution before recommending action.
...
Cost Benefit Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "identify costs, benefits, timing, risk, and distribution before recommending action" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Cost Benefit Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Cost Benefit Reasoning needs to inspect.
2. Apply the core move explicitly: identify costs, benefits, timing, risk, and distribution before recommending action.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Cost Benefit Reasoning rather than from a generic summary.
