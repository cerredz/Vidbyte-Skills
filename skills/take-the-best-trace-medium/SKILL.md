---
name: take-the-best-trace-medium
description: >
  Use this skill when the user invokes /take-the-best-trace-medium or asks for a medium public reasoning trace using Take-the-Best Heuristic.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Take-the-Best Heuristic as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by searching cues by validity and stopping at the first discriminating cue instead of a generic response.
---

# Take-the-Best Heuristic Reasoning Trace Medium

## Goal
Use Take-the-Best Heuristic to answer the user's question through searching cues by validity and stopping at the first discriminating cue, not through a generic checklist or interchangeable trace.
The trace should search cues in order of validity and stop at the first cue that discriminates between alternatives, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make take-the-best heuristic useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Take-the-Best Heuristic move: search cues in order of validity and stop at the first cue that discriminates between alternatives.
...
Take-the-Best Heuristic is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "search cues in order of validity and stop at the first cue that discriminates between alternatives" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Take-the-Best Heuristic trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Take-the-Best Heuristic needs to inspect.
2. Apply the core move explicitly: search cues in order of validity and stop at the first cue that discriminates between alternatives.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Take-the-Best Heuristic rather than from a generic summary.
