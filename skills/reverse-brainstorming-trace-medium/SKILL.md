---
name: reverse-brainstorming-trace-medium
description: >
  Use this skill when the user invokes /reverse-brainstorming-trace-medium or asks for a medium public reasoning trace using Reverse Brainstorming.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Reverse Brainstorming as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by creative inversion instead of a generic response.
---

# Reverse Brainstorming Reasoning Trace Medium

## Goal
Use Reverse Brainstorming to answer the user's question through creative inversion, not through a generic checklist or interchangeable trace.
The trace should ask how to worsen the outcome, then reverse those causes into improvements, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on frames, constraints, provocations, alternatives, combinations, and prototypeable ideas, because those artifacts make reverse brainstorming useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Reverse Brainstorming move: ask how to worsen the outcome, then reverse those causes into improvements.
...
Reverse Brainstorming is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "ask how to worsen the outcome, then reverse those causes into improvements" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Reverse Brainstorming trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Reverse Brainstorming needs to inspect.
2. Apply the core move explicitly: ask how to worsen the outcome, then reverse those causes into improvements.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Reverse Brainstorming rather than from a generic summary.
