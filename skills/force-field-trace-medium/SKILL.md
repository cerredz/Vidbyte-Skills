---
name: force-field-trace-medium
description: >
  Use this skill when the user invokes /force-field-trace-medium or asks for a medium public reasoning trace using Force Field Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Force Field Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by identifying driving and restraining forces affecting a situation instead of a generic response.
---

# Force Field Analysis Reasoning Trace Medium

## Goal
Use Force Field Analysis to answer the user's question through identifying driving and restraining forces affecting a situation, not through a generic checklist or interchangeable trace.
The trace should identify driving and restraining forces affecting a situation, then assess which can be strengthened or weakened, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make force field analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Force Field Analysis move: identify driving and restraining forces affecting a situation, then assess which can be strengthened or weakened.
...
Force Field Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "identify driving and restraining forces affecting a situation, then assess which can be strengthened or weakened" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Force Field Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Force Field Analysis needs to inspect.
2. Apply the core move explicitly: identify driving and restraining forces affecting a situation, then assess which can be strengthened or weakened.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Force Field Analysis rather than from a generic summary.
