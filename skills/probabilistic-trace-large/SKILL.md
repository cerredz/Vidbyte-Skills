---
name: probabilistic-trace-large
description: >
  Use this skill when the user invokes /probabilistic-trace-large or asks for a large public reasoning trace using Probabilistic Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Probabilistic Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by uncertainty analysis instead of a generic response.
---

# Probabilistic Reasoning Reasoning Trace Large

## Goal
Use Probabilistic Reasoning to answer the user's question through uncertainty analysis, not through a generic checklist or interchangeable trace.
The trace should represent uncertainty explicitly, compare likelihoods, and avoid binary certainty when evidence is incomplete, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make probabilistic reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Probabilistic Reasoning move: represent uncertainty explicitly, compare likelihoods, and avoid binary certainty when evidence is incomplete.
...
Probabilistic Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "represent uncertainty explicitly, compare likelihoods, and avoid binary certainty when evidence is incomplete" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Probabilistic Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Probabilistic Reasoning needs to inspect.
2. Apply the core move explicitly: represent uncertainty explicitly, compare likelihoods, and avoid binary certainty when evidence is incomplete.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Probabilistic Reasoning rather than from a generic summary.
