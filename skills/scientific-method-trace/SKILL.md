...
description: "Use this skill when the user invokes /scientific-method-trace or asks for a default public reasoning trace using Scientific Method. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Scientific Method Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Scientific Method strategy.
...
Apply Scientific Method by using it to form a question, propose hypotheses, test them, and revise based on observations, and make each numbered line advance that method rather than adding generic filler.
...
Scientific Method is a empirical inquiry strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to form a question, propose hypotheses, test them, and revise based on observations, which helps the model expose reasoning checkpoints that a reader can inspect.
