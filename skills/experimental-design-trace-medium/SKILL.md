---
name: experimental-design-trace-medium
description: >
  Use this skill when the user invokes /experimental-design-trace-medium or asks for a medium public reasoning trace using Experimental Design.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Experimental Design as the actual structure of the analysis.
...
# Experimental Design Reasoning Trace Medium

## Goal
Use Experimental Design to answer the user's question through empirical inquiry, not through a generic checklist or interchangeable trace.
The trace should define variables, controls, measures, randomization, and validity risks, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make experimental design useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Experimental Design move: define variables, controls, measures, randomization, and validity risks.
...
Experimental Design is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "define variables, controls, measures, randomization, and validity risks" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Experimental Design trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Experimental Design needs to inspect.
2. Apply the core move explicitly: define variables, controls, measures, randomization, and validity risks.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Experimental Design rather than from a generic summary.
