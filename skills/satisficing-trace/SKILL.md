...
description: "Use this skill when the user invokes /satisficing-trace or asks for a default public reasoning trace using Satisficing Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Satisficing Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Satisficing Reasoning strategy.
...
Apply Satisficing Reasoning by using it to define adequate thresholds and choose an option that meets them under real constraints, and make each numbered line advance that method rather than adding generic filler.
...
Satisficing Reasoning is a bounded rationality strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to define adequate thresholds and choose an option that meets them under real constraints, which helps the model expose reasoning checkpoints that a reader can inspect.
