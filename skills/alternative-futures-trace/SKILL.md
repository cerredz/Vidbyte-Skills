...
description: "Use this skill when the user invokes /alternative-futures-trace or asks for a default public reasoning trace using Alternative Futures. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Alternative Futures Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Alternative Futures strategy.
...
Apply Alternative Futures by using it to build multiple futures from key uncertainties and compare implications across them, and make each numbered line advance that method rather than adding generic filler.
...
Alternative Futures is a scenario planning strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to build multiple futures from key uncertainties and compare implications across them, which helps the model expose reasoning checkpoints that a reader can inspect.
