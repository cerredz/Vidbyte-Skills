---
name: cone-of-plausibility-trace-medium
description: >
  Use this skill when the user invokes /cone-of-plausibility-trace-medium or asks for a medium public reasoning trace using Cone Of Plausibility.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Cone Of Plausibility as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by forecast framing instead of a generic response.
---

# Cone Of Plausibility Reasoning Trace Medium

## Goal
Use Cone Of Plausibility to answer the user's question through forecast framing, not through a generic checklist or interchangeable trace.
The trace should separate probable, plausible, and possible futures so confidence stays calibrated, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make cone of plausibility useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Cone Of Plausibility move: separate probable, plausible, and possible futures so confidence stays calibrated.
...
Cone Of Plausibility is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "separate probable, plausible, and possible futures so confidence stays calibrated" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Cone Of Plausibility trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Cone Of Plausibility needs to inspect.
2. Apply the core move explicitly: separate probable, plausible, and possible futures so confidence stays calibrated.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Cone Of Plausibility rather than from a generic summary.
