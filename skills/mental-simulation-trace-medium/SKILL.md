---
name: mental-simulation-trace-medium
description: >
  Use this skill when the user invokes /mental-simulation-trace-medium or asks for a medium public reasoning trace using Mental Simulation.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Mental Simulation as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by envisioning future events in concrete detail to assess feasibility and risks instead of a generic response.
---

# Mental Simulation Reasoning Trace Medium

## Goal
Use Mental Simulation to answer the user's question through envisioning future events in concrete detail to assess feasibility and risks, not through a generic checklist or interchangeable trace.
The trace should envision a future event in concrete, step-by-step detail to assess whether it is feasible and to identify potential obstacles, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make mental simulation useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Mental Simulation move: envision a future event in concrete, step-by-step detail to assess whether it is feasible and to identify potential obstacles.
...
Mental Simulation is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "envision a future event in concrete, step-by-step detail to assess whether it is feasible and to identify potential obstacles" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Mental Simulation trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Mental Simulation needs to inspect.
2. Apply the core move explicitly: envision a future event in concrete, step-by-step detail to assess whether it is feasible and to identify potential obstacles.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Mental Simulation rather than from a generic summary.
