...
Base Rate Reasoning is a forecast calibration strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to start from reference frequencies before adjusting for case-specific evidence, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, base rate reasoning changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Base Rate Reasoning, not like a separate response pasted after the trace.
