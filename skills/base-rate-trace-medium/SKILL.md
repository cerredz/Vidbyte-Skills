---
name: base-rate-trace-medium
description: >
  Use this skill when the user invokes /base-rate-trace-medium or asks for a medium public reasoning trace using Base Rate Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Base Rate Reasoning as the actual structure of the analysis.
...
# Base Rate Reasoning Reasoning Trace Medium

## Goal
Use Base Rate Reasoning to answer the user's question through forecast calibration, not through a generic checklist or interchangeable trace.
The trace should start from reference frequencies before adjusting for case-specific evidence, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make base rate reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Base Rate Reasoning move: start from reference frequencies before adjusting for case-specific evidence.
...
Base Rate Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "start from reference frequencies before adjusting for case-specific evidence" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Base Rate Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Base Rate Reasoning needs to inspect.
2. Apply the core move explicitly: start from reference frequencies before adjusting for case-specific evidence.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Base Rate Reasoning rather than from a generic summary.
