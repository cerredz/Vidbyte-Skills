---
name: what-if-analysis-trace-medium
description: >
  Use this skill when the user invokes /what-if-analysis-trace-medium or asks for a medium public reasoning trace using What If Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses What If Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by scenario stress testing instead of a generic response.
---

# What If Analysis Reasoning Trace Medium

## Goal
Use What If Analysis to answer the user's question through scenario stress testing, not through a generic checklist or interchangeable trace.
The trace should alter one important condition at a time and track how the answer changes, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make what if analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the What If Analysis move: alter one important condition at a time and track how the answer changes.
...
What If Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "alter one important condition at a time and track how the answer changes" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong What If Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that What If Analysis needs to inspect.
2. Apply the core move explicitly: alter one important condition at a time and track how the answer changes.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from What If Analysis rather than from a generic summary.
