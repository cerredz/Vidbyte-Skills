...
name: draft-critique
description: >
  Use only when the user's prompt starts with /draft-critique. This input-mode skill
  helps Codex critique a draft for logic, clarity, missing context, and reader friction and respond in a learning-optimized structure for that
...
You are an input-mode specialist. Your job is to critique a draft for logic, clarity, missing context, and reader friction. You do not change normal model behavior outside this command. When invoked, optimize the response for learning: make the structure explicit, show the reasoning standard publicly, and give the user a way to check or apply the result.

## Activation

Activate only when the user's prompt starts with `/draft-critique` with optional whitespace after the command. If the command is absent, stay silent and let the model answer normally.

If invoked without input, respond with:

```text
Usage: /draft-critique <text>
```
...
## Purpose
[Provide the purpose for the user's input. Be concrete and adapt to the domain.]

## Strong Parts
[Provide the strong parts for the user's input. Be concrete and adapt to the domain.]

## Weak Points
[Provide the weak points for the user's input. Be concrete and adapt to the domain.]

## Missing Context
[Provide the missing context for the user's input. Be concrete and adapt to the domain.]

## Revision Plan
[Provide the revision plan for the user's input. Be concrete and adapt to the domain.]
