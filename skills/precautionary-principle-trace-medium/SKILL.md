---
name: precautionary-principle-trace-medium
description: >
  Use this skill when the user invokes /precautionary-principle-trace-medium or asks for a medium public reasoning trace using Precautionary Principle.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Precautionary Principle as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by taking preventive action when an activity raises threats of harm even without full scientific certainty instead of a generic response.
---

# Precautionary Principle Reasoning Trace Medium

## Goal
Use Precautionary Principle to answer the user's question through taking preventive action when an activity raises threats of harm even without full scientific certainty, not through a generic checklist or interchangeable trace.
The trace should when an activity raises threats of harm to human health or the environment, take precautionary measures even if cause-and-effect relationships are not fully established scientifically, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make precautionary principle useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Precautionary Principle move: when an activity raises threats of harm to human health or the environment, take precautionary measures even if cause-and-effect relationships are not fully established scientifically.
...
Precautionary Principle is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "when an activity raises threats of harm to human health or the environment, take precautionary measures even if cause-and-effect relationships are not fully established scientifically" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Precautionary Principle trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Precautionary Principle needs to inspect.
2. Apply the core move explicitly: when an activity raises threats of harm to human health or the environment, take precautionary measures even if cause-and-effect relationships are not fully established scientifically.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Precautionary Principle rather than from a generic summary.
