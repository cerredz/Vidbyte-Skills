---
name: premortem-trace-medium
description: >
  Use this skill when the user invokes /premortem-trace-medium or asks for a medium public reasoning trace using Premortem Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Premortem Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by failure anticipation instead of a generic response.
---

# Premortem Reasoning Reasoning Trace Medium

## Goal
Use Premortem Reasoning to answer the user's question through failure anticipation, not through a generic checklist or interchangeable trace.
The trace should assume the plan failed, identify causes of failure, and convert them into mitigations, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make premortem reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Premortem Reasoning move: assume the plan failed, identify causes of failure, and convert them into mitigations.
...
Premortem Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "assume the plan failed, identify causes of failure, and convert them into mitigations" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Premortem Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Premortem Reasoning needs to inspect.
2. Apply the core move explicitly: assume the plan failed, identify causes of failure, and convert them into mitigations.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Premortem Reasoning rather than from a generic summary.
