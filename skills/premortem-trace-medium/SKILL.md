...
Premortem Reasoning is a failure anticipation strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to assume the plan failed, identify causes of failure, and convert them into mitigations, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, premortem reasoning changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Premortem Reasoning, not like a separate response pasted after the trace.
