---
name: random-stimulus-trace-medium
description: >
  Use this skill when the user invokes /random-stimulus-trace-medium or asks for a medium public reasoning trace using Random Stimulus.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Random Stimulus as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by creative provocation instead of a generic response.
---

# Random Stimulus Reasoning Trace Medium

## Goal
Use Random Stimulus to answer the user's question through creative provocation, not through a generic checklist or interchangeable trace.
The trace should introduce an unrelated cue and force useful connections to the target problem, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on frames, constraints, provocations, alternatives, combinations, and prototypeable ideas, because those artifacts make random stimulus useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Random Stimulus move: introduce an unrelated cue and force useful connections to the target problem.
...
Random Stimulus is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "introduce an unrelated cue and force useful connections to the target problem" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Random Stimulus trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Random Stimulus needs to inspect.
2. Apply the core move explicitly: introduce an unrelated cue and force useful connections to the target problem.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Random Stimulus rather than from a generic summary.
