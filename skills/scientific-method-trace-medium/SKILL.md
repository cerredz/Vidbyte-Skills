---
name: scientific-method-trace-medium
description: >
  Use this skill when the user invokes /scientific-method-trace-medium or asks for a medium public reasoning trace using Scientific Method.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Scientific Method as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by empirical inquiry instead of a generic response.
---

# Scientific Method Reasoning Trace Medium

## Goal
Use Scientific Method to answer the user's question through empirical inquiry, not through a generic checklist or interchangeable trace.
The trace should form a question, propose hypotheses, test them, and revise based on observations, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make scientific method useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Scientific Method move: form a question, propose hypotheses, test them, and revise based on observations.
...
Scientific Method is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "form a question, propose hypotheses, test them, and revise based on observations" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Scientific Method trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Scientific Method needs to inspect.
2. Apply the core move explicitly: form a question, propose hypotheses, test them, and revise based on observations.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Scientific Method rather than from a generic summary.
