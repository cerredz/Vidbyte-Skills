...
description: "Use this skill when the user invokes /socratic-questioning-trace or asks for a default public reasoning trace using Socratic Questioning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Socratic Questioning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Socratic Questioning strategy.
...
Apply Socratic Questioning by using it to turn assertions into precise questions, probe definitions and assumptions, and answer each question in sequence, and make each numbered line advance that method rather than adding generic filler.
...
Socratic Questioning is a question-driven inquiry strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to turn assertions into precise questions, probe definitions and assumptions, and answer each question in sequence, which helps the model expose reasoning checkpoints that a reader can inspect.
