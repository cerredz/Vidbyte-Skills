...
description: "Use this skill when the user invokes /analysis-of-competing-hypotheses-trace or asks for a default public reasoning trace using Analysis Of Competing Hypotheses. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Analysis Of Competing Hypotheses Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Analysis Of Competing Hypotheses strategy.
...
Apply Analysis Of Competing Hypotheses by using it to list competing explanations, test evidence against each, and prefer the hypothesis with least conflict, and make each numbered line advance that method rather than adding generic filler.
...
Analysis Of Competing Hypotheses is a structured analytic technique strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to list competing explanations, test evidence against each, and prefer the hypothesis with least conflict, which helps the model expose reasoning checkpoints that a reader can inspect.
...
Include this exact scale statement near the top of the file: "Scale: default - this scratchpad is required to output 100 lines unless the user explicitly requests a different scale."
...
Write 100 numbered scratchpad lines by default, then add a short final answer after the numbered trace.
...
If the user gives a format, domain, or evidence constraint, preserve it inside this structure while still meeting the default length requirement.
