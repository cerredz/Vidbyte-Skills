...
description: "Use this skill when the user invokes /inductive-trace or asks for a default public reasoning trace using Inductive Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Inductive Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Inductive Reasoning strategy.
...
Apply Inductive Reasoning by using it to move from observations to patterns, estimate strength, and mark where the generalization could fail, and make each numbered line advance that method rather than adding generic filler.
...
Inductive Reasoning is a evidence generalization strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to move from observations to patterns, estimate strength, and mark where the generalization could fail, which helps the model expose reasoning checkpoints that a reader can inspect.
