---
name: gemba-walk-trace-medium
description: >
  Use this skill when the user invokes /gemba-walk-trace-medium or asks for a medium public reasoning trace using Gemba Walk.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Gemba Walk as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by going to the actual place where work happens to observe and understand instead of a generic response.
---

# Gemba Walk Reasoning Trace Medium

## Goal
Use Gemba Walk to answer the user's question through going to the actual place where work happens to observe and understand, not through a generic checklist or interchangeable trace.
The trace should go to the actual place where work happens, observe the process directly, ask why, and show respect to the people doing the work, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make gemba walk useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Gemba Walk move: go to the actual place where work happens, observe the process directly, ask why, and show respect to the people doing the work.
...
Gemba Walk is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "go to the actual place where work happens, observe the process directly, ask why, and show respect to the people doing the work" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Gemba Walk trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Gemba Walk needs to inspect.
2. Apply the core move explicitly: go to the actual place where work happens, observe the process directly, ask why, and show respect to the people doing the work.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Gemba Walk rather than from a generic summary.
