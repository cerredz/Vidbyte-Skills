...
description: "Use this skill when the user invokes /assumption-ladder-trace or asks for a default public reasoning trace using Assumption Ladder. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Assumption Ladder Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Assumption Ladder strategy.
...
Apply Assumption Ladder by using it to climb from observations to interpretations, beliefs, and actions while checking each rung, and make each numbered line advance that method rather than adding generic filler.
...
Assumption Ladder is a metacognitive analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to climb from observations to interpretations, beliefs, and actions while checking each rung, which helps the model expose reasoning checkpoints that a reader can inspect.
