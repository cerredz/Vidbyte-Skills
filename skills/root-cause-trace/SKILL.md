...
description: "Use this skill when the user invokes /root-cause-trace or asks for a default public reasoning trace using Root Cause Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Root Cause Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Root Cause Reasoning strategy.
...
Apply Root Cause Reasoning by using it to distinguish proximate symptoms from durable causes that can be corrected, and make each numbered line advance that method rather than adding generic filler.
...
Root Cause Reasoning is a cause analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to distinguish proximate symptoms from durable causes that can be corrected, which helps the model expose reasoning checkpoints that a reader can inspect.
