...
Morphological Analysis is a creative decomposition strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to list dimensions of the problem and recombine alternatives across dimensions, which forces the model to make the important reasoning moves visible.
This strategy is strongest when the question benefits from escaping stale frames and generating alternatives that can be evaluated and when the reader needs to see why the answer follows.
Compared with a generic scratchpad, morphological analysis changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Morphological Analysis, not like a separate response pasted after the trace.
