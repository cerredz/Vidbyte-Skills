---
name: morphological-analysis-trace-medium
description: >
  Use this skill when the user invokes /morphological-analysis-trace-medium or asks for a medium public reasoning trace using Morphological Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Morphological Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by creative decomposition instead of a generic response.
---

# Morphological Analysis Reasoning Trace Medium

## Goal
Use Morphological Analysis to answer the user's question through creative decomposition, not through a generic checklist or interchangeable trace.
The trace should list dimensions of the problem and recombine alternatives across dimensions, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on frames, constraints, provocations, alternatives, combinations, and prototypeable ideas, because those artifacts make morphological analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Morphological Analysis move: list dimensions of the problem and recombine alternatives across dimensions.
...
Morphological Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "list dimensions of the problem and recombine alternatives across dimensions" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Morphological Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Morphological Analysis needs to inspect.
2. Apply the core move explicitly: list dimensions of the problem and recombine alternatives across dimensions.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Morphological Analysis rather than from a generic summary.
