...
description: "Use this skill when the user invokes /first-principles-trace or asks for a default public reasoning trace using First Principles. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# First Principles Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the First Principles strategy.
...
Apply First Principles by using it to break claims into primitive facts, ask what must be true at each smaller level, and rebuild the answer upward, and make each numbered line advance that method rather than adding generic filler.
...
First Principles is a foundational decomposition strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to break claims into primitive facts, ask what must be true at each smaller level, and rebuild the answer upward, which helps the model expose reasoning checkpoints that a reader can inspect.
