...
description: "Use this skill when the user invokes /sensitivity-analysis-trace or asks for a default public reasoning trace using Sensitivity Analysis. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Sensitivity Analysis Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Sensitivity Analysis strategy.
...
Apply Sensitivity Analysis by using it to vary key assumptions to see which inputs most change the conclusion, and make each numbered line advance that method rather than adding generic filler.
...
Sensitivity Analysis is a robustness testing strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to vary key assumptions to see which inputs most change the conclusion, which helps the model expose reasoning checkpoints that a reader can inspect.
