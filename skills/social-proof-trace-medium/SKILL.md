---
name: social-proof-trace-medium
description: >
  Use this skill when the user invokes /social-proof-trace-medium or asks for a medium public reasoning trace using Social Proof.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Social Proof as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by determining correct behavior by observing what others do instead of a generic response.
---

# Social Proof Reasoning Trace Medium

## Goal
Use Social Proof to answer the user's question through determining correct behavior by observing what others do, not through a generic checklist or interchangeable trace.
The trace should determine what is correct by observing what other people do, especially in ambiguous situations, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make social proof useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Social Proof move: determine what is correct by observing what other people do, especially in ambiguous situations.
...
Social Proof is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "determine what is correct by observing what other people do, especially in ambiguous situations" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Social Proof trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Social Proof needs to inspect.
2. Apply the core move explicitly: determine what is correct by observing what other people do, especially in ambiguous situations.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Social Proof rather than from a generic summary.
