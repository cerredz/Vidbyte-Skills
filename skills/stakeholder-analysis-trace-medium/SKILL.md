---
name: stakeholder-analysis-trace-medium
description: >
  Use this skill when the user invokes /stakeholder-analysis-trace-medium or asks for a medium public reasoning trace using Stakeholder Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Stakeholder Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by social systems analysis instead of a generic response.
---

# Stakeholder Analysis Reasoning Trace Medium

## Goal
Use Stakeholder Analysis to answer the user's question through social systems analysis, not through a generic checklist or interchangeable trace.
The trace should map affected parties, incentives, power, needs, and likely reactions, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on boundaries, components, feedback loops, constraints, delays, and leverage points, because those artifacts make stakeholder analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Stakeholder Analysis move: map affected parties, incentives, power, needs, and likely reactions.
...
Stakeholder Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "map affected parties, incentives, power, needs, and likely reactions" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Stakeholder Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Stakeholder Analysis needs to inspect.
2. Apply the core move explicitly: map affected parties, incentives, power, needs, and likely reactions.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Stakeholder Analysis rather than from a generic summary.
