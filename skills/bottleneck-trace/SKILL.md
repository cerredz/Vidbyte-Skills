...
description: "Use this skill when the user invokes /bottleneck-trace or asks for a default public reasoning trace using Bottleneck Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Bottleneck Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Bottleneck Reasoning strategy.
...
Apply Bottleneck Reasoning by using it to locate the limiting factor that governs overall throughput or progress, and make each numbered line advance that method rather than adding generic filler.
...
Bottleneck Reasoning is a constraint analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to locate the limiting factor that governs overall throughput or progress, which helps the model expose reasoning checkpoints that a reader can inspect.
