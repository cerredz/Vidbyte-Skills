---
name: argument-map-trace-medium
description: >
  Use this skill when the user invokes /argument-map-trace-medium or asks for a medium public reasoning trace using Argument Mapping.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Argument Mapping as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by argument analysis instead of a generic response.
---

# Argument Mapping Reasoning Trace Medium

## Goal
Use Argument Mapping to answer the user's question through argument analysis, not through a generic checklist or interchangeable trace.
The trace should turn claims, supports, objections, and rebuttals into a visible logical structure, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make argument mapping useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Argument Mapping move: turn claims, supports, objections, and rebuttals into a visible logical structure.
...
Argument Mapping is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "turn claims, supports, objections, and rebuttals into a visible logical structure" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Argument Mapping trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Argument Mapping needs to inspect.
2. Apply the core move explicitly: turn claims, supports, objections, and rebuttals into a visible logical structure.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Argument Mapping rather than from a generic summary.
