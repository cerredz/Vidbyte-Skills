...
description: "Use this skill when the user invokes /legal-reasoning-trace or asks for a default public reasoning trace using Legal Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Legal Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Legal Reasoning strategy.
...
Apply Legal Reasoning by using it to identify governing rules, apply them to facts, and distinguish contrary authorities, and make each numbered line advance that method rather than adding generic filler.
...
Legal Reasoning is a rule and precedent analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to identify governing rules, apply them to facts, and distinguish contrary authorities, which helps the model expose reasoning checkpoints that a reader can inspect.
