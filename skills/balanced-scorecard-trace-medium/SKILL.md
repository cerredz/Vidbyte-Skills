---
name: balanced-scorecard-trace-medium
description: >
  Use this skill when the user invokes /balanced-scorecard-trace-medium or asks for a medium public reasoning trace using Balanced Scorecard.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Balanced Scorecard as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by translating strategy into financial, customer, process, and learning perspectives instead of a generic response.
---

# Balanced Scorecard Reasoning Trace Medium

## Goal
Use Balanced Scorecard to answer the user's question through translating strategy into financial, customer, process, and learning perspectives, not through a generic checklist or interchangeable trace.
The trace should translate strategy into four balanced perspectives: financial, customer, internal process, and learning and growth, with linked objectives and measures, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make balanced scorecard useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Balanced Scorecard move: translate strategy into four balanced perspectives: financial, customer, internal process, and learning and growth, with linked objectives and measures.
...
Balanced Scorecard is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "translate strategy into four balanced perspectives: financial, customer, internal process, and learning and growth, with linked objectives and measures" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Balanced Scorecard trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Balanced Scorecard needs to inspect.
2. Apply the core move explicitly: translate strategy into four balanced perspectives: financial, customer, internal process, and learning and growth, with linked objectives and measures.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Balanced Scorecard rather than from a generic summary.
