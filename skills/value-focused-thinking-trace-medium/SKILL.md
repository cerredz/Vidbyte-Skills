---
name: value-focused-thinking-trace-medium
description: >
  Use this skill when the user invokes /value-focused-thinking-trace-medium or asks for a medium public reasoning trace using Value-Focused Thinking.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Value-Focused Thinking as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by letting fundamental values drive the generation of decision alternatives instead of a generic response.
---

# Value-Focused Thinking Reasoning Trace Medium

## Goal
Use Value-Focused Thinking to answer the user's question through letting fundamental values drive the generation of decision alternatives, not through a generic checklist or interchangeable trace.
The trace should start with fundamental values and use them to generate decision alternatives, rather than starting with available alternatives and evaluating them, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make value-focused thinking useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Value-Focused Thinking move: start with fundamental values and use them to generate decision alternatives, rather than starting with available alternatives and evaluating them.
...
Value-Focused Thinking is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "start with fundamental values and use them to generate decision alternatives, rather than starting with available alternatives and evaluating them" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Value-Focused Thinking trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Value-Focused Thinking needs to inspect.
2. Apply the core move explicitly: start with fundamental values and use them to generate decision alternatives, rather than starting with available alternatives and evaluating them.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Value-Focused Thinking rather than from a generic summary.
