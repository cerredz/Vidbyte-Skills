---
name: familiarity-heuristic-trace-medium
description: >
  Use this skill when the user invokes /familiarity-heuristic-trace-medium or asks for a medium public reasoning trace using Familiarity Heuristic.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Familiarity Heuristic as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by preferring familiar options over unfamiliar ones when judging value or risk instead of a generic response.
---

# Familiarity Heuristic Reasoning Trace Medium

## Goal
Use Familiarity Heuristic to answer the user's question through preferring familiar options over unfamiliar ones when judging value or risk, not through a generic checklist or interchangeable trace.
The trace should prefer familiar options over unfamiliar ones when judging value, risk, or truth, because familiarity signals safety and reliability, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make familiarity heuristic useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Familiarity Heuristic move: prefer familiar options over unfamiliar ones when judging value, risk, or truth, because familiarity signals safety and reliability.
...
Familiarity Heuristic is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "prefer familiar options over unfamiliar ones when judging value, risk, or truth, because familiarity signals safety and reliability" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Familiarity Heuristic trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Familiarity Heuristic needs to inspect.
2. Apply the core move explicitly: prefer familiar options over unfamiliar ones when judging value, risk, or truth, because familiarity signals safety and reliability.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Familiarity Heuristic rather than from a generic summary.
