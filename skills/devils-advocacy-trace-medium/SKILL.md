---
name: devils-advocacy-trace-medium
description: >
  Use this skill when the user invokes /devils-advocacy-trace-medium or asks for a medium public reasoning trace using Devils Advocacy.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Devils Advocacy as the actual structure of the analysis.
...
# Devils Advocacy Reasoning Trace Medium

## Goal
Use Devils Advocacy to answer the user's question through adversarial critique, not through a generic checklist or interchangeable trace.
The trace should argue against the favored answer to expose unsupported claims and hidden vulnerabilities, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make devils advocacy useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Devils Advocacy move: argue against the favored answer to expose unsupported claims and hidden vulnerabilities.
...
Devils Advocacy is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "argue against the favored answer to expose unsupported claims and hidden vulnerabilities" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Devils Advocacy trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Devils Advocacy needs to inspect.
2. Apply the core move explicitly: argue against the favored answer to expose unsupported claims and hidden vulnerabilities.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Devils Advocacy rather than from a generic summary.
