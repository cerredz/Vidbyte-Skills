---
name: double-diamond-trace-medium
description: >
  Use this skill when the user invokes /double-diamond-trace-medium or asks for a medium public reasoning trace using Double Diamond.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Double Diamond as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by design process instead of a generic response.
---

# Double Diamond Reasoning Trace Medium

## Goal
Use Double Diamond to answer the user's question through design process, not through a generic checklist or interchangeable trace.
The trace should diverge and converge first on the problem, then diverge and converge on the solution, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on frames, constraints, provocations, alternatives, combinations, and prototypeable ideas, because those artifacts make double diamond useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Double Diamond move: diverge and converge first on the problem, then diverge and converge on the solution.
...
Double Diamond is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "diverge and converge first on the problem, then diverge and converge on the solution" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Double Diamond trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Double Diamond needs to inspect.
2. Apply the core move explicitly: diverge and converge first on the problem, then diverge and converge on the solution.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Double Diamond rather than from a generic summary.
