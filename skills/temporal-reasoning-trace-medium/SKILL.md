---
name: temporal-reasoning-trace-medium
description: >
  Use this skill when the user invokes /temporal-reasoning-trace-medium or asks for a medium public reasoning trace using Temporal Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Temporal Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by time-based logic instead of a generic response.
---

# Temporal Reasoning Reasoning Trace Medium

## Goal
Use Temporal Reasoning to answer the user's question through time-based logic, not through a generic checklist or interchangeable trace.
The trace should order events, durations, deadlines, lags, and time-dependent constraints, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on premises, rules, cases, implications, contradictions, and validity checks, because those artifacts make temporal reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Temporal Reasoning move: order events, durations, deadlines, lags, and time-dependent constraints.
...
Temporal Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "order events, durations, deadlines, lags, and time-dependent constraints" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Temporal Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Temporal Reasoning needs to inspect.
2. Apply the core move explicitly: order events, durations, deadlines, lags, and time-dependent constraints.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Temporal Reasoning rather than from a generic summary.
