---
name: constraint-satisfaction-trace-medium
description: >
  Use this skill when the user invokes /constraint-satisfaction-trace-medium or asks for a medium public reasoning trace using Constraint Satisfaction.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Constraint Satisfaction as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by search and optimization instead of a generic response.
---

# Constraint Satisfaction Reasoning Trace Medium

## Goal
Use Constraint Satisfaction to answer the user's question through search and optimization, not through a generic checklist or interchangeable trace.
The trace should list constraints, domains, and conflicts, then narrow toward feasible assignments, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make constraint satisfaction useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Constraint Satisfaction move: list constraints, domains, and conflicts, then narrow toward feasible assignments.
...
Constraint Satisfaction is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "list constraints, domains, and conflicts, then narrow toward feasible assignments" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Constraint Satisfaction trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Constraint Satisfaction needs to inspect.
2. Apply the core move explicitly: list constraints, domains, and conflicts, then narrow toward feasible assignments.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Constraint Satisfaction rather than from a generic summary.
