...
Constraint Satisfaction is a search and optimization strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to list constraints, domains, and conflicts, then narrow toward feasible assignments, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, constraint satisfaction changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Constraint Satisfaction, not like a separate response pasted after the trace.
