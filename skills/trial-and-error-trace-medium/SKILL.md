---
name: trial-and-error-trace-medium
description: >
  Use this skill when the user invokes /trial-and-error-trace-medium or asks for a medium public reasoning trace using Trial and Error.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Trial and Error as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by systematically trying possible solutions and learning from failed attempts instead of a generic response.
---

# Trial and Error Reasoning Trace Medium

## Goal
Use Trial and Error to answer the user's question through systematically trying possible solutions and learning from failed attempts, not through a generic checklist or interchangeable trace.
The trace should generate and test possible solutions, learn from each failed attempt, and use the feedback to converge on a solution, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make trial and error useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Trial and Error move: generate and test possible solutions, learn from each failed attempt, and use the feedback to converge on a solution.
...
Trial and Error is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "generate and test possible solutions, learn from each failed attempt, and use the feedback to converge on a solution" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Trial and Error trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Trial and Error needs to inspect.
2. Apply the core move explicitly: generate and test possible solutions, learn from each failed attempt, and use the feedback to converge on a solution.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Trial and Error rather than from a generic summary.
