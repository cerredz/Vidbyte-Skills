---
name: postmortem-trace
description: "Use this skill when the user invokes /postmortem-trace or asks for a default public reasoning trace using Postmortem Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Postmortem Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Postmortem Reasoning strategy.
...
Apply Postmortem Reasoning by using it to reconstruct what happened, separate symptoms from causes, and extract future safeguards, and make each numbered line advance that method rather than adding generic filler.
...
Postmortem Reasoning is a failure learning strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to reconstruct what happened, separate symptoms from causes, and extract future safeguards, which helps the model expose reasoning checkpoints that a reader can inspect.
