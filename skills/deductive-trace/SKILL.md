...
description: "Use this skill when the user invokes /deductive-trace or asks for a default public reasoning trace using Deductive Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Deductive Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Deductive Reasoning strategy.
...
Apply Deductive Reasoning by using it to start from stated premises and derive conclusions that must follow if the premises are true, and make each numbered line advance that method rather than adding generic filler.
...
Deductive Reasoning is a formal logic strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to start from stated premises and derive conclusions that must follow if the premises are true, which helps the model expose reasoning checkpoints that a reader can inspect.
