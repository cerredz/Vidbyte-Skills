---
name: peak-end-rule-trace-medium
description: >
  Use this skill when the user invokes /peak-end-rule-trace-medium or asks for a medium public reasoning trace using Peak-End Rule.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Peak-End Rule as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by judging experiences primarily by their most intense moment and their ending instead of a generic response.
---

# Peak-End Rule Reasoning Trace Medium

## Goal
Use Peak-End Rule to answer the user's question through judging experiences primarily by their most intense moment and their ending, not through a generic checklist or interchangeable trace.
The trace should judge a past experience primarily by how it felt at its peak and at its end, rather than by the average of every moment, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make peak-end rule useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Peak-End Rule move: judge a past experience primarily by how it felt at its peak and at its end, rather than by the average of every moment.
...
Peak-End Rule is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "judge a past experience primarily by how it felt at its peak and at its end, rather than by the average of every moment" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Peak-End Rule trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Peak-End Rule needs to inspect.
2. Apply the core move explicitly: judge a past experience primarily by how it felt at its peak and at its end, rather than by the average of every moment.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Peak-End Rule rather than from a generic summary.
