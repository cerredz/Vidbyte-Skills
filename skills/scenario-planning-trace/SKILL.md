...
description: "Use this skill when the user invokes /scenario-planning-trace or asks for a default public reasoning trace using Scenario Planning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Scenario Planning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Scenario Planning strategy.
...
Apply Scenario Planning by using it to construct distinct plausible futures and test decisions across them, and make each numbered line advance that method rather than adding generic filler.
...
Scenario Planning is a future uncertainty analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to construct distinct plausible futures and test decisions across them, which helps the model expose reasoning checkpoints that a reader can inspect.
