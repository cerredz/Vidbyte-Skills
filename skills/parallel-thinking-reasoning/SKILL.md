...
description: >
  ...
  into 3G��5 independent sub-problems, running the selected base strategy on each
...
separable aspects. Not ideal for simple single-domain questions G��
...
### Step 3 G�� Decompose the Problem

Analyze the user's problem and identify 3G��5 sub-problems that are as independent as possible. Good decomposition principles:
...
- **Dependency-minimal**: Sub-problems should be answerable independently G�� the answer to sub-problem A should not depend on the answer to sub-problem B. If there is a dependency, sequence the sub-problems (A then B) rather than running them in parallel, and note the dependency in the trace.
- **Difficulty-calibrated**: Assign sub-problems a difficulty rating (simple, moderate, complex) that will determine their reasoning budget. Simple sub-problems (factual, well-defined) get ~500G��1,000 tokens; moderate sub-problems get ~1,000G��2,500 tokens; complex sub-problems (analytical, multi-factor) get ~2,500G��4,000 tokens. This prevents overthinking on easy sub-problems and underthinking on hard ones.

Document the decomposition in the trace with a brief rationale for each sub-problem G�� what it covers, why it's independent, and what difficulty level it warrants.

### Step 4 G�� Web Search for Skills if Not Installed
...
### Step 5 G�� Select the Base Strategy for Each Sub-Problem

For each sub-problem, match its domain against the Reasoning Arsenal and select the strategy whose core move most directly addresses the sub-problem's dominant characteristic. A single complex problem may use different strategies for different sub-problems G�� that is the point of parallel decomposition. Document each selection with a brief justification in the trace.

If the user specified strategies via `--sub` flags, use those directly.

### Step 6 G�� Run Sub-Problems in Parallel
...
2. Apply the selected strategy's core move explicitly G�� this is the central reasoning act for that sub-problem.
...
Each sub-problem's reasoning should be focused and proportionate to its difficulty G�� do not generate a 100-item trace for a sub-problem that can be answered in 15 items. The goal is efficient depth, not uniform length.
...
- Note any interactions or tensions between sub-conclusions (e.g., sub-problem A says the market is large, but sub-problem B says competitive intensity makes entry unprofitable G�� the synthesis must reconcile these).
...
[Complete reasoning trace G�� strategy applied to this sub-problem]
...
[Cross-cutting observations G�� tensions, reinforcements, dependencies noted]
...
2. The decomposition summary G�� which sub-problems were identified and which strategies were used for each
...
**Root Cause Analysis** maps the full causal tree G�� direct causes, contributing factors, and systemic conditions G�� rather than stopping at one root. Best for complex failures with multiple interacting causes where fixing one factor without addressing systemic conditions would let the failure recur.

**Causal Reasoning** constructs a causal model linking causes to effects with explicit mechanisms and counterfactuals G�� what would have happened if the cause had been absent. Best for understanding causal mechanisms and planning interventions with predictable effects.
...
**Deductive Reasoning** derives conclusions from premises through logically necessary steps G�� if premises are true, the conclusion must be true. Best for problems where rules or axioms fully determine the answer.
...
**Regret Minimization** evaluates options by the maximum regret you'd feel G�� and minimizes that maximum. Best for irreversible, high-stakes decisions.

**Opportunity Cost** evaluates what you give up by choosing each option G�� the value of the foregone alternative. Best for resource allocation tradeoffs.
...
- The problem is decomposed into 3G��5 sub-problems that are as independent as possible, with each sub-problem falling cleanly into one reasoning domain.
...
- The trace demonstrates reasoning efficiency G�� total token consumption is proportionate to problem complexity rather than inflated by overthinking, with each sub-problem receiving a budget calibrated to its difficulty.
...
- No sub-problem's reasoning bleeds into or contaminates another sub-problem's analysis G�� each sub-problem's scratchpad starts fresh.
...
- Do not force all sub-problems into a single strategy. The value of parallel decomposition is that different sub-problems may need different reasoning approaches G�� a market sizing sub-problem might need reference class forecasting while a regulatory sub-problem might need legal reasoning. Applying the same strategy to all sub-problems defeats the purpose of decomposition.
...
- Do not underthink complex sub-problems. Efficiency is not the same as brevity G�� a complex sub-problem that genuinely needs 50 reasoning items should get them. The goal is to eliminate wasted tokens, not to truncate necessary reasoning.
- Do not skip the interactions and dependencies section. Sub-problems that appeared independent during decomposition may reveal interactions during analysis G�� a tension between the financial and operational sub-conclusions, a dependency that wasn't apparent upfront. The synthesis must identify and address these.
...
- Do not decompose problems that are fundamentally monolithic. Some questions cannot be sensibly decomposed G�� they require integrated reasoning where the pieces only make sense in relation to each other. Forcing decomposition on such problems produces fragmented analysis that misses the holistic insight. If the problem resists decomposition after genuine effort, fall back to running the single best-fit base strategy on the whole problem.
...
**Required G�� invocation:** `/parallel-thinking-reasoning <problem description>` G�� Sent by the user. Optionally, `--sub "<description>: <strategy>"` to specify sub-problems and strategies explicitly.

**Required G�� Reasoning Arsenal:** The Reasoning Arsenal section embedded in this SKILL.md. Used to match each sub-problem's domain to the right base strategy.

**Optional G�� sub-problem override:** The user may specify `--sub "<sub-problem description>: /<strategy-slash-command>"` for one or more sub-problems to bypass automatic decomposition and strategy selection for those sub-problems. Usage: `--sub "market sizing: /fermi-estimation-trace" --sub "competition: /porters-five-forces-trace"`.
