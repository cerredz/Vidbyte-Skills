...
description: "Use this skill when the user invokes /randomized-control-trial-trace or asks for a default public reasoning trace using Randomized Control Trial. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Randomized Control Trial Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Randomized Control Trial strategy.
...
Apply Randomized Control Trial by using it to compare treatment and control groups while guarding against confounds, and make each numbered line advance that method rather than adding generic filler.
...
Randomized Control Trial is a causal inference strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to compare treatment and control groups while guarding against confounds, which helps the model expose reasoning checkpoints that a reader can inspect.
