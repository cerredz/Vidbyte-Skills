...
description: "Use this skill when the user invokes /issue-tree-trace or asks for a default public reasoning trace using Issue Tree. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Issue Tree Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Issue Tree strategy.
...
Apply Issue Tree by using it to branch the central question into subissues, resolve leaf nodes, and roll findings back to the trunk, and make each numbered line advance that method rather than adding generic filler.
...
Issue Tree is a structured decomposition strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to branch the central question into subissues, resolve leaf nodes, and roll findings back to the trunk, which helps the model expose reasoning checkpoints that a reader can inspect.
