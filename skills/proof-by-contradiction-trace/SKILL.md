...
description: "Use this skill when the user invokes /proof-by-contradiction-trace or asks for a default public reasoning trace using Proof By Contradiction. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Proof By Contradiction Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Proof By Contradiction strategy.
...
Apply Proof By Contradiction by using it to assume the opposite of the target claim and show that it creates an inconsistency, and make each numbered line advance that method rather than adding generic filler.
...
Proof By Contradiction is a formal logic strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to assume the opposite of the target claim and show that it creates an inconsistency, which helps the model expose reasoning checkpoints that a reader can inspect.
