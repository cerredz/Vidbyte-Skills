---
name: analogical-trace-medium
description: >
  Use this skill when the user invokes /analogical-trace-medium or asks for a medium public reasoning trace using Analogical Reasoning.
...
# Analogical Reasoning Reasoning Trace Medium
...
Analogical Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "map a source case to the target case, test relevant similarities, and reject weak transfers" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Analogical Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Analogical Reasoning needs to inspect.
2. Apply the core move explicitly: map a source case to the target case, test relevant similarities, and reject weak transfers.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Analogical Reasoning rather than from a generic summary.
