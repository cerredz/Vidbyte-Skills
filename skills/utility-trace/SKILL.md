---
name: utility-trace
description: "Use this skill when the user invokes /utility-trace or asks for a default public reasoning trace using Utility Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Utility Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Utility Reasoning strategy.
...
Apply Utility Reasoning by using it to clarify objectives, weight preferences, and compare options by expected usefulness, and make each numbered line advance that method rather than adding generic filler.
...
Utility Reasoning is a preference analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to clarify objectives, weight preferences, and compare options by expected usefulness, which helps the model expose reasoning checkpoints that a reader can inspect.
