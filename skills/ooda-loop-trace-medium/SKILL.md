---
name: ooda-loop-trace-medium
description: >
  Use this skill when the user invokes /ooda-loop-trace-medium or asks for a medium public reasoning trace using OODA Loop.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses OODA Loop as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by adaptive decision cycle instead of a generic response.
---

# OODA Loop Reasoning Trace Medium

## Goal
Use OODA Loop to answer the user's question through adaptive decision cycle, not through a generic checklist or interchangeable trace.
The trace should observe, orient, decide, and act while updating the trace as the situation changes, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make ooda loop useful rather than decorative.
...
Build the scratchpad by repeatedly applying the OODA Loop move: observe, orient, decide, and act while updating the trace as the situation changes.
...
OODA Loop is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "observe, orient, decide, and act while updating the trace as the situation changes" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong OODA Loop trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that OODA Loop needs to inspect.
2. Apply the core move explicitly: observe, orient, decide, and act while updating the trace as the situation changes.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from OODA Loop rather than from a generic summary.
