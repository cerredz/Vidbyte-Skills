...
description: "Use this skill when the user invokes /argument-map-trace or asks for a default public reasoning trace using Argument Mapping. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Argument Mapping Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Argument Mapping strategy.
...
Apply Argument Mapping by using it to turn claims, supports, objections, and rebuttals into a visible logical structure, and make each numbered line advance that method rather than adding generic filler.
...
Argument Mapping is a argument analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to turn claims, supports, objections, and rebuttals into a visible logical structure, which helps the model expose reasoning checkpoints that a reader can inspect.
