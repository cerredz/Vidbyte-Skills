---
name: analysis-of-competing-hypotheses-trace-medium
description: >
  Use this skill when the user invokes /analysis-of-competing-hypotheses-trace-medium or asks for a medium public reasoning trace using Analysis Of Competing Hypotheses.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Analysis Of Competing Hypotheses as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by structured analytic technique instead of a generic response.
---

# Analysis Of Competing Hypotheses Reasoning Trace Medium

## Goal
Use Analysis Of Competing Hypotheses to answer the user's question through structured analytic technique, not through a generic checklist or interchangeable trace.
The trace should list competing explanations, test evidence against each, and prefer the hypothesis with least conflict, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on hypotheses, assumptions, diagnostic evidence, indicators, alternatives, and confidence levels, because those artifacts make analysis of competing hypotheses useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Analysis Of Competing Hypotheses move: list competing explanations, test evidence against each, and prefer the hypothesis with least conflict.
...
Analysis Of Competing Hypotheses is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "list competing explanations, test evidence against each, and prefer the hypothesis with least conflict" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Analysis Of Competing Hypotheses trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Analysis Of Competing Hypotheses needs to inspect.
2. Apply the core move explicitly: list competing explanations, test evidence against each, and prefer the hypothesis with least conflict.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Analysis Of Competing Hypotheses rather than from a generic summary.
