...
Analysis Of Competing Hypotheses is a structured analytic technique strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to list competing explanations, test evidence against each, and prefer the hypothesis with least conflict, which forces the model to make the important reasoning moves visible.
This strategy is strongest when the question benefits from reducing bias when evidence is incomplete, ambiguous, or contested and when the reader needs to see why the answer follows.
Compared with a generic scratchpad, analysis of competing hypotheses changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Analysis Of Competing Hypotheses, not like a separate response pasted after the trace.
