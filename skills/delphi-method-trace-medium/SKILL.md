---
name: delphi-method-trace-medium
description: >
  Use this skill when the user invokes /delphi-method-trace-medium or asks for a medium public reasoning trace using Delphi Method.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Delphi Method as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by iteratively polling experts anonymously and feeding back aggregated results to converge on a forecast instead of a generic response.
---

# Delphi Method Reasoning Trace Medium

## Goal
Use Delphi Method to answer the user's question through iteratively polling experts anonymously and feeding back aggregated results to converge on a forecast, not through a generic checklist or interchangeable trace.
The trace should iteratively poll a panel of experts anonymously, feed back aggregated results, and repeat until forecasts converge, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make delphi method useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Delphi Method move: iteratively poll a panel of experts anonymously, feed back aggregated results, and repeat until forecasts converge.
...
Delphi Method is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "iteratively poll a panel of experts anonymously, feed back aggregated results, and repeat until forecasts converge" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Delphi Method trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Delphi Method needs to inspect.
2. Apply the core move explicitly: iteratively poll a panel of experts anonymously, feed back aggregated results, and repeat until forecasts converge.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Delphi Method rather than from a generic summary.
