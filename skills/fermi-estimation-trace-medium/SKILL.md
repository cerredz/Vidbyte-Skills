---
name: fermi-estimation-trace-medium
description: >
  Use this skill when the user invokes /fermi-estimation-trace-medium or asks for a medium public reasoning trace using Fermi Estimation.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Fermi Estimation as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by decomposing quantitative questions into tractable sub-estimates instead of a generic response.
---

# Fermi Estimation Reasoning Trace Medium

## Goal
Use Fermi Estimation to answer the user's question through decomposing quantitative questions into tractable sub-estimates, not through a generic checklist or interchangeable trace.
The trace should break a seemingly impossible quantitative question into tractable sub-estimates, then multiply to reach a ballpark answer, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make fermi estimation useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Fermi Estimation move: break a seemingly impossible quantitative question into tractable sub-estimates, then multiply to reach a ballpark answer.
...
Fermi Estimation is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "break a seemingly impossible quantitative question into tractable sub-estimates, then multiply to reach a ballpark answer" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Fermi Estimation trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Fermi Estimation needs to inspect.
2. Apply the core move explicitly: break a seemingly impossible quantitative question into tractable sub-estimates, then multiply to reach a ballpark answer.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Fermi Estimation rather than from a generic summary.
