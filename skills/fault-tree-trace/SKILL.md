...
description: "Use this skill when the user invokes /fault-tree-trace or asks for a default public reasoning trace using Fault Tree Analysis. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Fault Tree Analysis Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Fault Tree Analysis strategy.
...
Apply Fault Tree Analysis by using it to work top-down from an undesirable event to combinations of contributing failures, and make each numbered line advance that method rather than adding generic filler.
...
Fault Tree Analysis is a failure analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to work top-down from an undesirable event to combinations of contributing failures, which helps the model expose reasoning checkpoints that a reader can inspect.
