---
name: five-whys-trace-medium
description: >
  Use this skill when the user invokes /five-whys-trace-medium or asks for a medium public reasoning trace using Five Whys.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Five Whys as the actual structure of the analysis.
...
# Five Whys Reasoning Trace Medium

## Goal
Use Five Whys to answer the user's question through root cause analysis, not through a generic checklist or interchangeable trace.
The trace should ask why repeatedly until the trace reaches a cause that can be acted on or tested, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make five whys useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Five Whys move: ask why repeatedly until the trace reaches a cause that can be acted on or tested.
...
Five Whys is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "ask why repeatedly until the trace reaches a cause that can be acted on or tested" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Five Whys trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Five Whys needs to inspect.
2. Apply the core move explicitly: ask why repeatedly until the trace reaches a cause that can be acted on or tested.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Five Whys rather than from a generic summary.
