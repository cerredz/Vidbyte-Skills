---
name: historical-reasoning-trace-medium
description: >
  Use this skill when the user invokes /historical-reasoning-trace-medium or asks for a medium public reasoning trace using Historical Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Historical Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by contextual analysis instead of a generic response.
---

# Historical Reasoning Reasoning Trace Medium

## Goal
Use Historical Reasoning to answer the user's question through contextual analysis, not through a generic checklist or interchangeable trace.
The trace should interpret events through chronology, context, sources, causation, and contingency, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make historical reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Historical Reasoning move: interpret events through chronology, context, sources, causation, and contingency.
