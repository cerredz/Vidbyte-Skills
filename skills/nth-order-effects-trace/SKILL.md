...
description: "Use this skill when the user invokes /nth-order-effects-trace or asks for a default public reasoning trace using Nth Order Effects. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Nth Order Effects Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Nth Order Effects strategy.
...
Apply Nth Order Effects by using it to extend consequence chains across multiple steps while pruning speculative branches, and make each numbered line advance that method rather than adding generic filler.
...
Nth Order Effects is a consequence analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to extend consequence chains across multiple steps while pruning speculative branches, which helps the model expose reasoning checkpoints that a reader can inspect.
