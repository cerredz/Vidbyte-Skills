---
name: socratic-questioning-trace-medium
description: >
  Use this skill when the user invokes /socratic-questioning-trace-medium or asks for a medium public reasoning trace using Socratic Questioning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Socratic Questioning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by question-driven inquiry instead of a generic response.
---

# Socratic Questioning Reasoning Trace Medium

## Goal
Use Socratic Questioning to answer the user's question through question-driven inquiry, not through a generic checklist or interchangeable trace.
The trace should turn assertions into precise questions, probe definitions and assumptions, and answer each question in sequence, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make socratic questioning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Socratic Questioning move: turn assertions into precise questions, probe definitions and assumptions, and answer each question in sequence.
...
Socratic Questioning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "turn assertions into precise questions, probe definitions and assumptions, and answer each question in sequence" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Socratic Questioning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Socratic Questioning needs to inspect.
2. Apply the core move explicitly: turn assertions into precise questions, probe definitions and assumptions, and answer each question in sequence.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Socratic Questioning rather than from a generic summary.
