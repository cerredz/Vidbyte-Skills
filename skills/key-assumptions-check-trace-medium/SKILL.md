---
name: key-assumptions-check-trace-medium
description: >
  Use this skill when the user invokes /key-assumptions-check-trace-medium or asks for a medium public reasoning trace using Key Assumptions Check.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Key Assumptions Check as the actual structure of the analysis.
...
# Key Assumptions Check Reasoning Trace Medium

## Goal
Use Key Assumptions Check to answer the user's question through structured analytic technique, not through a generic checklist or interchangeable trace.
The trace should surface critical assumptions, test fragility, and revise conclusions where assumptions are weak, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on hypotheses, assumptions, diagnostic evidence, indicators, alternatives, and confidence levels, because those artifacts make key assumptions check useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Key Assumptions Check move: surface critical assumptions, test fragility, and revise conclusions where assumptions are weak.
...
Key Assumptions Check is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "surface critical assumptions, test fragility, and revise conclusions where assumptions are weak" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Key Assumptions Check trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Key Assumptions Check needs to inspect.
2. Apply the core move explicitly: surface critical assumptions, test fragility, and revise conclusions where assumptions are weak.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Key Assumptions Check rather than from a generic summary.
