---
name: nonmonotonic-reasoning-trace-medium
description: >
  Use this skill when the user invokes /nonmonotonic-reasoning-trace-medium or asks for a medium public reasoning trace using Nonmonotonic Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Nonmonotonic Reasoning as the actual structure of the analysis.
...
# Nonmonotonic Reasoning Reasoning Trace Medium

## Goal
Use Nonmonotonic Reasoning to answer the user's question through revisable logic, not through a generic checklist or interchangeable trace.
The trace should allow new information to reduce or invalidate earlier conclusions, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on premises, rules, cases, implications, contradictions, and validity checks, because those artifacts make nonmonotonic reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Nonmonotonic Reasoning move: allow new information to reduce or invalidate earlier conclusions.
...
Nonmonotonic Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "allow new information to reduce or invalidate earlier conclusions" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Nonmonotonic Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Nonmonotonic Reasoning needs to inspect.
2. Apply the core move explicitly: allow new information to reduce or invalidate earlier conclusions.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Nonmonotonic Reasoning rather than from a generic summary.
