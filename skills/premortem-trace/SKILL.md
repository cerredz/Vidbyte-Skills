...
description: "Use this skill when the user invokes /premortem-trace or asks for a default public reasoning trace using Premortem Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Premortem Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Premortem Reasoning strategy.
...
Apply Premortem Reasoning by using it to assume the plan failed, identify causes of failure, and convert them into mitigations, and make each numbered line advance that method rather than adding generic filler.
...
Premortem Reasoning is a failure anticipation strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to assume the plan failed, identify causes of failure, and convert them into mitigations, which helps the model expose reasoning checkpoints that a reader can inspect.
