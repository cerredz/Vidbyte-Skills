...
description: "Use this skill when the user invokes /dependency-mapping-trace or asks for a default public reasoning trace using Dependency Mapping. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Dependency Mapping Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Dependency Mapping strategy.
...
Apply Dependency Mapping by using it to map prerequisites, blockers, interfaces, and sequencing before recommending work, and make each numbered line advance that method rather than adding generic filler.
...
Dependency Mapping is a systems decomposition strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to map prerequisites, blockers, interfaces, and sequencing before recommending work, which helps the model expose reasoning checkpoints that a reader can inspect.
