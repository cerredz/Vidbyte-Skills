...
description: >
  Use this skill when the user invokes /horizon-scanning-trace-medium or asks for a medium public reasoning trace using Horizon Scanning.
...
Horizon Scanning is a weak signal analysis strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to search for early signals, emerging patterns, and discontinuities that may matter later, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, horizon scanning changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Horizon Scanning, not like a separate response pasted after the trace.
