---
name: blue-ocean-strategy-trace-medium
description: >
  Use this skill when the user invokes /blue-ocean-strategy-trace-medium or asks for a medium public reasoning trace using Blue Ocean Strategy.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Blue Ocean Strategy as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by creating uncontested market space through value innovation across the strategy canvas instead of a generic response.
---

# Blue Ocean Strategy Reasoning Trace Medium

## Goal
Use Blue Ocean Strategy to answer the user's question through creating uncontested market space through value innovation across the strategy canvas, not through a generic checklist or interchangeable trace.
The trace should create uncontested market space by simultaneously pursuing differentiation and low cost, reconstructing market boundaries across the strategy canvas, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make blue ocean strategy useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Blue Ocean Strategy move: create uncontested market space by simultaneously pursuing differentiation and low cost, reconstructing market boundaries across the strategy canvas.
...
Blue Ocean Strategy is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "create uncontested market space by simultaneously pursuing differentiation and low cost, reconstructing market boundaries across the strategy canvas" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Blue Ocean Strategy trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Blue Ocean Strategy needs to inspect.
2. Apply the core move explicitly: create uncontested market space by simultaneously pursuing differentiation and low cost, reconstructing market boundaries across the strategy canvas.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Blue Ocean Strategy rather than from a generic summary.
