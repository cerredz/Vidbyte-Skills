...
description: "Use this skill when the user invokes /theory-of-constraints-trace or asks for a default public reasoning trace using Theory Of Constraints. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Theory Of Constraints Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Theory Of Constraints strategy.
...
Apply Theory Of Constraints by using it to identify the system constraint, exploit it, subordinate other work, and reassess, and make each numbered line advance that method rather than adding generic filler.
...
Theory Of Constraints is a constraint analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to identify the system constraint, exploit it, subordinate other work, and reassess, which helps the model expose reasoning checkpoints that a reader can inspect.
