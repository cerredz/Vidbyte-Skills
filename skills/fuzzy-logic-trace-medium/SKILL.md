---
name: fuzzy-logic-trace-medium
description: >
  Use this skill when the user invokes /fuzzy-logic-trace-medium or asks for a medium public reasoning trace using Fuzzy Logic Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Fuzzy Logic Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by graded truth analysis instead of a generic response.
---

# Fuzzy Logic Reasoning Reasoning Trace Medium

## Goal
Use Fuzzy Logic Reasoning to answer the user's question through graded truth analysis, not through a generic checklist or interchangeable trace.
The trace should use degrees of membership when categories are not cleanly true or false, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make fuzzy logic reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Fuzzy Logic Reasoning move: use degrees of membership when categories are not cleanly true or false.
...
Fuzzy Logic Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "use degrees of membership when categories are not cleanly true or false" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Fuzzy Logic Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Fuzzy Logic Reasoning needs to inspect.
2. Apply the core move explicitly: use degrees of membership when categories are not cleanly true or false.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Fuzzy Logic Reasoning rather than from a generic summary.
