---
name: ethnographic-reasoning-trace-medium
description: >
  Use this skill when the user invokes /ethnographic-reasoning-trace-medium or asks for a medium public reasoning trace using Ethnographic Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Ethnographic Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by contextual social analysis instead of a generic response.
---

# Ethnographic Reasoning Reasoning Trace Medium

## Goal
Use Ethnographic Reasoning to answer the user's question through contextual social analysis, not through a generic checklist or interchangeable trace.
The trace should infer meaning from practices, language, setting, and participant perspective, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make ethnographic reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Ethnographic Reasoning move: infer meaning from practices, language, setting, and participant perspective.
...
Ethnographic Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "infer meaning from practices, language, setting, and participant perspective" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Ethnographic Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Ethnographic Reasoning needs to inspect.
2. Apply the core move explicitly: infer meaning from practices, language, setting, and participant perspective.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Ethnographic Reasoning rather than from a generic summary.
