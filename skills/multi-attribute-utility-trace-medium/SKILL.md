---
name: multi-attribute-utility-trace-medium
description: >
  Use this skill when the user invokes /multi-attribute-utility-trace-medium or asks for a medium public reasoning trace using Multi-Attribute Utility Theory.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Multi-Attribute Utility Theory as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by quantifying preferences across multiple objectives to find the optimal choice instead of a generic response.
---

# Multi-Attribute Utility Theory Reasoning Trace Medium

## Goal
Use Multi-Attribute Utility Theory to answer the user's question through quantifying preferences across multiple objectives to find the optimal choice, not through a generic checklist or interchangeable trace.
The trace should quantify preferences across multiple conflicting objectives using utility functions to find the alternative that maximizes overall utility, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make multi-attribute utility theory useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Multi-Attribute Utility Theory move: quantify preferences across multiple conflicting objectives using utility functions to find the alternative that maximizes overall utility.
...
Multi-Attribute Utility Theory is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "quantify preferences across multiple conflicting objectives using utility functions to find the alternative that maximizes overall utility" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Multi-Attribute Utility Theory trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Multi-Attribute Utility Theory needs to inspect.
2. Apply the core move explicitly: quantify preferences across multiple conflicting objectives using utility functions to find the alternative that maximizes overall utility.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Multi-Attribute Utility Theory rather than from a generic summary.
