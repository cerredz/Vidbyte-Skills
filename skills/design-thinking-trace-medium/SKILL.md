---
name: design-thinking-trace-medium
description: >
  Use this skill when the user invokes /design-thinking-trace-medium or asks for a medium public reasoning trace using Design Thinking.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Design Thinking as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by human-centered design instead of a generic response.
---

# Design Thinking Reasoning Trace Medium

## Goal
Use Design Thinking to answer the user's question through human-centered design, not through a generic checklist or interchangeable trace.
The trace should empathize, define, ideate, prototype, and test around user needs, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on frames, constraints, provocations, alternatives, combinations, and prototypeable ideas, because those artifacts make design thinking useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Design Thinking move: empathize, define, ideate, prototype, and test around user needs.
...
Design Thinking is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "empathize, define, ideate, prototype, and test around user needs" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Design Thinking trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Design Thinking needs to inspect.
2. Apply the core move explicitly: empathize, define, ideate, prototype, and test around user needs.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Design Thinking rather than from a generic summary.
