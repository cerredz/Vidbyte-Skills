---
name: cynefin-trace-medium
description: >
  Use this skill when the user invokes /cynefin-trace-medium or asks for a medium public reasoning trace using Cynefin Framework.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Cynefin Framework as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by categorizing problem domains for situation-appropriate decision making instead of a generic response.
---

# Cynefin Framework Reasoning Trace Medium

## Goal
Use Cynefin Framework to answer the user's question through categorizing problem domains for situation-appropriate decision making, not through a generic checklist or interchangeable trace.
The trace should categorize a problem domain as simple, complicated, complex, chaotic, or disordered, then apply the situation-appropriate decision model, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make cynefin framework useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Cynefin Framework move: categorize a problem domain as simple, complicated, complex, chaotic, or disordered, then apply the situation-appropriate decision model.
...
Cynefin Framework is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "categorize a problem domain as simple, complicated, complex, chaotic, or disordered, then apply the situation-appropriate decision model" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Cynefin Framework trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Cynefin Framework needs to inspect.
2. Apply the core move explicitly: categorize a problem domain as simple, complicated, complex, chaotic, or disordered, then apply the situation-appropriate decision model.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Cynefin Framework rather than from a generic summary.
