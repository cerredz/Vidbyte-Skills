---
name: bowtie-risk-trace-medium
description: >
  Use this skill when the user invokes /bowtie-risk-trace-medium or asks for a medium public reasoning trace using Bowtie Risk Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Bowtie Risk Reasoning as the actual structure of the analysis.
...
# Bowtie Risk Reasoning Reasoning Trace Medium

## Goal
Use Bowtie Risk Reasoning to answer the user's question through risk analysis, not through a generic checklist or interchangeable trace.
The trace should connect threats, preventive controls, the central event, recovery controls, and consequences, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make bowtie risk reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Bowtie Risk Reasoning move: connect threats, preventive controls, the central event, recovery controls, and consequences.
...
Bowtie Risk Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "connect threats, preventive controls, the central event, recovery controls, and consequences" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Bowtie Risk Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Bowtie Risk Reasoning needs to inspect.
2. Apply the core move explicitly: connect threats, preventive controls, the central event, recovery controls, and consequences.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Bowtie Risk Reasoning rather than from a generic summary.
