---
name: values-tradeoff-trace-medium
description: >
  Use this skill when the user invokes /values-tradeoff-trace-medium or asks for a medium public reasoning trace using Values Tradeoff Reasoning.
...
# Values Tradeoff Reasoning Reasoning Trace Medium
...
Values Tradeoff Reasoning is a ethical decision analysis strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to make competing values explicit and explain which value prevails under which condition, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, values tradeoff reasoning changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Values Tradeoff Reasoning, not like a separate response pasted after the trace.
