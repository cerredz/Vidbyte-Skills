...
description: "Use this skill when the user invokes /reverse-brainstorming-trace or asks for a default public reasoning trace using Reverse Brainstorming. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Reverse Brainstorming Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Reverse Brainstorming strategy.
...
Apply Reverse Brainstorming by using it to ask how to worsen the outcome, then reverse those causes into improvements, and make each numbered line advance that method rather than adding generic filler.
...
Reverse Brainstorming is a creative inversion strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to ask how to worsen the outcome, then reverse those causes into improvements, which helps the model expose reasoning checkpoints that a reader can inspect.
