---
name: bayesian-trace
description: "Use this skill when the user invokes /bayesian-trace or asks for a default public reasoning trace using Bayesian Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Bayesian Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Bayesian Reasoning strategy.
...
Apply Bayesian Reasoning by using it to name priors, update with evidence, and explain how each item shifts confidence, and make each numbered line advance that method rather than adding generic filler.
...
Bayesian Reasoning is a probabilistic updating strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to name priors, update with evidence, and explain how each item shifts confidence, which helps the model expose reasoning checkpoints that a reader can inspect.
