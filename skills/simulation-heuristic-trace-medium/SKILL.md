---
name: simulation-heuristic-trace-medium
description: >
  Use this skill when the user invokes /simulation-heuristic-trace-medium or asks for a medium public reasoning trace using Simulation Heuristic.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Simulation Heuristic as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by mentally simulating events to judge their likelihood or emotional impact instead of a generic response.
---

# Simulation Heuristic Reasoning Trace Medium

## Goal
Use Simulation Heuristic to answer the user's question through mentally simulating events to judge their likelihood or emotional impact, not through a generic checklist or interchangeable trace.
The trace should mentally simulate how events could unfold to judge their likelihood, and use ease of simulation to estimate probability, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make simulation heuristic useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Simulation Heuristic move: mentally simulate how events could unfold to judge their likelihood, and use ease of simulation to estimate probability.
...
Simulation Heuristic is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "mentally simulate how events could unfold to judge their likelihood, and use ease of simulation to estimate probability" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Simulation Heuristic trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Simulation Heuristic needs to inspect.
2. Apply the core move explicitly: mentally simulate how events could unfold to judge their likelihood, and use ease of simulation to estimate probability.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Simulation Heuristic rather than from a generic summary.
