---
name: lateral-thinking-trace-medium
description: >
  Use this skill when the user invokes /lateral-thinking-trace-medium or asks for a medium public reasoning trace using Lateral Thinking.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Lateral Thinking as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by creative problem solving instead of a generic response.
---

# Lateral Thinking Reasoning Trace Medium

## Goal
Use Lateral Thinking to answer the user's question through creative problem solving, not through a generic checklist or interchangeable trace.
The trace should approach the problem indirectly, break habitual frames, and generate non-obvious moves, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on frames, constraints, provocations, alternatives, combinations, and prototypeable ideas, because those artifacts make lateral thinking useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Lateral Thinking move: approach the problem indirectly, break habitual frames, and generate non-obvious moves.
...
Lateral Thinking is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "approach the problem indirectly, break habitual frames, and generate non-obvious moves" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Lateral Thinking trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Lateral Thinking needs to inspect.
2. Apply the core move explicitly: approach the problem indirectly, break habitual frames, and generate non-obvious moves.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Lateral Thinking rather than from a generic summary.
