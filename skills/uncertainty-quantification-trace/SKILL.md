...
description: "Use this skill when the user invokes /uncertainty-quantification-trace or asks for a default public reasoning trace using Uncertainty Quantification. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Uncertainty Quantification Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Uncertainty Quantification strategy.
...
Apply Uncertainty Quantification by using it to express confidence ranges, sources of uncertainty, and what would narrow them, and make each numbered line advance that method rather than adding generic filler.
...
Uncertainty Quantification is a risk and probability analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to express confidence ranges, sources of uncertainty, and what would narrow them, which helps the model expose reasoning checkpoints that a reader can inspect.
