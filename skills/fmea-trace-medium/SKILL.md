---
name: fmea-trace-medium
description: >
  Use this skill when the user invokes /fmea-trace-medium or asks for a medium public reasoning trace using Failure Mode and Effects Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Failure Mode and Effects Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by systematically identifying how each component can fail and assessing the impact instead of a generic response.
---

# Failure Mode and Effects Analysis Reasoning Trace Medium

## Goal
Use Failure Mode and Effects Analysis to answer the user's question through systematically identifying how each component can fail and assessing the impact, not through a generic checklist or interchangeable trace.
The trace should systematically identify how each component can fail, assess the severity, occurrence, and detection of each failure, and prioritize corrective actions, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make failure mode and effects analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Failure Mode and Effects Analysis move: systematically identify how each component can fail, assess the severity, occurrence, and detection of each failure, and prioritize corrective actions.
...
Failure Mode and Effects Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "systematically identify how each component can fail, assess the severity, occurrence, and detection of each failure, and prioritize corrective actions" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Failure Mode and Effects Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Failure Mode and Effects Analysis needs to inspect.
2. Apply the core move explicitly: systematically identify how each component can fail, assess the severity, occurrence, and detection of each failure, and prioritize corrective actions.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Failure Mode and Effects Analysis rather than from a generic summary.
