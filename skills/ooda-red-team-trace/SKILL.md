...
description: "Use this skill when the user invokes /ooda-red-team-trace or asks for a default public reasoning trace using OODA Red Team. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# OODA Red Team Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the OODA Red Team strategy.
...
Apply OODA Red Team by using it to run the OODA cycle from an opposing actor's perspective and compare likely moves, and make each numbered line advance that method rather than adding generic filler.
...
OODA Red Team is a adversarial adaptation strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to run the OODA cycle from an opposing actor's perspective and compare likely moves, which helps the model expose reasoning checkpoints that a reader can inspect.
