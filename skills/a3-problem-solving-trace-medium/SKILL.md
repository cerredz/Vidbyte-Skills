---
name: a3-problem-solving-trace-medium
description: >
  Use this skill when the user invokes /a3-problem-solving-trace-medium or asks for a medium public reasoning trace using A3 Problem Solving.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses A3 Problem Solving as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by fitting problem analysis, root cause, corrective action, and follow-up on a single sheet instead of a generic response.
---

# A3 Problem Solving Reasoning Trace Medium

## Goal
Use A3 Problem Solving to answer the user's question through fitting problem analysis, root cause, corrective action, and follow-up on a single sheet, not through a generic checklist or interchangeable trace.
The trace should fit the entire problem-solving cycle on a single A3 sheet: background, current state, goal, root cause analysis, countermeasures, check, and follow-up, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make a3 problem solving useful rather than decorative.
...
Build the scratchpad by repeatedly applying the A3 Problem Solving move: fit the entire problem-solving cycle on a single A3 sheet: background, current state, goal, root cause analysis, countermeasures, check, and follow-up.
...
A3 Problem Solving is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "fit the entire problem-solving cycle on a single A3 sheet: background, current state, goal, root cause analysis, countermeasures, check, and follow-up" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong A3 Problem Solving trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that A3 Problem Solving needs to inspect.
2. Apply the core move explicitly: fit the entire problem-solving cycle on a single A3 sheet: background, current state, goal, root cause analysis, countermeasures, check, and follow-up.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from A3 Problem Solving rather than from a generic summary.
