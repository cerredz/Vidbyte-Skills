---
name: metacognitive-audit-trace-medium
description: >
  Use this skill when the user invokes /metacognitive-audit-trace-medium or asks for a medium public reasoning trace using Metacognitive Audit.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Metacognitive Audit as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by thinking quality review instead of a generic response.
---

# Metacognitive Audit Reasoning Trace Medium

## Goal
Use Metacognitive Audit to answer the user's question through thinking quality review, not through a generic checklist or interchangeable trace.
The trace should inspect the reasoning process itself for bias, gaps, overconfidence, and poor framing, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make metacognitive audit useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Metacognitive Audit move: inspect the reasoning process itself for bias, gaps, overconfidence, and poor framing.
...
Metacognitive Audit is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "inspect the reasoning process itself for bias, gaps, overconfidence, and poor framing" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Metacognitive Audit trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Metacognitive Audit needs to inspect.
2. Apply the core move explicitly: inspect the reasoning process itself for bias, gaps, overconfidence, and poor framing.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Metacognitive Audit rather than from a generic summary.
