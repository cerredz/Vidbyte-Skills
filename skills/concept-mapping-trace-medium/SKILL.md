---
name: concept-mapping-trace-medium
description: >
  Use this skill when the user invokes /concept-mapping-trace-medium or asks for a medium public reasoning trace using Concept Mapping.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Concept Mapping as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by organizing and representing knowledge as a network of concepts linked by relationships instead of a generic response.
---

# Concept Mapping Reasoning Trace Medium

## Goal
Use Concept Mapping to answer the user's question through organizing and representing knowledge as a network of concepts linked by relationships, not through a generic checklist or interchangeable trace.
The trace should organize knowledge as a network of concepts connected by labeled relationships, revealing the structure and gaps in understanding, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make concept mapping useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Concept Mapping move: organize knowledge as a network of concepts connected by labeled relationships, revealing the structure and gaps in understanding.
...
Concept Mapping is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "organize knowledge as a network of concepts connected by labeled relationships, revealing the structure and gaps in understanding" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Concept Mapping trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Concept Mapping needs to inspect.
2. Apply the core move explicitly: organize knowledge as a network of concepts connected by labeled relationships, revealing the structure and gaps in understanding.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Concept Mapping rather than from a generic summary.
