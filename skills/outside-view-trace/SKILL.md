...
description: "Use this skill when the user invokes /outside-view-trace or asks for a default public reasoning trace using Outside View. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Outside View Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Outside View strategy.
...
Apply Outside View by using it to compare the case to a reference class before trusting inside details, and make each numbered line advance that method rather than adding generic filler.
...
Outside View is a forecast calibration strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to compare the case to a reference class before trusting inside details, which helps the model expose reasoning checkpoints that a reader can inspect.
