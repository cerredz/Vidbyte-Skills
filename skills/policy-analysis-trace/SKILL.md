...
description: "Use this skill when the user invokes /policy-analysis-trace or asks for a default public reasoning trace using Policy Analysis. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Policy Analysis Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Policy Analysis strategy.
...
Apply Policy Analysis by using it to compare policy options by goals, feasibility, effects, equity, and implementation risk, and make each numbered line advance that method rather than adding generic filler.
...
Policy Analysis is a public decision analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to compare policy options by goals, feasibility, effects, equity, and implementation risk, which helps the model expose reasoning checkpoints that a reader can inspect.
