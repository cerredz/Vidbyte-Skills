---
name: feedback-loop-trace-medium
description: >
  Use this skill when the user invokes /feedback-loop-trace-medium or asks for a medium public reasoning trace using Feedback Loop Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Feedback Loop Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by systems analysis instead of a generic response.
---

# Feedback Loop Reasoning Reasoning Trace Medium

## Goal
Use Feedback Loop Reasoning to answer the user's question through systems analysis, not through a generic checklist or interchangeable trace.
The trace should identify reinforcing and balancing loops, then trace their effects over time, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on boundaries, components, feedback loops, constraints, delays, and leverage points, because those artifacts make feedback loop reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Feedback Loop Reasoning move: identify reinforcing and balancing loops, then trace their effects over time.
...
Feedback Loop Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "identify reinforcing and balancing loops, then trace their effects over time" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Feedback Loop Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Feedback Loop Reasoning needs to inspect.
2. Apply the core move explicitly: identify reinforcing and balancing loops, then trace their effects over time.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Feedback Loop Reasoning rather than from a generic summary.
