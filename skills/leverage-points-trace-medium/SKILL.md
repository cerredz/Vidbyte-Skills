---
name: leverage-points-trace-medium
description: >
  Use this skill when the user invokes /leverage-points-trace-medium or asks for a medium public reasoning trace using Leverage Points.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Leverage Points as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by systems intervention instead of a generic response.
---

# Leverage Points Reasoning Trace Medium

## Goal
Use Leverage Points to answer the user's question through systems intervention, not through a generic checklist or interchangeable trace.
The trace should find places where small changes can shift system behavior materially, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on boundaries, components, feedback loops, constraints, delays, and leverage points, because those artifacts make leverage points useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Leverage Points move: find places where small changes can shift system behavior materially.
...
Leverage Points is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "find places where small changes can shift system behavior materially" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Leverage Points trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Leverage Points needs to inspect.
2. Apply the core move explicitly: find places where small changes can shift system behavior materially.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Leverage Points rather than from a generic summary.
