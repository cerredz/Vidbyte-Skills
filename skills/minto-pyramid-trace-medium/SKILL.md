---
name: minto-pyramid-trace-medium
description: >
  Use this skill when the user invokes /minto-pyramid-trace-medium or asks for a medium public reasoning trace using Minto Pyramid.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Minto Pyramid as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by communication reasoning instead of a generic response.
---

# Minto Pyramid Reasoning Trace Medium

## Goal
Use Minto Pyramid to answer the user's question through communication reasoning, not through a generic checklist or interchangeable trace.
The trace should state the answer first, group supporting arguments, and order evidence so the logic is easy to inspect, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make minto pyramid useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Minto Pyramid move: state the answer first, group supporting arguments, and order evidence so the logic is easy to inspect.
...
Minto Pyramid is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "state the answer first, group supporting arguments, and order evidence so the logic is easy to inspect" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Minto Pyramid trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Minto Pyramid needs to inspect.
2. Apply the core move explicitly: state the answer first, group supporting arguments, and order evidence so the logic is easy to inspect.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Minto Pyramid rather than from a generic summary.
