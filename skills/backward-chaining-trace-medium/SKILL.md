---
name: backward-chaining-trace-medium
description: >
  Use this skill when the user invokes /backward-chaining-trace-medium or asks for a medium public reasoning trace using Backward Chaining.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Backward Chaining as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by goal-driven inference instead of a generic response.
---

# Backward Chaining Reasoning Trace Medium

## Goal
Use Backward Chaining to answer the user's question through goal-driven inference, not through a generic checklist or interchangeable trace.
The trace should start from the desired conclusion and work backward to required premises or actions, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make backward chaining useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Backward Chaining move: start from the desired conclusion and work backward to required premises or actions.
...
Backward Chaining is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "start from the desired conclusion and work backward to required premises or actions" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Backward Chaining trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Backward Chaining needs to inspect.
2. Apply the core move explicitly: start from the desired conclusion and work backward to required premises or actions.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Backward Chaining rather than from a generic summary.
