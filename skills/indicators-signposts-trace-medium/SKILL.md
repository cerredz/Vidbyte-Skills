...
Indicators And Signposts is a monitoring analysis strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to define observable signals that would confirm, weaken, or redirect the current assessment, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, indicators and signposts changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Indicators And Signposts, not like a separate response pasted after the trace.
