---
name: indicators-signposts-trace-medium
description: >
  Use this skill when the user invokes /indicators-signposts-trace-medium or asks for a medium public reasoning trace using Indicators And Signposts.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Indicators And Signposts as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by monitoring analysis instead of a generic response.
---

# Indicators And Signposts Reasoning Trace Medium

## Goal
Use Indicators And Signposts to answer the user's question through monitoring analysis, not through a generic checklist or interchangeable trace.
The trace should define observable signals that would confirm, weaken, or redirect the current assessment, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make indicators and signposts useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Indicators And Signposts move: define observable signals that would confirm, weaken, or redirect the current assessment.
...
Indicators And Signposts is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "define observable signals that would confirm, weaken, or redirect the current assessment" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Indicators And Signposts trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Indicators And Signposts needs to inspect.
2. Apply the core move explicitly: define observable signals that would confirm, weaken, or redirect the current assessment.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Indicators And Signposts rather than from a generic summary.
