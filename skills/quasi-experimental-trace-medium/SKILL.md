---
name: quasi-experimental-trace-medium
description: >
  Use this skill when the user invokes /quasi-experimental-trace-medium or asks for a medium public reasoning trace using Quasi Experimental Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Quasi Experimental Reasoning as the actual structure of the analysis.
...
# Quasi Experimental Reasoning Reasoning Trace Medium

## Goal
Use Quasi Experimental Reasoning to answer the user's question through causal inference, not through a generic checklist or interchangeable trace.
The trace should use imperfect natural comparisons while naming threats to validity, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on causes, mechanisms, counterfactual tests, evidence quality, and intervention points, because those artifacts make quasi experimental reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Quasi Experimental Reasoning move: use imperfect natural comparisons while naming threats to validity.
...
Quasi Experimental Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "use imperfect natural comparisons while naming threats to validity" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Quasi Experimental Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Quasi Experimental Reasoning needs to inspect.
2. Apply the core move explicitly: use imperfect natural comparisons while naming threats to validity.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Quasi Experimental Reasoning rather than from a generic summary.
