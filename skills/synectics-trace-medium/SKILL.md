---
name: synectics-trace-medium
description: >
  Use this skill when the user invokes /synectics-trace-medium or asks for a medium public reasoning trace using Synectics Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Synectics Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by creative analogy instead of a generic response.
---

# Synectics Reasoning Reasoning Trace Medium

## Goal
Use Synectics Reasoning to answer the user's question through creative analogy, not through a generic checklist or interchangeable trace.
The trace should use direct, personal, symbolic, and fantasy analogies to reframe the problem, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on frames, constraints, provocations, alternatives, combinations, and prototypeable ideas, because those artifacts make synectics reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Synectics Reasoning move: use direct, personal, symbolic, and fantasy analogies to reframe the problem.
...
Synectics Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "use direct, personal, symbolic, and fantasy analogies to reframe the problem" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Synectics Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Synectics Reasoning needs to inspect.
2. Apply the core move explicitly: use direct, personal, symbolic, and fantasy analogies to reframe the problem.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Synectics Reasoning rather than from a generic summary.
