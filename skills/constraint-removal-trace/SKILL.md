...
description: "Use this skill when the user invokes /constraint-removal-trace or asks for a default public reasoning trace using Constraint Removal. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Constraint Removal Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Constraint Removal strategy.
...
Apply Constraint Removal by using it to temporarily remove assumed limits and inspect what new options appear, and make each numbered line advance that method rather than adding generic filler.
...
Constraint Removal is a creative problem solving strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to temporarily remove assumed limits and inspect what new options appear, which helps the model expose reasoning checkpoints that a reader can inspect.
