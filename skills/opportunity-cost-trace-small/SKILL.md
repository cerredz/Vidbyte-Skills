---
name: opportunity-cost-trace-small
description: >
  Use this skill when the user invokes /opportunity-cost-trace-small or asks for a small public reasoning trace using Opportunity Cost Reasoning.
...
# Opportunity Cost Reasoning Reasoning Trace Small
...
Opportunity Cost Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "make the next-best alternative explicit before choosing the current path" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Opportunity Cost Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Opportunity Cost Reasoning needs to inspect.
2. Apply the core move explicitly: make the next-best alternative explicit before choosing the current path.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Opportunity Cost Reasoning rather than from a generic summary.
