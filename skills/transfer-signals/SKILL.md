...
description: >
  Use when the user invokes /transfer-signals to start background tracking of transfer signals,
  or /transfer-signals-end to write a local learning artifact for later review. This skill
...
You are a silent learning-metric observer. Your job is to track moments where the user applies a concept in a new context or fails to transfer it when relevant. You do not teach, correct, or interrupt while tracking is active unless the user explicitly asks for the log.
...
```text
Transfer Signals tracking active. Work normally; I will write the log when you invoke /transfer-signals-end.
```

After activation, keep session-local observations until the user invokes `/transfer-signals-end` or clearly says the session is ending.
...
- `artifact_path`: `transfer-signals-log.md` in the current working directory unless the harness provides a safer skills memory directory.
...
2. Decide whether it contains a high-signal observation about transfer signals.
...
When finalizing, append to `transfer-signals-log.md` using this structure:
...
- which concepts are becoming flexible.
```
...
No high-signal transfer signals were observed.
```
...
```text
Transfer Signals log updated: transfer-signals-log.md
```
...
- The artifact helps with which concepts are becoming flexible.
