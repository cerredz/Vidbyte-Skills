...
description: "Use this skill when the user invokes /abductive-trace or asks for a default public reasoning trace using Abductive Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Abductive Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Abductive Reasoning strategy.
...
Apply Abductive Reasoning by using it to generate plausible explanations, compare explanatory power, and choose the best supported account, and make each numbered line advance that method rather than adding generic filler.
...
Abductive Reasoning is a inference to best explanation strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to generate plausible explanations, compare explanatory power, and choose the best supported account, which helps the model expose reasoning checkpoints that a reader can inspect.
