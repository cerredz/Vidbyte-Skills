---
name: six-thinking-hats-trace-medium
description: >
  Use this skill when the user invokes /six-thinking-hats-trace-medium or asks for a medium public reasoning trace using Six Thinking Hats.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Six Thinking Hats as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by parallel thinking instead of a generic response.
---

# Six Thinking Hats Reasoning Trace Medium

## Goal
Use Six Thinking Hats to answer the user's question through parallel thinking, not through a generic checklist or interchangeable trace.
The trace should separate facts, feelings, risks, benefits, creativity, and process control into distinct passes, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make six thinking hats useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Six Thinking Hats move: separate facts, feelings, risks, benefits, creativity, and process control into distinct passes.
...
Six Thinking Hats is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "separate facts, feelings, risks, benefits, creativity, and process control into distinct passes" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Six Thinking Hats trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Six Thinking Hats needs to inspect.
2. Apply the core move explicitly: separate facts, feelings, risks, benefits, creativity, and process control into distinct passes.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Six Thinking Hats rather than from a generic summary.
