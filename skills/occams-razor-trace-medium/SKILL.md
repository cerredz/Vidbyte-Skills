---
name: occams-razor-trace-medium
description: >
  Use this skill when the user invokes /occams-razor-trace-medium or asks for a medium public reasoning trace using Occam's Razor.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Occam's Razor as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by preferring the simplest explanation that requires the fewest assumptions instead of a generic response.
---

# Occam's Razor Reasoning Trace Medium

## Goal
Use Occam's Razor to answer the user's question through preferring the simplest explanation that requires the fewest assumptions, not through a generic checklist or interchangeable trace.
The trace should prefer the explanation that requires the fewest assumptions while maintaining explanatory power, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make occam's razor useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Occam's Razor move: prefer the explanation that requires the fewest assumptions while maintaining explanatory power.
...
Occam's Razor is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "prefer the explanation that requires the fewest assumptions while maintaining explanatory power" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Occam's Razor trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Occam's Razor needs to inspect.
2. Apply the core move explicitly: prefer the explanation that requires the fewest assumptions while maintaining explanatory power.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Occam's Razor rather than from a generic summary.
