...
description: "Use this skill when the user invokes /incentive-analysis-trace or asks for a default public reasoning trace using Incentive Analysis. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Incentive Analysis Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Incentive Analysis strategy.
...
Apply Incentive Analysis by using it to infer behavior from rewards, constraints, penalties, and information available to actors, and make each numbered line advance that method rather than adding generic filler.
...
Incentive Analysis is a behavioral systems analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to infer behavior from rewards, constraints, penalties, and information available to actors, which helps the model expose reasoning checkpoints that a reader can inspect.
