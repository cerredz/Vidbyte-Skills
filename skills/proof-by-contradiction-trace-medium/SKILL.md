---
name: proof-by-contradiction-trace-medium
description: >
  Use this skill when the user invokes /proof-by-contradiction-trace-medium or asks for a medium public reasoning trace using Proof By Contradiction.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Proof By Contradiction as the actual structure of the analysis.
...
# Proof By Contradiction Reasoning Trace Medium

## Goal
Use Proof By Contradiction to answer the user's question through formal logic, not through a generic checklist or interchangeable trace.
The trace should assume the opposite of the target claim and show that it creates an inconsistency, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on premises, rules, cases, implications, contradictions, and validity checks, because those artifacts make proof by contradiction useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Proof By Contradiction move: assume the opposite of the target claim and show that it creates an inconsistency.
...
Proof By Contradiction is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "assume the opposite of the target claim and show that it creates an inconsistency" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Proof By Contradiction trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Proof By Contradiction needs to inspect.
2. Apply the core move explicitly: assume the opposite of the target claim and show that it creates an inconsistency.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Proof By Contradiction rather than from a generic summary.
