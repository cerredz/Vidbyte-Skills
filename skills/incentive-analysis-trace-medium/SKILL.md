---
name: incentive-analysis-trace-medium
description: >
  Use this skill when the user invokes /incentive-analysis-trace-medium or asks for a medium public reasoning trace using Incentive Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Incentive Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by behavioral systems analysis instead of a generic response.
---

# Incentive Analysis Reasoning Trace Medium

## Goal
Use Incentive Analysis to answer the user's question through behavioral systems analysis, not through a generic checklist or interchangeable trace.
The trace should infer behavior from rewards, constraints, penalties, and information available to actors, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on boundaries, components, feedback loops, constraints, delays, and leverage points, because those artifacts make incentive analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Incentive Analysis move: infer behavior from rewards, constraints, penalties, and information available to actors.
...
Incentive Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "infer behavior from rewards, constraints, penalties, and information available to actors" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Incentive Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Incentive Analysis needs to inspect.
2. Apply the core move explicitly: infer behavior from rewards, constraints, penalties, and information available to actors.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Incentive Analysis rather than from a generic summary.
