---
name: after-action-review-trace-medium
description: >
  Use this skill when the user invokes /after-action-review-trace-medium or asks for a medium public reasoning trace using After Action Review.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses After Action Review as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by conducting structured reflection on what happened, why, and how to improve instead of a generic response.
---

# After Action Review Reasoning Trace Medium

## Goal
Use After Action Review to answer the user's question through conducting structured reflection on what happened, why, and how to improve, not through a generic checklist or interchangeable trace.
The trace should answer four structured questions: what was expected, what actually happened, why was there a difference, and what should be done differently next time, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make after action review useful rather than decorative.
...
Build the scratchpad by repeatedly applying the After Action Review move: answer four structured questions: what was expected, what actually happened, why was there a difference, and what should be done differently next time.
...
After Action Review is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "answer four structured questions: what was expected, what actually happened, why was there a difference, and what should be done differently next time" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong After Action Review trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that After Action Review needs to inspect.
2. Apply the core move explicitly: answer four structured questions: what was expected, what actually happened, why was there a difference, and what should be done differently next time.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from After Action Review rather than from a generic summary.
