...
description: "Use this skill when the user invokes /propositional-logic-trace or asks for a default public reasoning trace using Propositional Logic Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Propositional Logic Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Propositional Logic Reasoning strategy.
...
Apply Propositional Logic Reasoning by using it to represent claims as propositions and test implications, conjunctions, disjunctions, and negations, and make each numbered line advance that method rather than adding generic filler.
...
Propositional Logic Reasoning is a formal logic strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to represent claims as propositions and test implications, conjunctions, disjunctions, and negations, which helps the model expose reasoning checkpoints that a reader can inspect.
