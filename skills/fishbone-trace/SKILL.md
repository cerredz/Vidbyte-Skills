---
name: fishbone-trace
description: "Use this skill when the user invokes /fishbone-trace or asks for a default public reasoning trace using Fishbone Diagram Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Fishbone Diagram Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Fishbone Diagram Reasoning strategy.
...
Apply Fishbone Diagram Reasoning by using it to sort candidate causes into categories so missing families of causes become visible, and make each numbered line advance that method rather than adding generic filler.
...
Fishbone Diagram Reasoning is a root cause analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to sort candidate causes into categories so missing families of causes become visible, which helps the model expose reasoning checkpoints that a reader can inspect.
