...
description: "Use this skill when the user invokes /porters-five-forces-trace or asks for a default public reasoning trace using Porters Five Forces. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Porters Five Forces Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Porters Five Forces strategy.
...
Apply Porters Five Forces by using it to evaluate rivalry, entrants, substitutes, suppliers, and buyers before judging attractiveness, and make each numbered line advance that method rather than adding generic filler.
...
Porters Five Forces is a competitive strategy strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to evaluate rivalry, entrants, substitutes, suppliers, and buyers before judging attractiveness, which helps the model expose reasoning checkpoints that a reader can inspect.
