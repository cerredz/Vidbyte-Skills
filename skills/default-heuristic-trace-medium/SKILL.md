---
name: default-heuristic-trace-medium
description: >
  Use this skill when the user invokes /default-heuristic-trace-medium or asks for a medium public reasoning trace using Default Heuristic.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Default Heuristic as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by accepting the preset option rather than actively choosing an alternative instead of a generic response.
---

# Default Heuristic Reasoning Trace Medium

## Goal
Use Default Heuristic to answer the user's question through accepting the preset option rather than actively choosing an alternative, not through a generic checklist or interchangeable trace.
The trace should accept the preset default option when making a choice rather than expending effort to evaluate alternatives, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make default heuristic useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Default Heuristic move: accept the preset default option when making a choice rather than expending effort to evaluate alternatives.
...
Default Heuristic is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "accept the preset default option when making a choice rather than expending effort to evaluate alternatives" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Default Heuristic trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Default Heuristic needs to inspect.
2. Apply the core move explicitly: accept the preset default option when making a choice rather than expending effort to evaluate alternatives.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Default Heuristic rather than from a generic summary.
