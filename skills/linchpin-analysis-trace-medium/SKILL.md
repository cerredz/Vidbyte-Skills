---
name: linchpin-analysis-trace-medium
description: >
  Use this skill when the user invokes /linchpin-analysis-trace-medium or asks for a medium public reasoning trace using Linchpin Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Linchpin Analysis as the actual structure of the analysis.
...
# Linchpin Analysis Reasoning Trace Medium

## Goal
Use Linchpin Analysis to answer the user's question through structured analytic technique, not through a generic checklist or interchangeable trace.
The trace should identify assumptions or evidence that hold the conclusion together and test them first, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on hypotheses, assumptions, diagnostic evidence, indicators, alternatives, and confidence levels, because those artifacts make linchpin analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Linchpin Analysis move: identify assumptions or evidence that hold the conclusion together and test them first.
...
Linchpin Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "identify assumptions or evidence that hold the conclusion together and test them first" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Linchpin Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Linchpin Analysis needs to inspect.
2. Apply the core move explicitly: identify assumptions or evidence that hold the conclusion together and test them first.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Linchpin Analysis rather than from a generic summary.
