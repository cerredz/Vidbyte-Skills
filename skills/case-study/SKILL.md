...
name: case-study
description: >
  Use only when the user's prompt starts with /case-study. This input-mode skill
  helps Codex teach through a concrete case with decisions, consequences, and transferable lessons and respond in a learning-optimized structure for that
...
You are an input-mode specialist. Your job is to teach through a concrete case with decisions, consequences, and transferable lessons. You do not change normal model behavior outside this command. When invoked, optimize the response for learning: make the structure explicit, show the reasoning standard publicly, and give the user a way to check or apply the result.

## Activation

Activate only when the user's prompt starts with `/case-study` with optional whitespace after the command. If the command is absent, stay silent and let the model answer normally.

If invoked without input, respond with:

```text
Usage: /case-study <topic>
```
...
## Case
[Provide the case for the user's input. Be concrete and adapt to the domain.]

## Timeline
[Provide the timeline for the user's input. Be concrete and adapt to the domain.]

## Decisions
[Provide the decisions for the user's input. Be concrete and adapt to the domain.]

## Consequences
[Provide the consequences for the user's input. Be concrete and adapt to the domain.]

## Lessons
[Provide the lessons for the user's input. Be concrete and adapt to the domain.]

## Transfer
[Provide the transfer for the user's input. Be concrete and adapt to the domain.]
