...
description: "Use this skill when the user invokes /lateral-thinking-trace or asks for a default public reasoning trace using Lateral Thinking. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Lateral Thinking Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Lateral Thinking strategy.
...
Apply Lateral Thinking by using it to approach the problem indirectly, break habitual frames, and generate non-obvious moves, and make each numbered line advance that method rather than adding generic filler.
...
Lateral Thinking is a creative problem solving strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to approach the problem indirectly, break habitual frames, and generate non-obvious moves, which helps the model expose reasoning checkpoints that a reader can inspect.
