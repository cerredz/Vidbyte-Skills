---
name: speed-accuracy-tradeoff-trace-medium
description: >
  Use this skill when the user invokes /speed-accuracy-tradeoff-trace-medium or asks for a medium public reasoning trace using Speed-Accuracy Tradeoff.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Speed-Accuracy Tradeoff as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by explicitly balancing decision speed against accuracy requirements for the context instead of a generic response.
---

# Speed-Accuracy Tradeoff Reasoning Trace Medium

## Goal
Use Speed-Accuracy Tradeoff to answer the user's question through explicitly balancing decision speed against accuracy requirements for the context, not through a generic checklist or interchangeable trace.
The trace should explicitly balance the need for decision speed against the need for accuracy by matching the decision strategy to the error costs and time constraints of the situation, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make speed-accuracy tradeoff useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Speed-Accuracy Tradeoff move: explicitly balance the need for decision speed against the need for accuracy by matching the decision strategy to the error costs and time constraints of the situation.
...
Speed-Accuracy Tradeoff is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "explicitly balance the need for decision speed against the need for accuracy by matching the decision strategy to the error costs and time constraints of the situation" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Speed-Accuracy Tradeoff trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Speed-Accuracy Tradeoff needs to inspect.
2. Apply the core move explicitly: explicitly balance the need for decision speed against the need for accuracy by matching the decision strategy to the error costs and time constraints of the situation.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Speed-Accuracy Tradeoff rather than from a generic summary.
