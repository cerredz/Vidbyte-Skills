---
name: ooda-red-team-trace-medium
description: >
  Use this skill when the user invokes /ooda-red-team-trace-medium or asks for a medium public reasoning trace using OODA Red Team.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses OODA Red Team as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by adversarial adaptation instead of a generic response.
---

# OODA Red Team Reasoning Trace Medium

## Goal
Use OODA Red Team to answer the user's question through adversarial adaptation, not through a generic checklist or interchangeable trace.
The trace should run the OODA cycle from an opposing actor's perspective and compare likely moves, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make ooda red team useful rather than decorative.
...
Build the scratchpad by repeatedly applying the OODA Red Team move: run the OODA cycle from an opposing actor's perspective and compare likely moves.
...
OODA Red Team is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "run the OODA cycle from an opposing actor's perspective and compare likely moves" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong OODA Red Team trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that OODA Red Team needs to inspect.
2. Apply the core move explicitly: run the OODA cycle from an opposing actor's perspective and compare likely moves.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from OODA Red Team rather than from a generic summary.
