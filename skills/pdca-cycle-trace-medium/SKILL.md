---
name: pdca-cycle-trace-medium
description: >
  Use this skill when the user invokes /pdca-cycle-trace-medium or asks for a medium public reasoning trace using PDCA Cycle.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses PDCA Cycle as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by iterating through Plan, Do, Check, and Act for continuous improvement instead of a generic response.
---

# PDCA Cycle Reasoning Trace Medium

## Goal
Use PDCA Cycle to answer the user's question through iterating through Plan, Do, Check, and Act for continuous improvement, not through a generic checklist or interchangeable trace.
The trace should iterate through four stages: plan a change, do it on a small scale, check the results, and act to standardize or adjust, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make pdca cycle useful rather than decorative.
...
Build the scratchpad by repeatedly applying the PDCA Cycle move: iterate through four stages: plan a change, do it on a small scale, check the results, and act to standardize or adjust.
...
PDCA Cycle is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "iterate through four stages: plan a change, do it on a small scale, check the results, and act to standardize or adjust" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong PDCA Cycle trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that PDCA Cycle needs to inspect.
2. Apply the core move explicitly: iterate through four stages: plan a change, do it on a small scale, check the results, and act to standardize or adjust.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from PDCA Cycle rather than from a generic summary.
