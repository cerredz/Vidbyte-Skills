...
description: "Use this skill when the user invokes /reference-class-forecasting-trace or asks for a default public reasoning trace using Reference Class Forecasting. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Reference Class Forecasting Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Reference Class Forecasting strategy.
...
Apply Reference Class Forecasting by using it to select comparable cases, derive an empirical baseline, and adjust only with justified evidence, and make each numbered line advance that method rather than adding generic filler.
...
Reference Class Forecasting is a forecast calibration strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to select comparable cases, derive an empirical baseline, and adjust only with justified evidence, which helps the model expose reasoning checkpoints that a reader can inspect.
