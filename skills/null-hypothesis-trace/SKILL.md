...
description: "Use this skill when the user invokes /null-hypothesis-trace or asks for a default public reasoning trace using Null Hypothesis Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Null Hypothesis Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Null Hypothesis Reasoning strategy.
...
Apply Null Hypothesis Reasoning by using it to assume no effect or no difference, then ask whether evidence is strong enough to reject that baseline, and make each numbered line advance that method rather than adding generic filler.
...
Null Hypothesis Reasoning is a statistical inference strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to assume no effect or no difference, then ask whether evidence is strong enough to reject that baseline, which helps the model expose reasoning checkpoints that a reader can inspect.
