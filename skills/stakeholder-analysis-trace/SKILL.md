...
description: "Use this skill when the user invokes /stakeholder-analysis-trace or asks for a default public reasoning trace using Stakeholder Analysis. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Stakeholder Analysis Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Stakeholder Analysis strategy.
...
Apply Stakeholder Analysis by using it to map affected parties, incentives, power, needs, and likely reactions, and make each numbered line advance that method rather than adding generic filler.
...
Stakeholder Analysis is a social systems analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to map affected parties, incentives, power, needs, and likely reactions, which helps the model expose reasoning checkpoints that a reader can inspect.
