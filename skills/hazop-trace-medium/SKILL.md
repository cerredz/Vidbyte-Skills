---
name: hazop-trace-medium
description: >
  Use this skill when the user invokes /hazop-trace-medium or asks for a medium public reasoning trace using HAZOP Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses HAZOP Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by systematically examining process deviations from design intent to identify hazards instead of a generic response.
---

# HAZOP Analysis Reasoning Trace Medium

## Goal
Use HAZOP Analysis to answer the user's question through systematically examining process deviations from design intent to identify hazards, not through a generic checklist or interchangeable trace.
The trace should systematically examine how deviations from design intent in each process parameter could lead to hazards or operability problems, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make hazop analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the HAZOP Analysis move: systematically examine how deviations from design intent in each process parameter could lead to hazards or operability problems.
...
HAZOP Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "systematically examine how deviations from design intent in each process parameter could lead to hazards or operability problems" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong HAZOP Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that HAZOP Analysis needs to inspect.
2. Apply the core move explicitly: systematically examine how deviations from design intent in each process parameter could lead to hazards or operability problems.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from HAZOP Analysis rather than from a generic summary.
