---
name: regret-minimization-trace-medium
description: >
  Use this skill when the user invokes /regret-minimization-trace-medium or asks for a medium public reasoning trace using Regret Minimization.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Regret Minimization as the actual structure of the analysis.
...
# Regret Minimization Reasoning Trace Medium

## Goal
Use Regret Minimization to answer the user's question through decision analysis, not through a generic checklist or interchangeable trace.
The trace should compare future remorse across choices and reduce the cost of being wrong, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make regret minimization useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Regret Minimization move: compare future remorse across choices and reduce the cost of being wrong.
...
Regret Minimization is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "compare future remorse across choices and reduce the cost of being wrong" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Regret Minimization trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Regret Minimization needs to inspect.
2. Apply the core move explicitly: compare future remorse across choices and reduce the cost of being wrong.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Regret Minimization rather than from a generic summary.
