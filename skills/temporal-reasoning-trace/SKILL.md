...
description: "Use this skill when the user invokes /temporal-reasoning-trace or asks for a default public reasoning trace using Temporal Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Temporal Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Temporal Reasoning strategy.
...
Apply Temporal Reasoning by using it to order events, durations, deadlines, lags, and time-dependent constraints, and make each numbered line advance that method rather than adding generic filler.
...
Temporal Reasoning is a time-based logic strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to order events, durations, deadlines, lags, and time-dependent constraints, which helps the model expose reasoning checkpoints that a reader can inspect.
