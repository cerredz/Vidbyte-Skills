---
name: steelman-trace
description: "Use this skill when the user invokes /steelman-trace or asks for a default public reasoning trace using Steelman Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Steelman Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Steelman Reasoning strategy.
...
Apply Steelman Reasoning by using it to reconstruct the strongest version of an opposing argument before responding, and make each numbered line advance that method rather than adding generic filler.
...
Steelman Reasoning is a argument improvement strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to reconstruct the strongest version of an opposing argument before responding, which helps the model expose reasoning checkpoints that a reader can inspect.
