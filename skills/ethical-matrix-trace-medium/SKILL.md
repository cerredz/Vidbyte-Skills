---
name: ethical-matrix-trace-medium
description: >
  Use this skill when the user invokes /ethical-matrix-trace-medium or asks for a medium public reasoning trace using Ethical Matrix.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Ethical Matrix as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by ethical analysis instead of a generic response.
---

# Ethical Matrix Reasoning Trace Medium

## Goal
Use Ethical Matrix to answer the user's question through ethical analysis, not through a generic checklist or interchangeable trace.
The trace should compare affected groups against principles such as wellbeing, autonomy, fairness, and responsibility, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on stakeholders, principles, rules, harms, benefits, precedents, and justified tradeoffs, because those artifacts make ethical matrix useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Ethical Matrix move: compare affected groups against principles such as wellbeing, autonomy, fairness, and responsibility.
...
Ethical Matrix is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "compare affected groups against principles such as wellbeing, autonomy, fairness, and responsibility" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Ethical Matrix trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Ethical Matrix needs to inspect.
2. Apply the core move explicitly: compare affected groups against principles such as wellbeing, autonomy, fairness, and responsibility.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Ethical Matrix rather than from a generic summary.
