---
name: comparative-case-trace-medium
description: >
  Use this skill when the user invokes /comparative-case-trace-medium or asks for a medium public reasoning trace using Comparative Case Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Comparative Case Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by case comparison instead of a generic response.
---

# Comparative Case Reasoning Reasoning Trace Medium

## Goal
Use Comparative Case Reasoning to answer the user's question through case comparison, not through a generic checklist or interchangeable trace.
The trace should compare cases systematically to isolate similarities, differences, and transfer limits, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make comparative case reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Comparative Case Reasoning move: compare cases systematically to isolate similarities, differences, and transfer limits.
...
Comparative Case Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "compare cases systematically to isolate similarities, differences, and transfer limits" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Comparative Case Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Comparative Case Reasoning needs to inspect.
2. Apply the core move explicitly: compare cases systematically to isolate similarities, differences, and transfer limits.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Comparative Case Reasoning rather than from a generic summary.
