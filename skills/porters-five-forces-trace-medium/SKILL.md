---
name: porters-five-forces-trace-medium
description: >
  Use this skill when the user invokes /porters-five-forces-trace-medium or asks for a medium public reasoning trace using Porters Five Forces.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Porters Five Forces as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by competitive strategy instead of a generic response.
---

# Porters Five Forces Reasoning Trace Medium

## Goal
Use Porters Five Forces to answer the user's question through competitive strategy, not through a generic checklist or interchangeable trace.
The trace should evaluate rivalry, entrants, substitutes, suppliers, and buyers before judging attractiveness, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make porters five forces useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Porters Five Forces move: evaluate rivalry, entrants, substitutes, suppliers, and buyers before judging attractiveness.
...
Porters Five Forces is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "evaluate rivalry, entrants, substitutes, suppliers, and buyers before judging attractiveness" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Porters Five Forces trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Porters Five Forces needs to inspect.
2. Apply the core move explicitly: evaluate rivalry, entrants, substitutes, suppliers, and buyers before judging attractiveness.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Porters Five Forces rather than from a generic summary.
