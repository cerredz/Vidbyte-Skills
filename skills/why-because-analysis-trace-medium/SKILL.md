---
name: why-because-analysis-trace-medium
description: >
  Use this skill when the user invokes /why-because-analysis-trace-medium or asks for a medium public reasoning trace using Why-Because Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Why-Because Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by tracing causal chains through formal counterfactual reasoning instead of a generic response.
---

# Why-Because Analysis Reasoning Trace Medium

## Goal
Use Why-Because Analysis to answer the user's question through tracing causal chains through formal counterfactual reasoning, not through a generic checklist or interchangeable trace.
The trace should trace causal chains using formal counterfactual tests to determine what was necessary and sufficient for an outcome, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make why-because analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Why-Because Analysis move: trace causal chains using formal counterfactual tests to determine what was necessary and sufficient for an outcome.
...
Why-Because Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "trace causal chains using formal counterfactual tests to determine what was necessary and sufficient for an outcome" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Why-Because Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Why-Because Analysis needs to inspect.
2. Apply the core move explicitly: trace causal chains using formal counterfactual tests to determine what was necessary and sufficient for an outcome.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Why-Because Analysis rather than from a generic summary.
