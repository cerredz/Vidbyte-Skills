...
Sensitivity Analysis is a robustness testing strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to vary key assumptions to see which inputs most change the conclusion, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, sensitivity analysis changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Sensitivity Analysis, not like a separate response pasted after the trace.
