---
name: sensitivity-analysis-trace-medium
description: >
  Use this skill when the user invokes /sensitivity-analysis-trace-medium or asks for a medium public reasoning trace using Sensitivity Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Sensitivity Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by robustness testing instead of a generic response.
---

# Sensitivity Analysis Reasoning Trace Medium

## Goal
Use Sensitivity Analysis to answer the user's question through robustness testing, not through a generic checklist or interchangeable trace.
The trace should vary key assumptions to see which inputs most change the conclusion, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make sensitivity analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Sensitivity Analysis move: vary key assumptions to see which inputs most change the conclusion.
...
Sensitivity Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "vary key assumptions to see which inputs most change the conclusion" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Sensitivity Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Sensitivity Analysis needs to inspect.
2. Apply the core move explicitly: vary key assumptions to see which inputs most change the conclusion.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Sensitivity Analysis rather than from a generic summary.
