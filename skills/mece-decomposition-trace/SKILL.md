...
description: "Use this skill when the user invokes /mece-decomposition-trace or asks for a default public reasoning trace using MECE Decomposition. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# MECE Decomposition Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the MECE Decomposition strategy.
...
Apply MECE Decomposition by using it to split the problem into mutually exclusive and collectively exhaustive parts before solving each part, and make each numbered line advance that method rather than adding generic filler.
...
MECE Decomposition is a structured decomposition strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to split the problem into mutually exclusive and collectively exhaustive parts before solving each part, which helps the model expose reasoning checkpoints that a reader can inspect.
