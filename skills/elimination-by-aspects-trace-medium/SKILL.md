---
name: elimination-by-aspects-trace-medium
description: >
  Use this skill when the user invokes /elimination-by-aspects-trace-medium or asks for a medium public reasoning trace using Elimination by Aspects.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Elimination by Aspects as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by eliminating alternatives that fail a criterion one attribute at a time instead of a generic response.
---

# Elimination by Aspects Reasoning Trace Medium

## Goal
Use Elimination by Aspects to answer the user's question through eliminating alternatives that fail a criterion one attribute at a time, not through a generic checklist or interchangeable trace.
The trace should eliminate alternatives that fail a criterion one attribute at a time until one choice remains, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make elimination by aspects useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Elimination by Aspects move: eliminate alternatives that fail a criterion one attribute at a time until one choice remains.
...
Elimination by Aspects is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "eliminate alternatives that fail a criterion one attribute at a time until one choice remains" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Elimination by Aspects trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Elimination by Aspects needs to inspect.
2. Apply the core move explicitly: eliminate alternatives that fail a criterion one attribute at a time until one choice remains.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Elimination by Aspects rather than from a generic summary.
