---
name: hypothesis-testing-trace-medium
description: >
  Use this skill when the user invokes /hypothesis-testing-trace-medium or asks for a medium public reasoning trace using Hypothesis Testing.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Hypothesis Testing as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by empirical evaluation instead of a generic response.
---

# Hypothesis Testing Reasoning Trace Medium

## Goal
Use Hypothesis Testing to answer the user's question through empirical evaluation, not through a generic checklist or interchangeable trace.
The trace should state testable claims, define disconfirming evidence, and compare observations to expectations, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make hypothesis testing useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Hypothesis Testing move: state testable claims, define disconfirming evidence, and compare observations to expectations.
...
Hypothesis Testing is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "state testable claims, define disconfirming evidence, and compare observations to expectations" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Hypothesis Testing trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Hypothesis Testing needs to inspect.
2. Apply the core move explicitly: state testable claims, define disconfirming evidence, and compare observations to expectations.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Hypothesis Testing rather than from a generic summary.
