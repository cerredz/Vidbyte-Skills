---
name: mind-map-trace-medium
description: >
  Use this skill when the user invokes /mind-map-trace-medium or asks for a medium public reasoning trace using Mind Mapping.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Mind Mapping as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by associative exploration instead of a generic response.
---

# Mind Mapping Reasoning Trace Medium

## Goal
Use Mind Mapping to answer the user's question through associative exploration, not through a generic checklist or interchangeable trace.
The trace should radiate related concepts from a central question and cluster them into patterns, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make mind mapping useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Mind Mapping move: radiate related concepts from a central question and cluster them into patterns.
...
Mind Mapping is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "radiate related concepts from a central question and cluster them into patterns" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Mind Mapping trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Mind Mapping needs to inspect.
2. Apply the core move explicitly: radiate related concepts from a central question and cluster them into patterns.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Mind Mapping rather than from a generic summary.
