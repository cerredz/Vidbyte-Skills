...
Correlation Versus Causation is a causal inference strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to separate association from mechanism, temporal order, and intervention evidence, which forces the model to make the important reasoning moves visible.
This strategy is strongest when the question benefits from separating mechanism from coincidence and testing what would change an outcome and when the reader needs to see why the answer follows.
Compared with a generic scratchpad, correlation versus causation changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Correlation Versus Causation, not like a separate response pasted after the trace.
