---
name: correlation-causation-trace-medium
description: >
  Use this skill when the user invokes /correlation-causation-trace-medium or asks for a medium public reasoning trace using Correlation Versus Causation.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Correlation Versus Causation as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by causal inference instead of a generic response.
---

# Correlation Versus Causation Reasoning Trace Medium

## Goal
Use Correlation Versus Causation to answer the user's question through causal inference, not through a generic checklist or interchangeable trace.
The trace should separate association from mechanism, temporal order, and intervention evidence, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on causes, mechanisms, counterfactual tests, evidence quality, and intervention points, because those artifacts make correlation versus causation useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Correlation Versus Causation move: separate association from mechanism, temporal order, and intervention evidence.
...
Correlation Versus Causation is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "separate association from mechanism, temporal order, and intervention evidence" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Correlation Versus Causation trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Correlation Versus Causation needs to inspect.
2. Apply the core move explicitly: separate association from mechanism, temporal order, and intervention evidence.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Correlation Versus Causation rather than from a generic summary.
