---
name: business-model-canvas-trace-medium
description: >
  Use this skill when the user invokes /business-model-canvas-trace-medium or asks for a medium public reasoning trace using Business Model Canvas.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Business Model Canvas as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by mapping nine building blocks of a business model on a single page instead of a generic response.
---

# Business Model Canvas Reasoning Trace Medium

## Goal
Use Business Model Canvas to answer the user's question through mapping nine building blocks of a business model on a single page, not through a generic checklist or interchangeable trace.
The trace should map nine building blocks: value propositions, customer segments, channels, customer relationships, revenue streams, key resources, key activities, key partners, and cost structure, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make business model canvas useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Business Model Canvas move: map nine building blocks: value propositions, customer segments, channels, customer relationships, revenue streams, key resources, key activities, key partners, and cost structure.
...
Business Model Canvas is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "map nine building blocks: value propositions, customer segments, channels, customer relationships, revenue streams, key resources, key activities, key partners, and cost structure" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Business Model Canvas trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Business Model Canvas needs to inspect.
2. Apply the core move explicitly: map nine building blocks: value propositions, customer segments, channels, customer relationships, revenue streams, key resources, key activities, key partners, and cost structure.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Business Model Canvas rather than from a generic summary.
