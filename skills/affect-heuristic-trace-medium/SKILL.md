---
name: affect-heuristic-trace-medium
description: >
  Use this skill when the user invokes /affect-heuristic-trace-medium or asks for a medium public reasoning trace using Affect Heuristic.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Affect Heuristic as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by using emotional response to judge risks and benefits of a decision instead of a generic response.
---

# Affect Heuristic Reasoning Trace Medium

## Goal
Use Affect Heuristic to answer the user's question through using emotional response to judge risks and benefits of a decision, not through a generic checklist or interchangeable trace.
The trace should use immediate emotional response to judge the risks and benefits of a decision, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make affect heuristic useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Affect Heuristic move: use immediate emotional response to judge the risks and benefits of a decision.
...
Affect Heuristic is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "use immediate emotional response to judge the risks and benefits of a decision" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Affect Heuristic trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Affect Heuristic needs to inspect.
2. Apply the core move explicitly: use immediate emotional response to judge the risks and benefits of a decision.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Affect Heuristic rather than from a generic summary.
