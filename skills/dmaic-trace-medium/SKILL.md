---
name: dmaic-trace-medium
description: >
  Use this skill when the user invokes /dmaic-trace-medium or asks for a medium public reasoning trace using DMAIC Methodology.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses DMAIC Methodology as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by following Define, Measure, Analyze, Improve, and Control for process improvement instead of a generic response.
---

# DMAIC Methodology Reasoning Trace Medium

## Goal
Use DMAIC Methodology to answer the user's question through following Define, Measure, Analyze, Improve, and Control for process improvement, not through a generic checklist or interchangeable trace.
The trace should follow five phases: define the problem, measure current performance, analyze root causes, improve the process, and control to sustain gains, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make dmaic methodology useful rather than decorative.
...
Build the scratchpad by repeatedly applying the DMAIC Methodology move: follow five phases: define the problem, measure current performance, analyze root causes, improve the process, and control to sustain gains.
...
DMAIC Methodology is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "follow five phases: define the problem, measure current performance, analyze root causes, improve the process, and control to sustain gains" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong DMAIC Methodology trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that DMAIC Methodology needs to inspect.
2. Apply the core move explicitly: follow five phases: define the problem, measure current performance, analyze root causes, improve the process, and control to sustain gains.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from DMAIC Methodology rather than from a generic summary.
