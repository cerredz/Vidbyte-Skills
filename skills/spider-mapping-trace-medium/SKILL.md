---
name: spider-mapping-trace-medium
description: >
  Use this skill when the user invokes /spider-mapping-trace-medium or asks for a medium public reasoning trace using Spider Mapping.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Spider Mapping as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by radiating ideas outward from a central concept in a visual web instead of a generic response.
---

# Spider Mapping Reasoning Trace Medium

## Goal
Use Spider Mapping to answer the user's question through radiating ideas outward from a central concept in a visual web, not through a generic checklist or interchangeable trace.
The trace should start with a central concept and radiate connected ideas outward in a visual web, then inspect the emerging structure, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make spider mapping useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Spider Mapping move: start with a central concept and radiate connected ideas outward in a visual web, then inspect the emerging structure.
...
Spider Mapping is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "start with a central concept and radiate connected ideas outward in a visual web, then inspect the emerging structure" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Spider Mapping trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Spider Mapping needs to inspect.
2. Apply the core move explicitly: start with a central concept and radiate connected ideas outward in a visual web, then inspect the emerging structure.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Spider Mapping rather than from a generic summary.
