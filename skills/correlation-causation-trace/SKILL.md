...
description: "Use this skill when the user invokes /correlation-causation-trace or asks for a default public reasoning trace using Correlation Versus Causation. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Correlation Versus Causation Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Correlation Versus Causation strategy.
...
Apply Correlation Versus Causation by using it to separate association from mechanism, temporal order, and intervention evidence, and make each numbered line advance that method rather than adding generic filler.
...
Correlation Versus Causation is a causal inference strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to separate association from mechanism, temporal order, and intervention evidence, which helps the model expose reasoning checkpoints that a reader can inspect.
