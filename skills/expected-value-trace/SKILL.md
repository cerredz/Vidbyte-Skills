...
description: "Use this skill when the user invokes /expected-value-trace or asks for a default public reasoning trace using Expected Value Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Expected Value Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Expected Value Reasoning strategy.
...
Apply Expected Value Reasoning by using it to combine outcomes with probabilities so decisions account for both payoff and likelihood, and make each numbered line advance that method rather than adding generic filler.
...
Expected Value Reasoning is a decision analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to combine outcomes with probabilities so decisions account for both payoff and likelihood, which helps the model expose reasoning checkpoints that a reader can inspect.
