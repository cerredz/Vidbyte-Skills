...
description: "Use this skill when the user invokes /stock-and-flow-trace or asks for a default public reasoning trace using Stock And Flow Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Stock And Flow Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Stock And Flow Reasoning strategy.
...
Apply Stock And Flow Reasoning by using it to distinguish accumulations from rates of change and explain delays, and make each numbered line advance that method rather than adding generic filler.
...
Stock And Flow Reasoning is a systems dynamics strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to distinguish accumulations from rates of change and explain delays, which helps the model expose reasoning checkpoints that a reader can inspect.
