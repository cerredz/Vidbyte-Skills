---
name: error-analysis-trace-medium
description: >
  Use this skill when the user invokes /error-analysis-trace-medium or asks for a medium public reasoning trace using Error Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Error Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by diagnostic evaluation instead of a generic response.
---

# Error Analysis Reasoning Trace Medium

## Goal
Use Error Analysis to answer the user's question through diagnostic evaluation, not through a generic checklist or interchangeable trace.
The trace should categorize failures, estimate frequency, and identify the changes most likely to reduce them, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make error analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Error Analysis move: categorize failures, estimate frequency, and identify the changes most likely to reduce them.
...
Error Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "categorize failures, estimate frequency, and identify the changes most likely to reduce them" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Error Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Error Analysis needs to inspect.
2. Apply the core move explicitly: categorize failures, estimate frequency, and identify the changes most likely to reduce them.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Error Analysis rather than from a generic summary.
