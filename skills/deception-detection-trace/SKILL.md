...
description: "Use this skill when the user invokes /deception-detection-trace or asks for a default public reasoning trace using Deception Detection. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Deception Detection Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Deception Detection strategy.
...
Apply Deception Detection by using it to ask how evidence could be manipulated, fabricated, omitted, or misread, and make each numbered line advance that method rather than adding generic filler.
...
Deception Detection is a adversarial evidence analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to ask how evidence could be manipulated, fabricated, omitted, or misread, which helps the model expose reasoning checkpoints that a reader can inspect.
