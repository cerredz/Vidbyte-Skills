---
name: red-team-trace-small
description: >
  Use this skill when the user invokes /red-team-trace-small or asks for a small public reasoning trace using Red Team Reasoning.
...
# Red Team Reasoning Reasoning Trace Small
...
Red Team Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "simulate a capable opponent or critic to stress-test plans, claims, and defenses" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Red Team Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Red Team Reasoning needs to inspect.
2. Apply the core move explicitly: simulate a capable opponent or critic to stress-test plans, claims, and defenses.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Red Team Reasoning rather than from a generic summary.
