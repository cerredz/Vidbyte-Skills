...
description: "Use this skill when the user invokes /tradeoff-matrix-trace or asks for a default public reasoning trace using Tradeoff Matrix. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Tradeoff Matrix Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Tradeoff Matrix strategy.
...
Apply Tradeoff Matrix by using it to score options across criteria and expose the compromises behind the recommendation, and make each numbered line advance that method rather than adding generic filler.
...
Tradeoff Matrix is a comparative decision analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to score options across criteria and expose the compromises behind the recommendation, which helps the model expose reasoning checkpoints that a reader can inspect.
