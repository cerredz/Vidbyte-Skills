---
name: fairness-analysis-trace-medium
description: >
  Use this skill when the user invokes /fairness-analysis-trace-medium or asks for a medium public reasoning trace using Fairness Analysis.
...
# Fairness Analysis Reasoning Trace Medium
...
Fairness Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "check who benefits, who bears costs, and whether criteria are consistent and justified" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Fairness Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Fairness Analysis needs to inspect.
2. Apply the core move explicitly: check who benefits, who bears costs, and whether criteria are consistent and justified.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Fairness Analysis rather than from a generic summary.
