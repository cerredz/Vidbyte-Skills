...
description: "Use this skill when the user invokes /evidence-triangulation-trace or asks for a default public reasoning trace using Evidence Triangulation. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Evidence Triangulation Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Evidence Triangulation strategy.
...
Apply Evidence Triangulation by using it to combine independent evidence streams and compare whether they converge, and make each numbered line advance that method rather than adding generic filler.
...
Evidence Triangulation is a evidence synthesis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to combine independent evidence streams and compare whether they converge, which helps the model expose reasoning checkpoints that a reader can inspect.
