---
name: expected-value-trace-medium
description: >
  Use this skill when the user invokes /expected-value-trace-medium or asks for a medium public reasoning trace using Expected Value Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Expected Value Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by decision analysis instead of a generic response.
---

# Expected Value Reasoning Reasoning Trace Medium

## Goal
Use Expected Value Reasoning to answer the user's question through decision analysis, not through a generic checklist or interchangeable trace.
The trace should combine outcomes with probabilities so decisions account for both payoff and likelihood, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make expected value reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Expected Value Reasoning move: combine outcomes with probabilities so decisions account for both payoff and likelihood.
...
Expected Value Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "combine outcomes with probabilities so decisions account for both payoff and likelihood" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Expected Value Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Expected Value Reasoning needs to inspect.
2. Apply the core move explicitly: combine outcomes with probabilities so decisions account for both payoff and likelihood.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Expected Value Reasoning rather than from a generic summary.
