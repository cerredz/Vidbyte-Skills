...
Expected Value Reasoning is a decision analysis strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to combine outcomes with probabilities so decisions account for both payoff and likelihood, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, expected value reasoning changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Expected Value Reasoning, not like a separate response pasted after the trace.
