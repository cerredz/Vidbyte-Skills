---
name: modal-reasoning-trace-medium
description: >
  Use this skill when the user invokes /modal-reasoning-trace-medium or asks for a medium public reasoning trace using Modal Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Modal Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by possibility and necessity logic instead of a generic response.
---

# Modal Reasoning Reasoning Trace Medium

## Goal
Use Modal Reasoning to answer the user's question through possibility and necessity logic, not through a generic checklist or interchangeable trace.
The trace should separate what is necessary, possible, impossible, and contingent, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on premises, rules, cases, implications, contradictions, and validity checks, because those artifacts make modal reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Modal Reasoning move: separate what is necessary, possible, impossible, and contingent.
...
Modal Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "separate what is necessary, possible, impossible, and contingent" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Modal Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Modal Reasoning needs to inspect.
2. Apply the core move explicitly: separate what is necessary, possible, impossible, and contingent.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Modal Reasoning rather than from a generic summary.
