...
description: "Use this skill when the user invokes /quasi-experimental-trace or asks for a default public reasoning trace using Quasi Experimental Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Quasi Experimental Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Quasi Experimental Reasoning strategy.
...
Apply Quasi Experimental Reasoning by using it to use imperfect natural comparisons while naming threats to validity, and make each numbered line advance that method rather than adding generic filler.
...
Quasi Experimental Reasoning is a causal inference strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to use imperfect natural comparisons while naming threats to validity, which helps the model expose reasoning checkpoints that a reader can inspect.
