---
name: uncertainty-quantification-trace-medium
description: >
  Use this skill when the user invokes /uncertainty-quantification-trace-medium or asks for a medium public reasoning trace using Uncertainty Quantification.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Uncertainty Quantification as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by risk and probability analysis instead of a generic response.
---

# Uncertainty Quantification Reasoning Trace Medium

## Goal
Use Uncertainty Quantification to answer the user's question through risk and probability analysis, not through a generic checklist or interchangeable trace.
The trace should express confidence ranges, sources of uncertainty, and what would narrow them, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make uncertainty quantification useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Uncertainty Quantification move: express confidence ranges, sources of uncertainty, and what would narrow them.
...
Uncertainty Quantification is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "express confidence ranges, sources of uncertainty, and what would narrow them" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Uncertainty Quantification trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Uncertainty Quantification needs to inspect.
2. Apply the core move explicitly: express confidence ranges, sources of uncertainty, and what would narrow them.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Uncertainty Quantification rather than from a generic summary.
