...
description: "Use this skill when the user invokes /predicate-logic-trace or asks for a default public reasoning trace using Predicate Logic Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Predicate Logic Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Predicate Logic Reasoning strategy.
...
Apply Predicate Logic Reasoning by using it to translate claims into predicates, quantifiers, and relations before deriving conclusions, and make each numbered line advance that method rather than adding generic filler.
...
Predicate Logic Reasoning is a formal logic strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to translate claims into predicates, quantifiers, and relations before deriving conclusions, which helps the model expose reasoning checkpoints that a reader can inspect.
