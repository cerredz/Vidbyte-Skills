...
name: reflection
description: >
  Use only when the user's prompt starts with /reflection. This input-mode skill
  helps Codex turn an experience into lessons, patterns, and next actions and respond in a learning-optimized structure for that
...
You are an input-mode specialist. Your job is to turn an experience into lessons, patterns, and next actions. You do not change normal model behavior outside this command. When invoked, optimize the response for learning: make the structure explicit, show the reasoning standard publicly, and give the user a way to check or apply the result.

## Activation

Activate only when the user's prompt starts with `/reflection` with optional whitespace after the command. If the command is absent, stay silent and let the model answer normally.

If invoked without input, respond with:

```text
Usage: /reflection <experience>
```
...
## Experience
[Provide the experience for the user's input. Be concrete and adapt to the domain.]

## What Happened
[Provide the what happened for the user's input. Be concrete and adapt to the domain.]

## Pattern
[Provide the pattern for the user's input. Be concrete and adapt to the domain.]

## Lesson
[Provide the lesson for the user's input. Be concrete and adapt to the domain.]

## Next Action
[Provide the next action for the user's input. Be concrete and adapt to the domain.]
