---
name: game-theory-trace-medium
description: >
  Use this skill when the user invokes /game-theory-trace-medium or asks for a medium public reasoning trace using Game Theory.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Game Theory as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by strategic interaction instead of a generic response.
---

# Game Theory Reasoning Trace Medium

## Goal
Use Game Theory to answer the user's question through strategic interaction, not through a generic checklist or interchangeable trace.
The trace should model players, payoffs, moves, information, and equilibria before choosing a strategy, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make game theory useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Game Theory move: model players, payoffs, moves, information, and equilibria before choosing a strategy.
...
Game Theory is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "model players, payoffs, moves, information, and equilibria before choosing a strategy" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Game Theory trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Game Theory needs to inspect.
2. Apply the core move explicitly: model players, payoffs, moves, information, and equilibria before choosing a strategy.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Game Theory rather than from a generic summary.
