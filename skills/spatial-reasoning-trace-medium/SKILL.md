---
name: spatial-reasoning-trace-medium
description: >
  Use this skill when the user invokes /spatial-reasoning-trace-medium or asks for a medium public reasoning trace using Spatial Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Spatial Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by geometric and layout logic instead of a generic response.
---

# Spatial Reasoning Reasoning Trace Medium

## Goal
Use Spatial Reasoning to answer the user's question through geometric and layout logic, not through a generic checklist or interchangeable trace.
The trace should reason about position, adjacency, distance, containment, and movement, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on premises, rules, cases, implications, contradictions, and validity checks, because those artifacts make spatial reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Spatial Reasoning move: reason about position, adjacency, distance, containment, and movement.
...
Spatial Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "reason about position, adjacency, distance, containment, and movement" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Spatial Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Spatial Reasoning needs to inspect.
2. Apply the core move explicitly: reason about position, adjacency, distance, containment, and movement.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Spatial Reasoning rather than from a generic summary.
