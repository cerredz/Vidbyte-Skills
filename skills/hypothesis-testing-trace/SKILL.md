...
description: "Use this skill when the user invokes /hypothesis-testing-trace or asks for a default public reasoning trace using Hypothesis Testing. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Hypothesis Testing Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Hypothesis Testing strategy.
...
Apply Hypothesis Testing by using it to state testable claims, define disconfirming evidence, and compare observations to expectations, and make each numbered line advance that method rather than adding generic filler.
...
Hypothesis Testing is a empirical evaluation strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to state testable claims, define disconfirming evidence, and compare observations to expectations, which helps the model expose reasoning checkpoints that a reader can inspect.
