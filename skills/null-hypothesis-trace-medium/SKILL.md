---
name: null-hypothesis-trace-medium
description: >
  Use this skill when the user invokes /null-hypothesis-trace-medium or asks for a medium public reasoning trace using Null Hypothesis Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Null Hypothesis Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by statistical inference instead of a generic response.
---

# Null Hypothesis Reasoning Reasoning Trace Medium

## Goal
Use Null Hypothesis Reasoning to answer the user's question through statistical inference, not through a generic checklist or interchangeable trace.
The trace should assume no effect or no difference, then ask whether evidence is strong enough to reject that baseline, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make null hypothesis reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Null Hypothesis Reasoning move: assume no effect or no difference, then ask whether evidence is strong enough to reject that baseline.
...
Null Hypothesis Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "assume no effect or no difference, then ask whether evidence is strong enough to reject that baseline" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Null Hypothesis Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Null Hypothesis Reasoning needs to inspect.
2. Apply the core move explicitly: assume no effect or no difference, then ask whether evidence is strong enough to reject that baseline.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Null Hypothesis Reasoning rather than from a generic summary.
