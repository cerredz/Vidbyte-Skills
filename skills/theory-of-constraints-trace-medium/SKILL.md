---
name: theory-of-constraints-trace-medium
description: >
  Use this skill when the user invokes /theory-of-constraints-trace-medium or asks for a medium public reasoning trace using Theory Of Constraints.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Theory Of Constraints as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by constraint analysis instead of a generic response.
---

# Theory Of Constraints Reasoning Trace Medium

## Goal
Use Theory Of Constraints to answer the user's question through constraint analysis, not through a generic checklist or interchangeable trace.
The trace should identify the system constraint, exploit it, subordinate other work, and reassess, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on boundaries, components, feedback loops, constraints, delays, and leverage points, because those artifacts make theory of constraints useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Theory Of Constraints move: identify the system constraint, exploit it, subordinate other work, and reassess.
...
Theory Of Constraints is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "identify the system constraint, exploit it, subordinate other work, and reassess" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Theory Of Constraints trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Theory Of Constraints needs to inspect.
2. Apply the core move explicitly: identify the system constraint, exploit it, subordinate other work, and reassess.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Theory Of Constraints rather than from a generic summary.
