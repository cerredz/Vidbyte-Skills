---
name: horizon-scanning-trace
description: "Use this skill when the user invokes /horizon-scanning-trace or asks for a default public reasoning trace using Horizon Scanning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Horizon Scanning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Horizon Scanning strategy.
...
Apply Horizon Scanning by using it to search for early signals, emerging patterns, and discontinuities that may matter later, and make each numbered line advance that method rather than adding generic filler.
...
Horizon Scanning is a weak signal analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to search for early signals, emerging patterns, and discontinuities that may matter later, which helps the model expose reasoning checkpoints that a reader can inspect.
