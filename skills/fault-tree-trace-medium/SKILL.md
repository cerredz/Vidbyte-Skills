...
Fault Tree Analysis is a failure analysis strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to work top-down from an undesirable event to combinations of contributing failures, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, fault tree analysis changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Fault Tree Analysis, not like a separate response pasted after the trace.
