---
name: fault-tree-trace-medium
description: >
  Use this skill when the user invokes /fault-tree-trace-medium or asks for a medium public reasoning trace using Fault Tree Analysis.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Fault Tree Analysis as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by failure analysis instead of a generic response.
---

# Fault Tree Analysis Reasoning Trace Medium

## Goal
Use Fault Tree Analysis to answer the user's question through failure analysis, not through a generic checklist or interchangeable trace.
The trace should work top-down from an undesirable event to combinations of contributing failures, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make fault tree analysis useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Fault Tree Analysis move: work top-down from an undesirable event to combinations of contributing failures.
...
Fault Tree Analysis is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "work top-down from an undesirable event to combinations of contributing failures" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Fault Tree Analysis trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Fault Tree Analysis needs to inspect.
2. Apply the core move explicitly: work top-down from an undesirable event to combinations of contributing failures.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Fault Tree Analysis rather than from a generic summary.
