---
name: reference-class-forecasting-trace-medium
description: >
  Use this skill when the user invokes /reference-class-forecasting-trace-medium or asks for a medium public reasoning trace using Reference Class Forecasting.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Reference Class Forecasting as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by forecast calibration instead of a generic response.
---

# Reference Class Forecasting Reasoning Trace Medium

## Goal
Use Reference Class Forecasting to answer the user's question through forecast calibration, not through a generic checklist or interchangeable trace.
The trace should select comparable cases, derive an empirical baseline, and adjust only with justified evidence, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make reference class forecasting useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Reference Class Forecasting move: select comparable cases, derive an empirical baseline, and adjust only with justified evidence.
...
Reference Class Forecasting is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "select comparable cases, derive an empirical baseline, and adjust only with justified evidence" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Reference Class Forecasting trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Reference Class Forecasting needs to inspect.
2. Apply the core move explicitly: select comparable cases, derive an empirical baseline, and adjust only with justified evidence.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Reference Class Forecasting rather than from a generic summary.
