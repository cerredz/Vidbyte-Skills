...
Reference Class Forecasting is a forecast calibration strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to select comparable cases, derive an empirical baseline, and adjust only with justified evidence, which forces the model to make the important reasoning moves visible.
This strategy is strongest when the question benefits from choosing under uncertainty, comparing options, or calibrating confidence and when the reader needs to see why the answer follows.
Compared with a generic scratchpad, reference class forecasting changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Reference Class Forecasting, not like a separate response pasted after the trace.
