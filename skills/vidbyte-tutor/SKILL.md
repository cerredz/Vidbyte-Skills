...
Invoke as `/define-success`. The skill blocks with a four-field template (WHAT, HOW, THRESHOLD, DEADLINE) and returns any field that fails the third-party evaluability test. Does not accept partial progress.
...
Invoke as `/no-conclusions`. The model uses only permitted vocabulary (observations, data, mechanisms, patterns) and never concludes, recommends, diagnoses, identifies, or evaluates. Does not relent when pushed. Surfaces contradictory data when the user's stated conclusion is demonstrably incorrect.
