---
name: value-stream-mapping-trace-medium
description: >
  Use this skill when the user invokes /value-stream-mapping-trace-medium or asks for a medium public reasoning trace using Value Stream Mapping.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Value Stream Mapping as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by mapping material and information flow to identify waste and improvement opportunities instead of a generic response.
---

# Value Stream Mapping Reasoning Trace Medium

## Goal
Use Value Stream Mapping to answer the user's question through mapping material and information flow to identify waste and improvement opportunities, not through a generic checklist or interchangeable trace.
The trace should map the flow of materials and information required to bring a product or service to the customer, identifying value-added and non-value-added steps, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make value stream mapping useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Value Stream Mapping move: map the flow of materials and information required to bring a product or service to the customer, identifying value-added and non-value-added steps.
...
Value Stream Mapping is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "map the flow of materials and information required to bring a product or service to the customer, identifying value-added and non-value-added steps" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Value Stream Mapping trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Value Stream Mapping needs to inspect.
2. Apply the core move explicitly: map the flow of materials and information required to bring a product or service to the customer, identifying value-added and non-value-added steps.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Value Stream Mapping rather than from a generic summary.
