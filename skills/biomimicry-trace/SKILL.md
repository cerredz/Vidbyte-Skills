...
description: "Use this skill when the user invokes /biomimicry-trace or asks for a default public reasoning trace using Biomimicry Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Biomimicry Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Biomimicry Reasoning strategy.
...
Apply Biomimicry Reasoning by using it to look to biological systems for patterns that can be adapted to the current challenge, and make each numbered line advance that method rather than adding generic filler.
...
Biomimicry Reasoning is a analogical design strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to look to biological systems for patterns that can be adapted to the current challenge, which helps the model expose reasoning checkpoints that a reader can inspect.
