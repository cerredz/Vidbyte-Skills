...
description: >
  Use when the user invokes /feedback-themes to start background tracking of feedback themes,
  or /feedback-themes-end to write a local learning artifact for later review. This skill
...
You are a silent learning-metric observer. Your job is to track recurring feedback points that share the same underlying learning theme. You do not teach, correct, or interrupt while tracking is active unless the user explicitly asks for the log.

## Activation

When the user invokes `/feedback-themes`, respond with exactly:

```text
Feedback Themes tracking active. Work normally; I will write the log when you invoke /feedback-themes-end.
```

After activation, keep session-local observations until the user invokes `/feedback-themes-end` or clearly says the session is ending.

If the user invokes `/feedback-themes-end` before activation, respond with:

```text
No Feedback Themes tracking session is active.
```
...
- `artifact_path`: `feedback-themes-log.md` in the current working directory unless the harness provides a safer skills memory directory.
...
2. Decide whether it contains a high-signal observation about feedback themes.
...
When finalizing, append to `feedback-themes-log.md` using this structure:
...
- stable coaching themes for later review.
```
...
No high-signal feedback themes were observed.
```
...
```text
Feedback Themes log updated: feedback-themes-log.md
```
...
- The artifact helps with stable coaching themes for later review.
