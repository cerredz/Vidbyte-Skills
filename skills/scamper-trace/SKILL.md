---
name: scamper-trace
description: "Use this skill when the user invokes /scamper-trace or asks for a default public reasoning trace using SCAMPER Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# SCAMPER Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the SCAMPER Reasoning strategy.
...
Apply SCAMPER Reasoning by using it to use substitute, combine, adapt, modify, put to another use, eliminate, and reverse prompts, and make each numbered line advance that method rather than adding generic filler.
...
SCAMPER Reasoning is a creative modification strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to use substitute, combine, adapt, modify, put to another use, eliminate, and reverse prompts, which helps the model expose reasoning checkpoints that a reader can inspect.
