...
description: "Use this skill when the user invokes /design-thinking-trace or asks for a default public reasoning trace using Design Thinking. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Design Thinking Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Design Thinking strategy.
...
Apply Design Thinking by using it to empathize, define, ideate, prototype, and test around user needs, and make each numbered line advance that method rather than adding generic filler.
...
Design Thinking is a human-centered design strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to empathize, define, ideate, prototype, and test around user needs, which helps the model expose reasoning checkpoints that a reader can inspect.
