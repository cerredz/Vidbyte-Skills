---
name: propositional-logic-trace-medium
description: >
  Use this skill when the user invokes /propositional-logic-trace-medium or asks for a medium public reasoning trace using Propositional Logic Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Propositional Logic Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by formal logic instead of a generic response.
---

# Propositional Logic Reasoning Reasoning Trace Medium

## Goal
Use Propositional Logic Reasoning to answer the user's question through formal logic, not through a generic checklist or interchangeable trace.
The trace should represent claims as propositions and test implications, conjunctions, disjunctions, and negations, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on premises, rules, cases, implications, contradictions, and validity checks, because those artifacts make propositional logic reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Propositional Logic Reasoning move: represent claims as propositions and test implications, conjunctions, disjunctions, and negations.
...
Propositional Logic Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "represent claims as propositions and test implications, conjunctions, disjunctions, and negations" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Propositional Logic Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Propositional Logic Reasoning needs to inspect.
2. Apply the core move explicitly: represent claims as propositions and test implications, conjunctions, disjunctions, and negations.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Propositional Logic Reasoning rather than from a generic summary.
