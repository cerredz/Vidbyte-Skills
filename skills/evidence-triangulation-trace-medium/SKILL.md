---
name: evidence-triangulation-trace-medium
description: >
  Use this skill when the user invokes /evidence-triangulation-trace-medium or asks for a medium public reasoning trace using Evidence Triangulation.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Evidence Triangulation as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by evidence synthesis instead of a generic response.
---

# Evidence Triangulation Reasoning Trace Medium

## Goal
Use Evidence Triangulation to answer the user's question through evidence synthesis, not through a generic checklist or interchangeable trace.
The trace should combine independent evidence streams and compare whether they converge, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make evidence triangulation useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Evidence Triangulation move: combine independent evidence streams and compare whether they converge.
...
Evidence Triangulation is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "combine independent evidence streams and compare whether they converge" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Evidence Triangulation trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Evidence Triangulation needs to inspect.
2. Apply the core move explicitly: combine independent evidence streams and compare whether they converge.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Evidence Triangulation rather than from a generic summary.
