...
description: "Use this skill when the user invokes /provocation-trace or asks for a default public reasoning trace using Provocation Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Provocation Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Provocation Reasoning strategy.
...
Apply Provocation Reasoning by using it to make a deliberate unreasonable statement and mine it for practical insight, and make each numbered line advance that method rather than adding generic filler.
...
Provocation Reasoning is a lateral thinking strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to make a deliberate unreasonable statement and mine it for practical insight, which helps the model expose reasoning checkpoints that a reader can inspect.
