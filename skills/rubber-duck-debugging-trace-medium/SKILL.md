---
name: rubber-duck-debugging-trace-medium
description: >
  Use this skill when the user invokes /rubber-duck-debugging-trace-medium or asks for a medium public reasoning trace using Rubber Duck Debugging.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Rubber Duck Debugging as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by explaining a problem aloud step-by-step to an inanimate listener to discover the solution instead of a generic response.
---

# Rubber Duck Debugging Reasoning Trace Medium

## Goal
Use Rubber Duck Debugging to answer the user's question through explaining a problem aloud step-by-step to an inanimate listener to discover the solution, not through a generic checklist or interchangeable trace.
The trace should explain a problem aloud step-by-step to an inanimate listener until the act of articulation reveals the flaw or solution, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make rubber duck debugging useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Rubber Duck Debugging move: explain a problem aloud step-by-step to an inanimate listener until the act of articulation reveals the flaw or solution.
...
Rubber Duck Debugging is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "explain a problem aloud step-by-step to an inanimate listener until the act of articulation reveals the flaw or solution" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Rubber Duck Debugging trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Rubber Duck Debugging needs to inspect.
2. Apply the core move explicitly: explain a problem aloud step-by-step to an inanimate listener until the act of articulation reveals the flaw or solution.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Rubber Duck Debugging rather than from a generic summary.
