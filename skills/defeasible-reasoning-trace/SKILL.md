...
description: "Use this skill when the user invokes /defeasible-reasoning-trace or asks for a default public reasoning trace using Defeasible Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Defeasible Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Defeasible Reasoning strategy.
...
Apply Defeasible Reasoning by using it to make provisional conclusions that can be withdrawn when stronger contrary evidence appears, and make each numbered line advance that method rather than adding generic filler.
...
Defeasible Reasoning is a revisable logic strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to make provisional conclusions that can be withdrawn when stronger contrary evidence appears, which helps the model expose reasoning checkpoints that a reader can inspect.
