...
description: "Use this skill when the user invokes /second-order-effects-trace or asks for a default public reasoning trace using Second Order Effects. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Second Order Effects Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Second Order Effects strategy.
...
Apply Second Order Effects by using it to reason beyond immediate outcomes into follow-on effects and feedback, and make each numbered line advance that method rather than adding generic filler.
...
Second Order Effects is a consequence analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to reason beyond immediate outcomes into follow-on effects and feedback, which helps the model expose reasoning checkpoints that a reader can inspect.
