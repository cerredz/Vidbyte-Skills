---
name: causal-trace
description: "Use this skill when the user invokes /causal-trace or asks for a default public reasoning trace using Causal Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Causal Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Causal Reasoning strategy.
...
Apply Causal Reasoning by using it to separate causes from correlations, map mechanisms, and test whether interventions would change outcomes, and make each numbered line advance that method rather than adding generic filler.
...
Causal Reasoning is a cause and effect analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to separate causes from correlations, map mechanisms, and test whether interventions would change outcomes, which helps the model expose reasoning checkpoints that a reader can inspect.
