---
name: storyboarding-trace-medium
description: >
  Use this skill when the user invokes /storyboarding-trace-medium or asks for a medium public reasoning trace using Storyboarding.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Storyboarding as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by sequencing visual frames to explore a narrative or user experience before building instead of a generic response.
---

# Storyboarding Reasoning Trace Medium

## Goal
Use Storyboarding to answer the user's question through sequencing visual frames to explore a narrative or user experience before building, not through a generic checklist or interchangeable trace.
The trace should sequence visual frames to explore a narrative, process, or user experience step by step before committing resources to implementation, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make storyboarding useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Storyboarding move: sequence visual frames to explore a narrative, process, or user experience step by step before committing resources to implementation.
...
Storyboarding is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "sequence visual frames to explore a narrative, process, or user experience step by step before committing resources to implementation" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Storyboarding trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Storyboarding needs to inspect.
2. Apply the core move explicitly: sequence visual frames to explore a narrative, process, or user experience step by step before committing resources to implementation.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Storyboarding rather than from a generic summary.
