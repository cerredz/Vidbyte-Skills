...
description: "Use this skill when the user invokes /feedback-loop-trace or asks for a default public reasoning trace using Feedback Loop Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Feedback Loop Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Feedback Loop Reasoning strategy.
...
Apply Feedback Loop Reasoning by using it to identify reinforcing and balancing loops, then trace their effects over time, and make each numbered line advance that method rather than adding generic filler.
...
Feedback Loop Reasoning is a systems analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to identify reinforcing and balancing loops, then trace their effects over time, which helps the model expose reasoning checkpoints that a reader can inspect.
