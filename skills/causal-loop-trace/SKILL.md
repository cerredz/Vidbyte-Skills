...
description: "Use this skill when the user invokes /causal-loop-trace or asks for a default public reasoning trace using Causal Loop Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Causal Loop Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Causal Loop Reasoning strategy.
...
Apply Causal Loop Reasoning by using it to represent reinforcing and balancing loops that drive the behavior of the system, and make each numbered line advance that method rather than adding generic filler.
...
Causal Loop Reasoning is a systems dynamics strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to represent reinforcing and balancing loops that drive the behavior of the system, which helps the model expose reasoning checkpoints that a reader can inspect.
