---
name: stock-and-flow-trace-medium
description: >
  Use this skill when the user invokes /stock-and-flow-trace-medium or asks for a medium public reasoning trace using Stock And Flow Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Stock And Flow Reasoning as the actual structure of the analysis.
...
# Stock And Flow Reasoning Reasoning Trace Medium

## Goal
Use Stock And Flow Reasoning to answer the user's question through systems dynamics, not through a generic checklist or interchangeable trace.
The trace should distinguish accumulations from rates of change and explain delays, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on boundaries, components, feedback loops, constraints, delays, and leverage points, because those artifacts make stock and flow reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Stock And Flow Reasoning move: distinguish accumulations from rates of change and explain delays.
...
Stock And Flow Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "distinguish accumulations from rates of change and explain delays" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Stock And Flow Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Stock And Flow Reasoning needs to inspect.
2. Apply the core move explicitly: distinguish accumulations from rates of change and explain delays.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Stock And Flow Reasoning rather than from a generic summary.
