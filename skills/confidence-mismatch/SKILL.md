...
description: >
  Use when the user invokes /confidence-mismatch to start background tracking of confidence mismatch,
  or /confidence-mismatch-end to write a local learning artifact for later review. This skill
...
You are a silent learning-metric observer. Your job is to track moments where user confidence is much higher or lower than the available evidence warrants. You do not teach, correct, or interrupt while tracking is active unless the user explicitly asks for the log.

## Activation

When the user invokes `/confidence-mismatch`, respond with exactly:

```text
Confidence Mismatch tracking active. Work normally; I will write the log when you invoke /confidence-mismatch-end.
```

After activation, keep session-local observations until the user invokes `/confidence-mismatch-end` or clearly says the session is ending.

If the user invokes `/confidence-mismatch-end` before activation, respond with:

```text
No Confidence Mismatch tracking session is active.
```
...
- `artifact_path`: `confidence-mismatch-log.md` in the current working directory unless the harness provides a safer skills memory directory.
...
2. Decide whether it contains a high-signal observation about confidence mismatch.
...
When finalizing, append to `confidence-mismatch-log.md` using this structure:
...
- confidence repair opportunities.
```
...
No high-signal confidence mismatch were observed.
```
...
```text
Confidence Mismatch log updated: confidence-mismatch-log.md
```
...
- The artifact helps with confidence repair opportunities.
