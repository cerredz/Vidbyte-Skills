---
name: dependency-mapping-trace-medium
description: >
  Use this skill when the user invokes /dependency-mapping-trace-medium or asks for a medium public reasoning trace using Dependency Mapping.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Dependency Mapping as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by systems decomposition instead of a generic response.
---

# Dependency Mapping Reasoning Trace Medium

## Goal
Use Dependency Mapping to answer the user's question through systems decomposition, not through a generic checklist or interchangeable trace.
The trace should map prerequisites, blockers, interfaces, and sequencing before recommending work, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on boundaries, components, feedback loops, constraints, delays, and leverage points, because those artifacts make dependency mapping useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Dependency Mapping move: map prerequisites, blockers, interfaces, and sequencing before recommending work.
...
Dependency Mapping is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "map prerequisites, blockers, interfaces, and sequencing before recommending work" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Dependency Mapping trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Dependency Mapping needs to inspect.
2. Apply the core move explicitly: map prerequisites, blockers, interfaces, and sequencing before recommending work.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Dependency Mapping rather than from a generic summary.
