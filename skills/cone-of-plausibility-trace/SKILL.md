...
description: "Use this skill when the user invokes /cone-of-plausibility-trace or asks for a default public reasoning trace using Cone Of Plausibility. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Cone Of Plausibility Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Cone Of Plausibility strategy.
...
Apply Cone Of Plausibility by using it to separate probable, plausible, and possible futures so confidence stays calibrated, and make each numbered line advance that method rather than adding generic filler.
...
Cone Of Plausibility is a forecast framing strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to separate probable, plausible, and possible futures so confidence stays calibrated, which helps the model expose reasoning checkpoints that a reader can inspect.
