---
name: biomimicry-trace-medium
description: >
  Use this skill when the user invokes /biomimicry-trace-medium or asks for a medium public reasoning trace using Biomimicry Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Biomimicry Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by analogical design instead of a generic response.
---

# Biomimicry Reasoning Reasoning Trace Medium

## Goal
Use Biomimicry Reasoning to answer the user's question through analogical design, not through a generic checklist or interchangeable trace.
The trace should look to biological systems for patterns that can be adapted to the current challenge, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on frames, constraints, provocations, alternatives, combinations, and prototypeable ideas, because those artifacts make biomimicry reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Biomimicry Reasoning move: look to biological systems for patterns that can be adapted to the current challenge.
...
Biomimicry Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "look to biological systems for patterns that can be adapted to the current challenge" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Biomimicry Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Biomimicry Reasoning needs to inspect.
2. Apply the core move explicitly: look to biological systems for patterns that can be adapted to the current challenge.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Biomimicry Reasoning rather than from a generic summary.
