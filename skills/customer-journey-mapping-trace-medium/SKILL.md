---
name: customer-journey-mapping-trace-medium
description: >
  Use this skill when the user invokes /customer-journey-mapping-trace-medium or asks for a medium public reasoning trace using Customer Journey Mapping.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Customer Journey Mapping as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by visualizing end-to-end customer experience across all touchpoints instead of a generic response.
---

# Customer Journey Mapping Reasoning Trace Medium

## Goal
Use Customer Journey Mapping to answer the user's question through visualizing end-to-end customer experience across all touchpoints, not through a generic checklist or interchangeable trace.
The trace should map the end-to-end customer experience across all touchpoints, capturing actions, thoughts, emotions, and pain points at each stage, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make customer journey mapping useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Customer Journey Mapping move: map the end-to-end customer experience across all touchpoints, capturing actions, thoughts, emotions, and pain points at each stage.
...
Customer Journey Mapping is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "map the end-to-end customer experience across all touchpoints, capturing actions, thoughts, emotions, and pain points at each stage" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Customer Journey Mapping trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Customer Journey Mapping needs to inspect.
2. Apply the core move explicitly: map the end-to-end customer experience across all touchpoints, capturing actions, thoughts, emotions, and pain points at each stage.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Customer Journey Mapping rather than from a generic summary.
