...
description: "Use this skill when the user invokes /what-if-analysis-trace or asks for a default public reasoning trace using What If Analysis. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# What If Analysis Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the What If Analysis strategy.
...
Apply What If Analysis by using it to alter one important condition at a time and track how the answer changes, and make each numbered line advance that method rather than adding generic filler.
...
What If Analysis is a scenario stress testing strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to alter one important condition at a time and track how the answer changes, which helps the model expose reasoning checkpoints that a reader can inspect.
