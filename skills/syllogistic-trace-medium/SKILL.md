---
name: syllogistic-trace-medium
description: >
  Use this skill when the user invokes /syllogistic-trace-medium or asks for a medium public reasoning trace using Syllogistic Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Syllogistic Reasoning as the actual structure of the analysis.
...
# Syllogistic Reasoning Reasoning Trace Medium

## Goal
Use Syllogistic Reasoning to answer the user's question through formal logic, not through a generic checklist or interchangeable trace.
The trace should evaluate categorical premises and derive whether the conclusion follows, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on premises, rules, cases, implications, contradictions, and validity checks, because those artifacts make syllogistic reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Syllogistic Reasoning move: evaluate categorical premises and derive whether the conclusion follows.
...
Syllogistic Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "evaluate categorical premises and derive whether the conclusion follows" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Syllogistic Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Syllogistic Reasoning needs to inspect.
2. Apply the core move explicitly: evaluate categorical premises and derive whether the conclusion follows.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Syllogistic Reasoning rather than from a generic summary.
