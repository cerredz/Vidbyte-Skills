---
name: abductive-trace-medium
description: >
  Use this skill when the user invokes /abductive-trace-medium or asks for a medium public reasoning trace using Abductive Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Abductive Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by inference to best explanation instead of a generic response.
---

# Abductive Reasoning Reasoning Trace Medium

## Goal
Use Abductive Reasoning to answer the user's question through inference to best explanation, not through a generic checklist or interchangeable trace.
The trace should generate plausible explanations, compare explanatory power, and choose the best supported account, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make abductive reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Abductive Reasoning move: generate plausible explanations, compare explanatory power, and choose the best supported account.
...
Abductive Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "generate plausible explanations, compare explanatory power, and choose the best supported account" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Abductive Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Abductive Reasoning needs to inspect.
2. Apply the core move explicitly: generate plausible explanations, compare explanatory power, and choose the best supported account.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Abductive Reasoning rather than from a generic summary.
