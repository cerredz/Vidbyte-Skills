...
description: "Use this skill when the user invokes /hermeneutic-trace or asks for a default public reasoning trace using Hermeneutic Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Hermeneutic Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Hermeneutic Reasoning strategy.
...
Apply Hermeneutic Reasoning by using it to move between parts and whole to refine interpretation of a text, event, or artifact, and make each numbered line advance that method rather than adding generic filler.
...
Hermeneutic Reasoning is a interpretive analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to move between parts and whole to refine interpretation of a text, event, or artifact, which helps the model expose reasoning checkpoints that a reader can inspect.
