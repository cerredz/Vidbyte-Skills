---
name: causal-loop-trace-medium
description: >
  Use this skill when the user invokes /causal-loop-trace-medium or asks for a medium public reasoning trace using Causal Loop Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Causal Loop Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by systems dynamics instead of a generic response.
---

# Causal Loop Reasoning Reasoning Trace Medium

## Goal
Use Causal Loop Reasoning to answer the user's question through systems dynamics, not through a generic checklist or interchangeable trace.
The trace should represent reinforcing and balancing loops that drive the behavior of the system, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on boundaries, components, feedback loops, constraints, delays, and leverage points, because those artifacts make causal loop reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Causal Loop Reasoning move: represent reinforcing and balancing loops that drive the behavior of the system.
...
Causal Loop Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "represent reinforcing and balancing loops that drive the behavior of the system" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Causal Loop Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Causal Loop Reasoning needs to inspect.
2. Apply the core move explicitly: represent reinforcing and balancing loops that drive the behavior of the system.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Causal Loop Reasoning rather than from a generic summary.
