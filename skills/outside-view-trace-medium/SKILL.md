---
name: outside-view-trace-medium
description: >
  Use this skill when the user invokes /outside-view-trace-medium or asks for a medium public reasoning trace using Outside View.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Outside View as the actual structure of the analysis.
...
# Outside View Reasoning Trace Medium

## Goal
Use Outside View to answer the user's question through forecast calibration, not through a generic checklist or interchangeable trace.
The trace should compare the case to a reference class before trusting inside details, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make outside view useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Outside View move: compare the case to a reference class before trusting inside details.
...
Outside View is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "compare the case to a reference class before trusting inside details" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Outside View trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Outside View needs to inspect.
2. Apply the core move explicitly: compare the case to a reference class before trusting inside details.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Outside View rather than from a generic summary.
