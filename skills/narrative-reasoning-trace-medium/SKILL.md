---
name: narrative-reasoning-trace-medium
description: >
  Use this skill when the user invokes /narrative-reasoning-trace-medium or asks for a medium public reasoning trace using Narrative Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Narrative Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by story and causality analysis instead of a generic response.
---

# Narrative Reasoning Reasoning Trace Medium

## Goal
Use Narrative Reasoning to answer the user's question through story and causality analysis, not through a generic checklist or interchangeable trace.
The trace should trace actors, motives, conflicts, turning points, and implied causal arcs, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on causes, mechanisms, counterfactual tests, evidence quality, and intervention points, because those artifacts make narrative reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Narrative Reasoning move: trace actors, motives, conflicts, turning points, and implied causal arcs.
...
Narrative Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "trace actors, motives, conflicts, turning points, and implied causal arcs" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Narrative Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Narrative Reasoning needs to inspect.
2. Apply the core move explicitly: trace actors, motives, conflicts, turning points, and implied causal arcs.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Narrative Reasoning rather than from a generic summary.
