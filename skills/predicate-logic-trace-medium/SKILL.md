---
name: predicate-logic-trace-medium
description: >
  Use this skill when the user invokes /predicate-logic-trace-medium or asks for a medium public reasoning trace using Predicate Logic Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Predicate Logic Reasoning as the actual structure of the analysis.
...
# Predicate Logic Reasoning Reasoning Trace Medium

## Goal
Use Predicate Logic Reasoning to answer the user's question through formal logic, not through a generic checklist or interchangeable trace.
The trace should translate claims into predicates, quantifiers, and relations before deriving conclusions, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on premises, rules, cases, implications, contradictions, and validity checks, because those artifacts make predicate logic reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Predicate Logic Reasoning move: translate claims into predicates, quantifiers, and relations before deriving conclusions.
...
Predicate Logic Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "translate claims into predicates, quantifiers, and relations before deriving conclusions" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Predicate Logic Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Predicate Logic Reasoning needs to inspect.
2. Apply the core move explicitly: translate claims into predicates, quantifiers, and relations before deriving conclusions.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Predicate Logic Reasoning rather than from a generic summary.
