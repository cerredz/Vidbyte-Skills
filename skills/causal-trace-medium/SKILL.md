---
name: causal-trace-medium
description: >
  Use this skill when the user invokes /causal-trace-medium or asks for a medium public reasoning trace using Causal Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Causal Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by cause and effect analysis instead of a generic response.
---

# Causal Reasoning Reasoning Trace Medium

## Goal
Use Causal Reasoning to answer the user's question through cause and effect analysis, not through a generic checklist or interchangeable trace.
The trace should separate causes from correlations, map mechanisms, and test whether interventions would change outcomes, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make causal reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Causal Reasoning move: separate causes from correlations, map mechanisms, and test whether interventions would change outcomes.
...
Causal Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "separate causes from correlations, map mechanisms, and test whether interventions would change outcomes" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Causal Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Causal Reasoning needs to inspect.
2. Apply the core move explicitly: separate causes from correlations, map mechanisms, and test whether interventions would change outcomes.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Causal Reasoning rather than from a generic summary.
