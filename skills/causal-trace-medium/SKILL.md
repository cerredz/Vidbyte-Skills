...
Causal Reasoning is a cause and effect analysis strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to separate causes from correlations, map mechanisms, and test whether interventions would change outcomes, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, causal reasoning changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Causal Reasoning, not like a separate response pasted after the trace.
