...
description: "Use this skill when the user invokes /fuzzy-logic-trace or asks for a default public reasoning trace using Fuzzy Logic Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Fuzzy Logic Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Fuzzy Logic Reasoning strategy.
...
Apply Fuzzy Logic Reasoning by using it to use degrees of membership when categories are not cleanly true or false, and make each numbered line advance that method rather than adding generic filler.
...
Fuzzy Logic Reasoning is a graded truth analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to use degrees of membership when categories are not cleanly true or false, which helps the model expose reasoning checkpoints that a reader can inspect.
