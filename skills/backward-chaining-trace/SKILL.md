...
description: "Use this skill when the user invokes /backward-chaining-trace or asks for a default public reasoning trace using Backward Chaining. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Backward Chaining Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Backward Chaining strategy.
...
Apply Backward Chaining by using it to start from the desired conclusion and work backward to required premises or actions, and make each numbered line advance that method rather than adding generic filler.
...
Backward Chaining is a goal-driven inference strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to start from the desired conclusion and work backward to required premises or actions, which helps the model expose reasoning checkpoints that a reader can inspect.
