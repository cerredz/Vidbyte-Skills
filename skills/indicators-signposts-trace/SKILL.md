...
description: "Use this skill when the user invokes /indicators-signposts-trace or asks for a default public reasoning trace using Indicators And Signposts. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Indicators And Signposts Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Indicators And Signposts strategy.
...
Apply Indicators And Signposts by using it to define observable signals that would confirm, weaken, or redirect the current assessment, and make each numbered line advance that method rather than adding generic filler.
...
Indicators And Signposts is a monitoring analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to define observable signals that would confirm, weaken, or redirect the current assessment, which helps the model expose reasoning checkpoints that a reader can inspect.
