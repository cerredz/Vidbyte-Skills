---
name: pareto-principle-trace-medium
description: >
  Use this skill when the user invokes /pareto-principle-trace-medium or asks for a medium public reasoning trace using Pareto Principle.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Pareto Principle as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by focusing on the vital few causes that produce the majority of effects instead of a generic response.
---

# Pareto Principle Reasoning Trace Medium

## Goal
Use Pareto Principle to answer the user's question through focusing on the vital few causes that produce the majority of effects, not through a generic checklist or interchangeable trace.
The trace should identify the vital few inputs that produce the majority of outputs and focus analysis there, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make pareto principle useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Pareto Principle move: identify the vital few inputs that produce the majority of outputs and focus analysis there.
...
Pareto Principle is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "identify the vital few inputs that produce the majority of outputs and focus analysis there" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Pareto Principle trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Pareto Principle needs to inspect.
2. Apply the core move explicitly: identify the vital few inputs that produce the majority of outputs and focus analysis there.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Pareto Principle rather than from a generic summary.
