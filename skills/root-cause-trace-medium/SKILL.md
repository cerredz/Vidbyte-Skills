---
name: root-cause-trace-medium
description: >
  Use this skill when the user invokes /root-cause-trace-medium or asks for a medium public reasoning trace using Root Cause Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Root Cause Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by cause analysis instead of a generic response.
---

# Root Cause Reasoning Reasoning Trace Medium

## Goal
Use Root Cause Reasoning to answer the user's question through cause analysis, not through a generic checklist or interchangeable trace.
The trace should distinguish proximate symptoms from durable causes that can be corrected, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make root cause reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Root Cause Reasoning move: distinguish proximate symptoms from durable causes that can be corrected.
...
Root Cause Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "distinguish proximate symptoms from durable causes that can be corrected" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Root Cause Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Root Cause Reasoning needs to inspect.
2. Apply the core move explicitly: distinguish proximate symptoms from durable causes that can be corrected.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Root Cause Reasoning rather than from a generic summary.
