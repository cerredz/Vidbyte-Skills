...
Root Cause Reasoning is a cause analysis strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to distinguish proximate symptoms from durable causes that can be corrected, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, root cause reasoning changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Root Cause Reasoning, not like a separate response pasted after the trace.
