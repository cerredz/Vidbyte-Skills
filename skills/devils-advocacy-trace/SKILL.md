...
description: "Use this skill when the user invokes /devils-advocacy-trace or asks for a default public reasoning trace using Devils Advocacy. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Devils Advocacy Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Devils Advocacy strategy.
...
Apply Devils Advocacy by using it to argue against the favored answer to expose unsupported claims and hidden vulnerabilities, and make each numbered line advance that method rather than adding generic filler.
...
Devils Advocacy is a adversarial critique strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to argue against the favored answer to expose unsupported claims and hidden vulnerabilities, which helps the model expose reasoning checkpoints that a reader can inspect.
