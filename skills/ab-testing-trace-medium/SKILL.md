---
name: ab-testing-trace-medium
description: >
  Use this skill when the user invokes /ab-testing-trace-medium or asks for a medium public reasoning trace using A B Testing.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses A B Testing as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by experimental decision analysis instead of a generic response.
---

# A B Testing Reasoning Trace Medium

## Goal
Use A B Testing to answer the user's question through experimental decision analysis, not through a generic checklist or interchangeable trace.
The trace should compare variants with predefined metrics, sample rules, and decision thresholds, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make a b testing useful rather than decorative.
...
Build the scratchpad by repeatedly applying the A B Testing move: compare variants with predefined metrics, sample rules, and decision thresholds.
...
A B Testing is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "compare variants with predefined metrics, sample rules, and decision thresholds" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong A B Testing trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that A B Testing needs to inspect.
2. Apply the core move explicitly: compare variants with predefined metrics, sample rules, and decision thresholds.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from A B Testing rather than from a generic summary.
