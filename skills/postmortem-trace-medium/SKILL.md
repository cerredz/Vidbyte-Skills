---
name: postmortem-trace-medium
description: >
  Use this skill when the user invokes /postmortem-trace-medium or asks for a medium public reasoning trace using Postmortem Reasoning.
...
# Postmortem Reasoning Reasoning Trace Medium
...
Postmortem Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "reconstruct what happened, separate symptoms from causes, and extract future safeguards" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Postmortem Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Postmortem Reasoning needs to inspect.
2. Apply the core move explicitly: reconstruct what happened, separate symptoms from causes, and extract future safeguards.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Postmortem Reasoning rather than from a generic summary.
