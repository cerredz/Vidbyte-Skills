...
description: "Use this skill when the user invokes /base-rate-trace or asks for a default public reasoning trace using Base Rate Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Base Rate Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Base Rate Reasoning strategy.
...
Apply Base Rate Reasoning by using it to start from reference frequencies before adjusting for case-specific evidence, and make each numbered line advance that method rather than adding generic filler.
...
Base Rate Reasoning is a forecast calibration strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to start from reference frequencies before adjusting for case-specific evidence, which helps the model expose reasoning checkpoints that a reader can inspect.
