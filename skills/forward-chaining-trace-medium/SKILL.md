---
name: forward-chaining-trace-medium
description: >
  Use this skill when the user invokes /forward-chaining-trace-medium or asks for a medium public reasoning trace using Forward Chaining.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Forward Chaining as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by data-driven inference instead of a generic response.
---

# Forward Chaining Reasoning Trace Medium

## Goal
Use Forward Chaining to answer the user's question through data-driven inference, not through a generic checklist or interchangeable trace.
The trace should start from known facts and apply rules until useful conclusions emerge, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make forward chaining useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Forward Chaining move: start from known facts and apply rules until useful conclusions emerge.
...
Forward Chaining is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "start from known facts and apply rules until useful conclusions emerge" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Forward Chaining trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Forward Chaining needs to inspect.
2. Apply the core move explicitly: start from known facts and apply rules until useful conclusions emerge.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Forward Chaining rather than from a generic summary.
