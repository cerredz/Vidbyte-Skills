---
name: scarcity-heuristic-trace-medium
description: >
  Use this skill when the user invokes /scarcity-heuristic-trace-medium or asks for a medium public reasoning trace using Scarcity Heuristic.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Scarcity Heuristic as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by assigning greater value to items or opportunities perceived as limited instead of a generic response.
---

# Scarcity Heuristic Reasoning Trace Medium

## Goal
Use Scarcity Heuristic to answer the user's question through assigning greater value to items or opportunities perceived as limited, not through a generic checklist or interchangeable trace.
The trace should assign greater value to items or opportunities perceived as limited or rare, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make scarcity heuristic useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Scarcity Heuristic move: assign greater value to items or opportunities perceived as limited or rare.
...
Scarcity Heuristic is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "assign greater value to items or opportunities perceived as limited or rare" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Scarcity Heuristic trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Scarcity Heuristic needs to inspect.
2. Apply the core move explicitly: assign greater value to items or opportunities perceived as limited or rare.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Scarcity Heuristic rather than from a generic summary.
