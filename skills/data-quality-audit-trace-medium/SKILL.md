---
name: data-quality-audit-trace-medium
description: >
  Use this skill when the user invokes /data-quality-audit-trace-medium or asks for a medium public reasoning trace using Data Quality Audit.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Data Quality Audit as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by evidence reliability instead of a generic response.
---

# Data Quality Audit Reasoning Trace Medium

## Goal
Use Data Quality Audit to answer the user's question through evidence reliability, not through a generic checklist or interchangeable trace.
The trace should inspect completeness, accuracy, freshness, lineage, and measurement bias, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make data quality audit useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Data Quality Audit move: inspect completeness, accuracy, freshness, lineage, and measurement bias.
...
Data Quality Audit is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "inspect completeness, accuracy, freshness, lineage, and measurement bias" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Data Quality Audit trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Data Quality Audit needs to inspect.
2. Apply the core move explicitly: inspect completeness, accuracy, freshness, lineage, and measurement bias.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Data Quality Audit rather than from a generic summary.
