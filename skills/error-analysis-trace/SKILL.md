...
description: "Use this skill when the user invokes /error-analysis-trace or asks for a default public reasoning trace using Error Analysis. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Error Analysis Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Error Analysis strategy.
...
Apply Error Analysis by using it to categorize failures, estimate frequency, and identify the changes most likely to reduce them, and make each numbered line advance that method rather than adding generic filler.
...
Error Analysis is a diagnostic evaluation strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to categorize failures, estimate frequency, and identify the changes most likely to reduce them, which helps the model expose reasoning checkpoints that a reader can inspect.
