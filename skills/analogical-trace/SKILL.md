---
name: analogical-trace
description: "Use this skill when the user invokes /analogical-trace or asks for a default public reasoning trace using Analogical Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Analogical Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Analogical Reasoning strategy.
...
Apply Analogical Reasoning by using it to map a source case to the target case, test relevant similarities, and reject weak transfers, and make each numbered line advance that method rather than adding generic filler.
...
Analogical Reasoning is a similarity transfer strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to map a source case to the target case, test relevant similarities, and reject weak transfers, which helps the model expose reasoning checkpoints that a reader can inspect.
