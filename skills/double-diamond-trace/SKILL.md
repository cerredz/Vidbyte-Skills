...
description: "Use this skill when the user invokes /double-diamond-trace or asks for a default public reasoning trace using Double Diamond. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Double Diamond Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Double Diamond strategy.
...
Apply Double Diamond by using it to diverge and converge first on the problem, then diverge and converge on the solution, and make each numbered line advance that method rather than adding generic filler.
...
Double Diamond is a design process strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to diverge and converge first on the problem, then diverge and converge on the solution, which helps the model expose reasoning checkpoints that a reader can inspect.
