---
name: tallying-trace-medium
description: >
  Use this skill when the user invokes /tallying-trace-medium or asks for a medium public reasoning trace using Tallying.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Tallying as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by counting favorable cues across alternatives and choosing the one with the most positives instead of a generic response.
---

# Tallying Reasoning Trace Medium

## Goal
Use Tallying to answer the user's question through counting favorable cues across alternatives and choosing the one with the most positives, not through a generic checklist or interchangeable trace.
The trace should count the number of favorable cues for each alternative and choose the one with the most positives, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make tallying useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Tallying move: count the number of favorable cues for each alternative and choose the one with the most positives.
...
Tallying is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "count the number of favorable cues for each alternative and choose the one with the most positives" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Tallying trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Tallying needs to inspect.
2. Apply the core move explicitly: count the number of favorable cues for each alternative and choose the one with the most positives.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Tallying rather than from a generic summary.
