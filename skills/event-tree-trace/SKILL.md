---
name: event-tree-trace
description: "Use this skill when the user invokes /event-tree-trace or asks for a default public reasoning trace using Event Tree Analysis. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Event Tree Analysis Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Event Tree Analysis strategy.
...
Apply Event Tree Analysis by using it to start from an initiating event and branch through possible outcomes and controls, and make each numbered line advance that method rather than adding generic filler.
...
Event Tree Analysis is a risk analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to start from an initiating event and branch through possible outcomes and controls, which helps the model expose reasoning checkpoints that a reader can inspect.
