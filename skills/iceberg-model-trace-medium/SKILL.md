---
name: iceberg-model-trace-medium
description: >
  Use this skill when the user invokes /iceberg-model-trace-medium or asks for a medium public reasoning trace using Iceberg Model.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Iceberg Model as the actual structure of the analysis.
...
# Iceberg Model Reasoning Trace Medium

## Goal
Use Iceberg Model to answer the user's question through systems analysis, not through a generic checklist or interchangeable trace.
The trace should move from visible events to patterns, structures, and mental models underneath, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on boundaries, components, feedback loops, constraints, delays, and leverage points, because those artifacts make iceberg model useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Iceberg Model move: move from visible events to patterns, structures, and mental models underneath.
...
Iceberg Model is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "move from visible events to patterns, structures, and mental models underneath" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Iceberg Model trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Iceberg Model needs to inspect.
2. Apply the core move explicitly: move from visible events to patterns, structures, and mental models underneath.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Iceberg Model rather than from a generic summary.
