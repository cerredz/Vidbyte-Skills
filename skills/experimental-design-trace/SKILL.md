...
description: "Use this skill when the user invokes /experimental-design-trace or asks for a default public reasoning trace using Experimental Design. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Experimental Design Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Experimental Design strategy.
...
Apply Experimental Design by using it to define variables, controls, measures, randomization, and validity risks, and make each numbered line advance that method rather than adding generic filler.
...
Experimental Design is a empirical inquiry strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to define variables, controls, measures, randomization, and validity risks, which helps the model expose reasoning checkpoints that a reader can inspect.
