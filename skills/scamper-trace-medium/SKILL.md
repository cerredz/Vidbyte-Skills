---
name: scamper-trace-medium
description: >
  Use this skill when the user invokes /scamper-trace-medium or asks for a medium public reasoning trace using SCAMPER Reasoning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses SCAMPER Reasoning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by creative modification instead of a generic response.
---

# SCAMPER Reasoning Reasoning Trace Medium

## Goal
Use SCAMPER Reasoning to answer the user's question through creative modification, not through a generic checklist or interchangeable trace.
The trace should use substitute, combine, adapt, modify, put to another use, eliminate, and reverse prompts, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on frames, constraints, provocations, alternatives, combinations, and prototypeable ideas, because those artifacts make scamper reasoning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the SCAMPER Reasoning move: use substitute, combine, adapt, modify, put to another use, eliminate, and reverse prompts.
...
SCAMPER Reasoning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "use substitute, combine, adapt, modify, put to another use, eliminate, and reverse prompts" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong SCAMPER Reasoning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that SCAMPER Reasoning needs to inspect.
2. Apply the core move explicitly: use substitute, combine, adapt, modify, put to another use, eliminate, and reverse prompts.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from SCAMPER Reasoning rather than from a generic summary.
