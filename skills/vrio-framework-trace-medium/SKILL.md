---
name: vrio-framework-trace-medium
description: >
  Use this skill when the user invokes /vrio-framework-trace-medium or asks for a medium public reasoning trace using VRIO Framework.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses VRIO Framework as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by evaluating resources by Value, Rarity, Imitability, and Organization instead of a generic response.
---

# VRIO Framework Reasoning Trace Medium

## Goal
Use VRIO Framework to answer the user's question through evaluating resources by Value, Rarity, Imitability, and Organization, not through a generic checklist or interchangeable trace.
The trace should evaluate each resource or capability across four dimensions: is it valuable, rare, costly to imitate, and is the organization structured to capture its value, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make vrio framework useful rather than decorative.
...
Build the scratchpad by repeatedly applying the VRIO Framework move: evaluate each resource or capability across four dimensions: is it valuable, rare, costly to imitate, and is the organization structured to capture its value.
...
VRIO Framework is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "evaluate each resource or capability across four dimensions: is it valuable, rare, costly to imitate, and is the organization structured to capture its value" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong VRIO Framework trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that VRIO Framework needs to inspect.
2. Apply the core move explicitly: evaluate each resource or capability across four dimensions: is it valuable, rare, costly to imitate, and is the organization structured to capture its value.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from VRIO Framework rather than from a generic summary.
