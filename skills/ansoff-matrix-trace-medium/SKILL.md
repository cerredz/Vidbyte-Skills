---
name: ansoff-matrix-trace-medium
description: >
  Use this skill when the user invokes /ansoff-matrix-trace-medium or asks for a medium public reasoning trace using Ansoff Matrix.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Ansoff Matrix as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by analyzing growth strategies across products and markets dimensions instead of a generic response.
---

# Ansoff Matrix Reasoning Trace Medium

## Goal
Use Ansoff Matrix to answer the user's question through analyzing growth strategies across products and markets dimensions, not through a generic checklist or interchangeable trace.
The trace should analyze growth opportunities across two dimensions: existing vs new products and existing vs new markets, generating four strategic directions, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make ansoff matrix useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Ansoff Matrix move: analyze growth opportunities across two dimensions: existing vs new products and existing vs new markets, generating four strategic directions.
...
Ansoff Matrix is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "analyze growth opportunities across two dimensions: existing vs new products and existing vs new markets, generating four strategic directions" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Ansoff Matrix trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Ansoff Matrix needs to inspect.
2. Apply the core move explicitly: analyze growth opportunities across two dimensions: existing vs new products and existing vs new markets, generating four strategic directions.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Ansoff Matrix rather than from a generic summary.
