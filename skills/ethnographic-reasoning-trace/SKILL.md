...
description: "Use this skill when the user invokes /ethnographic-reasoning-trace or asks for a default public reasoning trace using Ethnographic Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Ethnographic Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Ethnographic Reasoning strategy.
...
Apply Ethnographic Reasoning by using it to infer meaning from practices, language, setting, and participant perspective, and make each numbered line advance that method rather than adding generic filler.
...
Ethnographic Reasoning is a contextual social analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to infer meaning from practices, language, setting, and participant perspective, which helps the model expose reasoning checkpoints that a reader can inspect.
