---
name: analytic-hierarchy-process-trace-medium
description: >
  Use this skill when the user invokes /analytic-hierarchy-process-trace-medium or asks for a medium public reasoning trace using Analytic Hierarchy Process.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Analytic Hierarchy Process as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by decomposing a decision into a hierarchy and using pairwise comparisons to determine priorities instead of a generic response.
---

# Analytic Hierarchy Process Reasoning Trace Medium

## Goal
Use Analytic Hierarchy Process to answer the user's question through decomposing a decision into a hierarchy and using pairwise comparisons to determine priorities, not through a generic checklist or interchangeable trace.
The trace should decompose a decision into a hierarchy of criteria and alternatives, then use pairwise comparisons to determine priorities, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make analytic hierarchy process useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Analytic Hierarchy Process move: decompose a decision into a hierarchy of criteria and alternatives, then use pairwise comparisons to determine priorities.
...
Analytic Hierarchy Process is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "decompose a decision into a hierarchy of criteria and alternatives, then use pairwise comparisons to determine priorities" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Analytic Hierarchy Process trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Analytic Hierarchy Process needs to inspect.
2. Apply the core move explicitly: decompose a decision into a hierarchy of criteria and alternatives, then use pairwise comparisons to determine priorities.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Analytic Hierarchy Process rather than from a generic summary.
