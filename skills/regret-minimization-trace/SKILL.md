...
description: "Use this skill when the user invokes /regret-minimization-trace or asks for a default public reasoning trace using Regret Minimization. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Regret Minimization Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Regret Minimization strategy.
...
Apply Regret Minimization by using it to compare future remorse across choices and reduce the cost of being wrong, and make each numbered line advance that method rather than adding generic filler.
...
Regret Minimization is a decision analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to compare future remorse across choices and reduce the cost of being wrong, which helps the model expose reasoning checkpoints that a reader can inspect.
