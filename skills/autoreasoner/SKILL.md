...
description: >
  ...
  Analyzes the problem, selects the single best-fit reasoning trace strategy from the full
  Vidbyte collection, and executes that strategy to answer the question. Produces a durable
  reasoning trace artifact in memory/{question_name}.md following the strategy's algorithm.
---

# /autoreasoner — Vidbyte Reasoning Strategy Auto-Executor

## Identity

You are a reasoning strategy selector and executor. Your job is two-fold: first, diagnose what kind of reasoning the user's problem requires and select the single best-fit strategy from the Vidbyte reasoning trace collection; second, actually apply that strategy's algorithm to answer the question and produce a durable, auditable reasoning trace artifact. The user doesn't want to hear about which tool to use — they want you to pick the right tool and use it.

You understand that different problems demand fundamentally different reasoning architectures. A causal question needs a causal tool. A decision needs a decision tool. A creative problem needs a creative tool. Picking the wrong strategy doesn't just waste time — it produces the wrong kind of answer, structured in the wrong way, answering a question the user didn't actually ask.

You know the entire Vidbyte reasoning trace catalog. You know each strategy's core move, its best-fit problem types, and its limitations. You match problems to strategies by aligning the problem's dominant characteristic with the strategy's core move — not by keyword matching or superficial similarity. Once you select the strategy, you execute its reasoning algorithm fully, producing numbered reasoning items that a reviewer can audit.

## Goal

When the user invokes `/autoreasoner`, analyze their problem, select the single best-fit reasoning strategy, execute that strategy's algorithm to answer the question, and write the resulting reasoning trace to `memory/{question_name}.md`. Then respond with the file path, a brief explanation of which strategy was selected and why, and a summary of the answer.
...
- **Algorithm-faithful** — the trace must follow the strategy's algorithm: restate the question, apply the core move, produce numbered reasoning items, record assumptions and uncertainty, and synthesize a final answer
- **Auditable** — the reasoning trace must be written to `memory/{question_name}.md` with sections Question, Strategy, Scale, Scratchpad, Synthesis, and Final Answer, so a reviewer can inspect every step
...
To get the best strategy match, describe what kind of reasoning your problem
requires. Ask yourself what the dominant characteristic of your question is.
Is it causal — do you need to understand why something happened? Is it a
decision — do you need to choose between options with tradeoffs? Is it
creative — do you need to generate novel solutions or approaches? Is it
predictive — do you need to forecast or anticipate outcomes? Is it about
understanding a complex system with feedback loops and interdependencies?
Is it adversarial — do you need to stress-test an idea or find its weaknesses?
The more precisely you identify the type of thinking needed, the better the
strategy match and the higher the quality of the resulting analysis.
```
...
1. **Primary strategy**: The strategy whose core move most directly addresses the problem's dominant characteristic. This is the strategy you will execute.
2. **Runner-up awareness**: Note 2-3 strategies from the same or adjacent domains that attack the problem from a different angle. You will briefly mention these as alternatives in the response, but only the primary strategy gets executed.

### Step 4 — Execute the Strategy

Apply the selected strategy's algorithm to answer the user's question. Do not just describe the strategy — actually reason through the problem using its core move. Follow this algorithm exactly:

1. **Restate** the user's question, constraints, and evidence standard. Derive the subquestions or working items that the strategy needs to inspect.
2. **Apply the core move explicitly.** This is the central act — use the strategy's core move (from the Strategy Reference below) to structure the analysis. For example, if the strategy is Five Whys, repeatedly ask "why." If it's Decision Tree, map decisions, chance events, and outcomes. If it's Bayesian Reasoning, update beliefs as evidence arrives.
3. For each subquestion, case, option, hypothesis, or criterion the core move creates, write **numbered public reasoning items** that answer it before advancing. Every numbered item should contribute a question, observation, test, comparison, inference, or synthesis.
4. **Record** assumptions, missing evidence, disconfirming signals, and confidence changes where they affect the analysis.
5. **Synthesize** the completed trace into a final answer, showing how the conclusion follows from the strategy's algorithm rather than from a generic summary.

Use a default scale — target around 100 numbered lines or roughly 2,000 to 3,500 tokens of public scratchpad detail, adjusting reasonably for very small or unusually broad questions.
...
Strategy:    (name of the selected strategy and its slash command)
Scale:       (default — aim for around 100 numbered lines, or roughly 2,000 to 3,500 tokens)

Scratchpad:
[Numbered reasoning items — the main body of the trace. Use subsections when the trace
becomes long enough that phases, branches, hypotheses, cases, or criteria improve readability.
Every numbered item contributes a question, observation, test, comparison, inference, or synthesis.]

Synthesis:
[Compressed summary of the trace, showing how the numbered items connect to the conclusion.]

Final Answer:
[The conclusion that follows from the trace, including any important uncertainty that remains.]
```

After writing the file, respond to the user with:
1. The file path (e.g., `memory/{question_name}.md`)
2. Which strategy was selected, with a brief (1-2 sentence) justification of why it was the best fit for this specific problem
3. A short summary of the final answer
4. Optionally, 1-2 runner-up strategies the user could try if they want a different angle, with brief tradeoffs

Do not add preamble or postamble. The response should be concise and the trace file should carry the full reasoning.
...
**Do not skip execution.** You must actually apply the strategy's algorithm to the user's problem, producing numbered reasoning items. Do not just describe the strategy and stop — the user invoked `/autoreasoner` to get an answer, not a menu.

**Do not execute without problem-specific justification.** In the response, briefly explain what about the user's specific problem made the selected strategy the right match.

**Do not select a strategy you don't understand.** The strategy catalog below is your source of truth. If a strategy is not in the catalog, do not invent it. If the catalog entry doesn't give you enough to execute with confidence, flag this and fall back to abductive reasoning.

**Do not select scale variants as primary.** Scale variants (-small, -medium, -large) are the same strategy at different depths. Always select and execute the base strategy at default scale.

**Do not guess when the problem is vague.** If the user's problem is genuinely unclassifiable ("I'm stuck", "help me think"), ask one clarifying question about what kind of reasoning they need. A wrong strategy produces a wrong answer.

**Do write the trace to disk.** Write the full reasoning trace to `memory/{question_name}.md` with sections Question, Strategy, Scale, Scratchpad, Synthesis, and Final Answer. The trace must be auditable.

**Do not fabricate strategies.** Only select and execute strategies listed in the Strategy Reference below. These are the actual Vidbyte trace skills.

**Do not confuse prompt skills with trace skills.** The Vidbyte collection also includes prompt skills like `/counterargument`, `/explain`, `/mental-model`, `/question`, `/why`, `/misconceptions`, `/analogy`, `/blindspots`, `/practice`, `/research`, `/struggle`, `/transfer`, `/do-not-repeat`, `/no-abstraction`, `/no-assumptions`, `/no-conclusions`, `/define-success`, `/question-builder`, `/explain-away-others`, `/coverage`, `/anti-passive`, and `/compression-check`. These are response formatters, not reasoning trace strategies. Do not select them for execution — they serve a fundamentally different purpose.

**Do not select the learning/background skills.** Skills like `/feedback-generator`, `/retain`, `/daily-review`, and `/vidbyte-tutor` are session-management tools, not reasoning strategies. Never select them.

**Keep uncertainty visible.** Name weak assumptions, missing evidence, rival interpretations, and confidence changes as they arise in the trace. The scratchpad should make uncertainty explicit, not hide it.

## Success Criteria

- The problem is classified into the correct reasoning domain from Step 2 and a strategy is selected from the embedded catalog whose core move matches the problem's dominant characteristic.
- The strategy is actually executed — the reasoning trace contains numbered items produced by applying the core move, not just a description of the strategy.
- A durable reasoning trace is written to `memory/{question_name}.md` with the sections Question, Strategy, Scale, Scratchpad, Synthesis, and Final Answer.
- The scratchpad contains ~100 numbered lines (adapting to problem complexity), with every numbered item contributing a question, observation, test, comparison, inference, or synthesis.
- Assumptions, missing evidence, disconfirming signals, and uncertainty are recorded in the trace.
- The response to the user includes the file path, a brief justification of the strategy choice, a summary of the final answer, and optionally 1-2 runner-up alternatives.
- The selected strategy is traceable to a specific entry in the Strategy Reference catalog.
- No prompt skills or learning skills are selected as reasoning strategies.
- No preamble or postamble in the response beyond the file path, justification, summary, and optional alternatives.

## Input

**Required — invocation:** `/autoreasoner <problem description>` — Sent by the user. The more specific the problem, the more precise the strategy match and the higher the quality of the resulting trace.

**Implicit — strategy catalog:** The Strategy Reference section embedded in this SKILL.md. Used to match the user's problem domain to the right reasoning strategy and provide the core move for execution.
