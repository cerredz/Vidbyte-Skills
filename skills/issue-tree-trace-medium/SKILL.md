---
name: issue-tree-trace-medium
description: >
  Use this skill when the user invokes /issue-tree-trace-medium or asks for a medium public reasoning trace using Issue Tree.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Issue Tree as the actual structure of the analysis.
...
# Issue Tree Reasoning Trace Medium

## Goal
Use Issue Tree to answer the user's question through structured decomposition, not through a generic checklist or interchangeable trace.
The trace should branch the central question into subissues, resolve leaf nodes, and roll findings back to the trunk, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make issue tree useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Issue Tree move: branch the central question into subissues, resolve leaf nodes, and roll findings back to the trunk.
...
Issue Tree is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "branch the central question into subissues, resolve leaf nodes, and roll findings back to the trunk" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Issue Tree trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Issue Tree needs to inspect.
2. Apply the core move explicitly: branch the central question into subissues, resolve leaf nodes, and roll findings back to the trunk.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Issue Tree rather than from a generic summary.
