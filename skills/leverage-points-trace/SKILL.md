...
description: "Use this skill when the user invokes /leverage-points-trace or asks for a default public reasoning trace using Leverage Points. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Leverage Points Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Leverage Points strategy.
...
Apply Leverage Points by using it to find places where small changes can shift system behavior materially, and make each numbered line advance that method rather than adding generic filler.
...
Leverage Points is a systems intervention strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to find places where small changes can shift system behavior materially, which helps the model expose reasoning checkpoints that a reader can inspect.
