...
description: "Use this skill when the user invokes /values-tradeoff-trace or asks for a default public reasoning trace using Values Tradeoff Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Values Tradeoff Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Values Tradeoff Reasoning strategy.
...
Apply Values Tradeoff Reasoning by using it to make competing values explicit and explain which value prevails under which condition, and make each numbered line advance that method rather than adding generic filler.
...
Values Tradeoff Reasoning is a ethical decision analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to make competing values explicit and explain which value prevails under which condition, which helps the model expose reasoning checkpoints that a reader can inspect.
