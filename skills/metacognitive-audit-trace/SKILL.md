...
description: "Use this skill when the user invokes /metacognitive-audit-trace or asks for a default public reasoning trace using Metacognitive Audit. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Metacognitive Audit Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Metacognitive Audit strategy.
...
Apply Metacognitive Audit by using it to inspect the reasoning process itself for bias, gaps, overconfidence, and poor framing, and make each numbered line advance that method rather than adding generic filler.
...
Metacognitive Audit is a thinking quality review strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to inspect the reasoning process itself for bias, gaps, overconfidence, and poor framing, which helps the model expose reasoning checkpoints that a reader can inspect.
