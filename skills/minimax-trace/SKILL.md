---
name: minimax-trace
description: "Use this skill when the user invokes /minimax-trace or asks for a default public reasoning trace using Minimax Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Minimax Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Minimax Reasoning strategy.
...
Apply Minimax Reasoning by using it to choose the option whose worst credible outcome is strongest, and make each numbered line advance that method rather than adding generic filler.
...
Minimax Reasoning is a adversarial decision analysis strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to choose the option whose worst credible outcome is strongest, which helps the model expose reasoning checkpoints that a reader can inspect.
