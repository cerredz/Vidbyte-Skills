---
name: scenario-planning-trace-medium
description: >
  Use this skill when the user invokes /scenario-planning-trace-medium or asks for a medium public reasoning trace using Scenario Planning.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Scenario Planning as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by future uncertainty analysis instead of a generic response.
---

# Scenario Planning Reasoning Trace Medium

## Goal
Use Scenario Planning to answer the user's question through future uncertainty analysis, not through a generic checklist or interchangeable trace.
The trace should construct distinct plausible futures and test decisions across them, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make scenario planning useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Scenario Planning move: construct distinct plausible futures and test decisions across them.
...
Scenario Planning is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "construct distinct plausible futures and test decisions across them" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Scenario Planning trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Scenario Planning needs to inspect.
2. Apply the core move explicitly: construct distinct plausible futures and test decisions across them.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Scenario Planning rather than from a generic summary.
