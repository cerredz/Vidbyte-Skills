...
description: "Use this skill when the user invokes /nonmonotonic-reasoning-trace or asks for a default public reasoning trace using Nonmonotonic Reasoning. It writes a structured scratchpad to root memory/{question_name}.md and scales the trace to 100 numbered scratchpad lines unless the user explicitly requests a different scale."
---

# Nonmonotonic Reasoning Reasoning Trace

## Goal
Use this skill to produce a public, audit-friendly reasoning trace for the user's question using the Nonmonotonic Reasoning strategy.
...
Apply Nonmonotonic Reasoning by using it to allow new information to reduce or invalidate earlier conclusions, and make each numbered line advance that method rather than adding generic filler.
...
Nonmonotonic Reasoning is a revisable logic strategy that gives the trace a specific shape instead of a loose stream of thoughts.
Its core move is to allow new information to reduce or invalidate earlier conclusions, which helps the model expose reasoning checkpoints that a reader can inspect.
