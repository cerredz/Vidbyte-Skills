---
name: decision-matrix-trace-medium
description: >
  Use this skill when the user invokes /decision-matrix-trace-medium or asks for a medium public reasoning trace using Decision Matrix.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Decision Matrix as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by evaluating alternatives against weighted criteria to identify the best option instead of a generic response.
---

# Decision Matrix Reasoning Trace Medium

## Goal
Use Decision Matrix to answer the user's question through evaluating alternatives against weighted criteria to identify the best option, not through a generic checklist or interchangeable trace.
The trace should score each alternative against weighted criteria and select the option with the highest total weighted score, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on subquestions, assumptions, evidence, contrasts, tests, intermediate conclusions, and implications, because those artifacts make decision matrix useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Decision Matrix move: score each alternative against weighted criteria and select the option with the highest total weighted score.
...
Decision Matrix is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "score each alternative against weighted criteria and select the option with the highest total weighted score" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Decision Matrix trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Decision Matrix needs to inspect.
2. Apply the core move explicitly: score each alternative against weighted criteria and select the option with the highest total weighted score.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Decision Matrix rather than from a generic summary.
