...
Tradeoff Matrix is a comparative decision analysis strategy that gives analysis a recognizable pattern and prevents the answer from becoming an unstructured opinion.
Its central discipline is to score options across criteria and expose the compromises behind the recommendation, which forces the model to make the important reasoning moves visible.
...
Compared with a generic scratchpad, tradeoff matrix changes what gets noticed, which alternatives get compared, and which assumptions receive pressure.
...
The final answer should feel like the compressed result of Tradeoff Matrix, not like a separate response pasted after the trace.
