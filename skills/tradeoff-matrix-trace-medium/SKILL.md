---
name: tradeoff-matrix-trace-medium
description: >
  Use this skill when the user invokes /tradeoff-matrix-trace-medium or asks for a medium public reasoning trace using Tradeoff Matrix.
  The skill writes a durable scratchpad to root memory/{question_name}.md and uses Tradeoff Matrix as the actual structure of the analysis.
  ...
  Use this skill when the user wants the answer shaped by comparative decision analysis instead of a generic response.
---

# Tradeoff Matrix Reasoning Trace Medium

## Goal
Use Tradeoff Matrix to answer the user's question through comparative decision analysis, not through a generic checklist or interchangeable trace.
The trace should score options across criteria and expose the compromises behind the recommendation, so the visible reasoning follows the same path the strategy is known for.
The goal is to create a public scratchpad that a reviewer can audit without exposing hidden private chain-of-thought.
Center the scratchpad on options, probabilities, tradeoffs, risks, sensitivities, and consequences, because those artifacts make tradeoff matrix useful rather than decorative.
...
Build the scratchpad by repeatedly applying the Tradeoff Matrix move: score options across criteria and expose the compromises behind the recommendation.
...
Tradeoff Matrix is a reasoning strategy for turning a question into an auditable public trace instead of an unstructured opinion.
...
Use it when the problem benefits from the core move "score options across criteria and expose the compromises behind the recommendation" and when the final answer needs evidence, assumptions, and uncertainty kept in view.
A strong Tradeoff Matrix trace lets that move determine the order and granularity of the scratchpad, then compresses the result into a concise final answer.

Algorithm for the output document:
1. Restate the user's question, constraints, and evidence standard, then derive the subquestions or working items that Tradeoff Matrix needs to inspect.
2. Apply the core move explicitly: score options across criteria and expose the compromises behind the recommendation.
...
5. Synthesize the completed trace into the final answer, showing how the conclusion follows from Tradeoff Matrix rather than from a generic summary.
