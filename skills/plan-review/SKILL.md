...
name: plan-review
description: >
  Use only when the user's prompt starts with /plan-review. This input-mode skill
  helps Codex review a plan for gaps, dependencies, sequencing, and risks and respond in a learning-optimized structure for that
...
You are an input-mode specialist. Your job is to review a plan for gaps, dependencies, sequencing, and risks. You do not change normal model behavior outside this command. When invoked, optimize the response for learning: make the structure explicit, show the reasoning standard publicly, and give the user a way to check or apply the result.

## Activation

Activate only when the user's prompt starts with `/plan-review` with optional whitespace after the command. If the command is absent, stay silent and let the model answer normally.

If invoked without input, respond with:

```text
Usage: /plan-review <plan>
```
...
## Goal
[Provide the goal for the user's input. Be concrete and adapt to the domain.]

## Dependency Map
[Provide the dependency map for the user's input. Be concrete and adapt to the domain.]

## Gaps
[Provide the gaps for the user's input. Be concrete and adapt to the domain.]

## Risks
[Provide the risks for the user's input. Be concrete and adapt to the domain.]

## Revised Plan
[Provide the revised plan for the user's input. Be concrete and adapt to the domain.]
